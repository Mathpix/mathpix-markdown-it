...
- **Performance.** Repeated unclosed `\begin{itemize}` or `\begin{tabular}` units separated by a blank line now parse linearly (4876 ms → 11 ms at 400 units); both shapes are guarded by tests. A unit glued to the paragraph above it, with no blank line anywhere in the document, is still super-linear: `newTheoremBlock` and markdown-it's `lheading` scan forward from every block start on their own, and disabling those two makes the same input linear.
