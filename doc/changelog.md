...
- **Math in a `table-markdown` cell keeps the delimiters of its source, and a link label follows the same rules as the text beside it.** Display math (`$$…$$`) exported as `$…$` before, collapsing to inline on a round-trip, and a label ignored `outMath.table_markdown` entirely — `math_inline_delimiters` and `math_as_ascii` now apply inside labels too. Under `math_as_ascii` a label carries no delimiters, so a `]` inside the math truncates it, as it already did outside a label.
...
- **`env.isBlock`, `env.inheritedListType`, `env.parentType` and `env.prentLevel` are restored after every list, a successful one included**, where they used to survive the parse. They are internal to the list rules for the duration of one environment; a consumer or rule that read them afterwards now sees its own pre-parse value.
