...
A markdown link in a `\begin{tabular}` cell skipped two tokens too many, so its `</a>` was never emitted and the following content was absorbed: `[t](url) tail` rendered as `<a …>t tail` with the anchor left open, a second link lost its opening tag, and `**bold**` after a link lost its `<strong>`. The `table-markdown` export dropped the same content and is fixed with it. The link now ends at its own `link_close`. Known limitation, unchanged by this fix: `tsv`/`csv` record a link by its href only, so the link text is absent there, and the smoothed (pptx) output carries the opening `<a>` without the text or its closing tag. Markdown tables (`| … |`) were never affected — they use a different render path. Pre-existing; unrelated to the list fixes above.
