...
- Caption numbering (`Figure N` / `Table N`) is no longer shifted by a speculative list parse: a `\footnote` after a list containing a `\begin{figure}` no longer bumps the number, and a figure inside a list is now `Figure 1` (was `Figure 3`). Deep-import helpers `ClearTableNumbers`/`ClearFigureNumbers` are renamed `clearTableNumbers`/`clearFigureNumbers` (not in the public entry, so only deep imports are affected).
- Marker `padding-inline-start` is now emitted in `em` (was `px`), so the marker indent scales with the container `font-size` and no longer clips math markers. The default list indent is `2.5em` (was `40px`); a custom indent is emitted only when the marker is wider, clamped at `20em`, and attributed per nesting level — so a wide marker in a nested list no longer inflates the outer list, and all list forms (own-line, same-line, single-line, in a table cell) resolve the same. **Migration:** `data-padding-inline-start` now holds an em value with its unit (e.g. `"3.51em"`, was a bare px number) — downstream consumers reading this attribute must update. See the spec for the width model.
