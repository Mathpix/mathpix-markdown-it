...
- **LaTeX lists.**
  ...
  - Not fixed: a leftover after the outermost closer that opens a list of its own is still dropped (plain text and a block env there now render — see below); and `\verb` is not modelled as verbatim, so `\item \verb|\end{itemize}|` still ends the list at that closer and prints what follows as literal LaTeX rather than as items — byte-identical to 3.0.1.
...
- **Every list reads its own copy of the marker tokens on `*_list_open.itemizeLevel`.** A macro is parsed once per render and the copy carries its own `attrs`, `children`, `meta` and `map`, so a write into a marker — a consumer's or a render rule's — stays with that list. Sharing the tokens leaked: a write reached every later list with the same marker, and a marker built from a link or `\includegraphics` collected `target`/`style` from the renderer itself, so the second `<a>` carried each attribute twice and the third three times, where 3.0.1 rendered each list alike. Copying is not measurable over the fixture corpus; on 400 lists of five items with a math marker it costs about 1 ms of 22, where the same input takes 31 ms on 3.0.1.
...
- **Deep imports:** `ClearTableNumbers`/`ClearFigureNumbers` → `clearTableNumbers`/`clearFigureNumbers`, moved to `lib/markdown/common/caption-counters` with no re-export; `getListLevelState` and the unused `ListItemsBlock` are gone (level state is a stack now, read through `getListDepth`); `ListItemsResult`/`ListInlineContext` dropped `padding` and gained required `openTokens`/`allListTokens`, and `ListInlineContext.li.value` narrowed from `any` to `number`; the list rule's view of its source text lives in the new `md-latex-lists-env/list-source-model`, which takes over `splitInlineListEnv` from `latex-list-items`, and the brace pairing under it in `common/argument-spans` (`braceMatches`, `commandArgumentSpans`), both exported for their own tests rather than for use; `skipBackticks` in `lib/markdown/utils` answers past an unmatched run of backticks instead of the end of the string, matching how such a run renders — a deep importer using it to find code spans sees that change; `lib/markdown/common` gains `skipOptionalArg`, the one reader for a `[...]` option that the three places skipping one now share; `md-latex-lists-env/latex-list-env-engine` gains `listRuleFailureCount`/`resetListRuleFailures`, a count of renders where the list rule failed and fell back to literal LaTeX, exported for the suite to assert zero rather than for use, and `list-source-model` gains `unanchoredOffsetCount`/`resetUnanchoredOffsets` in the same shape; `common/consts` exports `LATEX_ITEM_COMMAND_INLINE_RE` with a narrower meaning — it matched the bare word `item` and now needs a `\item` with a command boundary, so `\itemsep` and prose are no longer matches; `md-latex-lists-env/latex-list-types` widens `OpaqueEnvType` from `lstlisting`/`tabular` to five more names (`table`, `figure`, `center`, `left`, `right`), which is additive for code reading the type and breaking for code producing it; `lib/styles/styles-code` alone no longer sizes code text (its `pre` base moved to `MathpixStyle`).
...
- **Text before a `\diagbox` on the same line is no longer eaten.** The inline rule runs at every backslash and asked an unanchored pattern, so standing at one it matched a `\diagbox` further along and consumed everything between: `\alpha VISIBLE text \diagbox{a}{b}` lost `VISIBLE text`, on 3.0.1 too. It is anchored now, so the rule answers for its own position only — `\diagbox`, `\slashbox` and `\backslashbox` render exactly as before wherever one really starts. The scan of the rest of the source at every backslash goes with it: a 183KB document dense in backslashes renders in 340ms against 1206.

- **A chunk before the first `\item` is wrapped in a marker-less `<li>`**, where text, a fence or a block env used to sit straight inside the `<ul>`, which admits only `<li>`. A sublist written in that chunk moves inside the wrapper. A chunk holding only `\setcounter` or `\renewcommand` gets no wrapper: those apply to the list and print nothing, so the `<li>` was invisible in the HTML while a consumer reading the token stream for LaTeX emitted an `\item` the source never had.
- **A list closer written in a command argument no longer closes the list.** A rule that re-parses its own argument — `\footnote`, `\footnotetext`, `\text`, `\textbf` — shares the `env` the list set, so an `\end{itemize}` in there took the list's level from inside that parse: on `3.0.1` `\text{f \end{itemize} g}` emitted a `</ul>` with no opener, and here the list was left open instead, taking the rest of the document into it. The list rules now answer only for the source the list handed them, as they already did for a marker body. `\caption`, `\label` and `\url` were never affected — their own rules take the argument first.
- **Text sharing a line with `\renewcommand` is no longer eaten.** The block rule claimed the whole line and renders to nothing, so `\renewcommand{\x}{y} FFF` lost `FFF` — in a list body once such a chunk reached the block path, and in any document on `3.0.1` too. The rule applies the command and leaves the line to the paragraph when something else is on it; the inline rule reads the command there, as it already did mid-paragraph. Verbatim is untouched, no block rule running inside a fence, `lstlisting` or `tabular`. **If you read a `renewcommand` token's `latex`**, note that such a line now yields the inline rule's payload — everything past the command name (`{\x}{y} FFF`) instead of the whole command (`\renewcommand{\x}{y} FFF`); the shape a line without a tail gives is unchanged.
...
- **Text written after the outermost `\end{itemize}` on that line is no longer dropped.** `\end{itemize} tail text` lost `tail text`, and the same position lost a `}` left by an unclosed command, a code span, and the opening of a fence — the rule reported success having dropped them. That stretch of the line now goes back to the block phase and renders as it would anywhere else (`tail text` becomes the same `<div>` a standalone paragraph does) — including when the line also carries a `\renewcommand`, a `\setcounter` or a wrapper env closed on it, where the handoff pointed inside the eaten command and printed a truncated one. Two shapes are not covered. A leftover that opens a list of its own is still dropped, as on 3.0.1: read from that point it would become a top-level list, skipping the check that declined it as a sibling. And a wrapper env after the *outermost* closer — `\begin{itemize}\n\item a\n\end{itemize} \begin{center}c\end{center}` — is kept but not rendered: the list rule declines that line, the paragraph rule takes it, and inside a paragraph `\begin{center}` has no rule to render it, so it prints as literal LaTeX with the list beside it. On 3.0.1 the wrapper was dropped instead, so the output changed. `tabular` and `lstlisting` do have an inline rule and render there, as they did. After a *nested* closer every wrapper renders — that is the next entry.
