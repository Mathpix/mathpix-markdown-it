...
- Caption numbering (`Figure N` / `Table N`) is no longer shifted by a speculative list parse: a `\footnote` after a list containing a `\begin{figure}` no longer bumps the number, and a figure inside a list is now `Figure 1` (was `Figure 3`). A discarded parse also no longer leaves the caption text of an unconsumed `\caption` on the shared parser state, where a later float could pick it up. Deep-import helpers `ClearTableNumbers`/`ClearFigureNumbers` are renamed `clearTableNumbers`/`clearFigureNumbers` (not in the public entry, so only deep imports are affected).
