...
- **Performance.**
  - Repeated unclosed `\begin{itemize}` or `\begin{tabular}` units no longer cost a scan per line: 400 units go from 865 ms to 12 ms. Input interleaving real closers with unclosed envs is improved but still super-linear, as is a unit glued to the paragraph above it.
  - Well-formed lists parse faster than 3.0.1 — ~48% on 400 list units, ~30% on a mixed 24 KB document, ~14% with a wrapper env in every list — mostly because the itemize marker macros are parsed once per document instead of on every list open. Peak heap is ~8% lower.
