...
- Marker width is measured more accurately, so markers no longer get too small an indent: non-ASCII letters are classified by case instead of all counting as narrow, so an uppercase Cyrillic/Greek/accented-Latin marker (`\item[ПРИМЕЧАНИЕ]`) is no longer clipped by the item text; East-Asian Wide/Fullwidth characters are measured as wide glyphs (`\item[11．]`, U+FF0E, no longer undercounted vs `\item[11.42]`) — including the ones outside the BMP, so an emoji or a rare-ideograph marker reserves the same as a common ideograph (`\item[😀😀]` now indents `3.03em`, the same as `\item[一一]`, and used to get no custom indent at all); math markers (`\item[$x^4+x^4$]`) use their rendered `widthEx` instead of contributing 0; wrapped markers (`\item[\textbf{…}]`) and code-span markers (`\item[\`x\`]`) are measured through their content. Edge whitespace in a marker (`\item[  wide  ]`) no longer inflates the padding — the marker is now parsed trimmed, so it is both measured and rendered without the edge spaces (its HTML changes from `<span class="li_level">  wide  </span>` to `<span class="li_level">wide</span>`).
- Empty `<>` list-item bodies (from a leaked internal `env` flag, closed on two independent paths) are fixed: an unclosed `itemize` before a `tabular`, and a figure-list followed by a `tabular`-list, now render correctly.
- Caption numbering (`Figure N` / `Table N`) is no longer shifted by a speculative list parse: a `\footnote` after a list containing a `\begin{figure}` no longer bumps the number, and a figure inside a list is now `Figure 1` (was `Figure 3`). A discarded parse also no longer leaves state behind on the shared parser env — the caption text of an unconsumed `\caption`, which a later float could pick up, along with the rest of the float and tabular env its body wrote. Deep-import helpers `ClearTableNumbers`/`ClearFigureNumbers` are renamed `clearTableNumbers`/`clearFigureNumbers` (not in the public entry, so only deep imports are affected).
- Inline content that sits directly in a `\begin{tabular}` cell — with no paragraph wrapper — is now collected into the `table-markdown` / `tsv` / `csv` export instead of being rendered to HTML only. The reported case is a list written entirely on one line in a cell, which unwraps its item bodies to such content: `\item[x] d` exported as `x ` without the `d`, and now matches the multi-line form. The same branch covers any inline run in that position (text, links, images, bold/italic, code spans, inline math), so those now reach the export too. The exported markdown also gains a `<br>` before a nested sublist, as the multi-line form already had. HTML output is unchanged. **Migration:** consumers diffing `table-markdown`/`tsv`/`csv` for cells containing single-line LaTeX lists will see these two differences — for `\begin{itemize}\item[1.] a \begin{itemize}\item[x] d\end{itemize}\end{itemize}` in a cell, `table-markdown` goes from `| 1. a&#160;&#160;x <br> |` to `| 1. a<br>&#160;&#160;x d<br> |` (the trailing `<br>` is unchanged).
- Marker `padding-inline-start` is now emitted in `em` (was `px`), so the marker indent scales with the container `font-size` and no longer clips math markers. The default list indent is `2.5em` (was `40px`); a custom indent is emitted only when the marker is wider, with the reserved part clamped at `20em` (the per-level `2.5em` default still accumulates on top), and attributed per nesting level — so a wide marker in a nested list no longer inflates the outer list, and all list forms (own-line, same-line, single-line, in a table cell) resolve the same. **Migration:** `data-padding-inline-start` now holds an em value with its unit (e.g. `"3.51em"`, was a bare px number) — downstream consumers reading this attribute must update. See the spec for the width model.
- `\item` detection now requires a real `\item` command: the bare word "item" (no backslash), and `\item`-prefixed commands like `\itemsep`/`\itemindent`, are no longer mistaken for `\item`. A list-item body that mentions "item" — including a multiline `\footnote`/`\footnotetext` inside the item — no longer splits into a spurious item or leaks the footnote as literal text. The same anchoring applies where a line is split into items, so an `\itemsep` in the middle of a continuation line no longer starts a new item (which also dropped the space before it). Unsupported commands still render as literal text, as any unrecognised LaTeX command does — note that before the first `\item` such text lands directly inside the `<ul>`, which is invalid HTML, so consumers that walk the DOM (`forDocx`, sanitisers, table extraction) may drop or move it. This PR makes it more visible rather than more frequent: `\itemsep` used to be mistaken for `\item` and swallowed into an `<li>`. (Pre-existing; item detection now anchors on `\item` + a command boundary.)
- The `\footnote`/`\footnotetext` block rules stop at a `\begin{itemize}`/`\begin{enumerate}`, so a list right after a paragraph (no blank line) is no longer swallowed and rendered as literal text. This also keeps the scan linear on repeated list + footnote input without blank separators (was O(N²); guarded by a scaling test).
- List parsing is faster, most of all where it used to degrade quadratically. Repeated paragraph + list + `\footnote`/`\footnotetext` units without blank separators: 200 units 47 → 24 ms, 400 units 152 → 43 ms (the scan now stops at the list, and the "does a list start here" answer is reused within a parse instead of re-parsing the list on every scan). A document built from **unclosed** `\begin{itemize}` starts no longer degrades quadratically either — 100 units 197 → 6 ms, 200 units 892 → 10 ms, 400 units 4876 → 11 ms (parse only), guarded by a scaling test. Such an env can only be rejected, but that was discovered by reading the body to the end of the document on every probe, and terminator scans probe every line; both the block rule and the inline aggregator now look for a closing `\end` first and answer immediately when there is none. These shapes still grow faster than linearly at larger sizes (193 ms at 400 units): what remains is an unterminated forward scan in `newTheoremBlock` and in markdown-it's own `lheading`, neither touched by this release, and reachable with no list in the input at all. A blank line between blocks avoids it. Lists holding a figure: 22 → 18 ms — neither the speculative-parse env rollback nor the `envToInline` snapshot `delete`s keys any more, which had moved the parser `env` and every snapshot taken from it into a slow property mode (a list of tabular items: 22 → 19 ms). A plain list-heavy document is roughly unchanged (4119 lines of lists: 60 → 58 ms). Output is unchanged throughout. **Migration:** a consumer that passes its own `env` to `md.parse()`/`md.render()` and inspects it afterwards will now see the internal list/float keys present with value `undefined` instead of absent (`'isBlock' in env` is now `true`); every value-level check behaves as before.
- A document with unclosed `\begin{tabular}` environments and no blank lines between them parses dramatically faster: 100 units 160 → 24 ms, 200 units 1239 → 82 ms, 400 units 9422 → 309 ms. An env with no `\end{tabular}` can only be rejected, but that was decided only after reading the rest of the document, once for every line a paragraph or footnote scan asks about — so the cost grew quadratically. The rule now looks for a closing `\end{tabular}` first and rejects immediately when there is none. Output is unchanged, and no list is involved: this is the same pattern as the list fix above, in the tabular rule, and it dominated every measurement of malformed-LaTeX input.
...
### A link inside a tabular cell no longer swallows the rest of the cell

A markdown link in a `\begin{tabular}` cell skipped two tokens too many, so its `</a>` was never emitted and the following content was absorbed: `[t](url) tail` rendered as `<a …>t tail` with the anchor left open, a second link lost its opening tag, and `**bold**` after a link lost its `<strong>`. The `table-markdown` export dropped the same content and is fixed with it. The link now ends at its own `link_close`. Two more link outputs are fixed with it: a link whose text is itself formatted now exports its full label (`[**bold** x](http://a.b)` used to come out as `[](http://a.b)`, since the Markdown writer read only the token right after `link_open`), and the smoothed (pptx) output no longer carries an opening `<a>` without its text or closing tag, nor the raw HTML of a table written inside a link label (`[\begin{tabular}{l}q\end{tabular}](url)` in a cell now smooths to `q` there, as the same table outside a link already did). Reading the whole label also means labels that carry no Markdown marker of their own keep the text they had before: an image label falls back to its alt text (`[![alt](i.png)](url)` → `[alt](url)`) and an inline-math label to its LaTeX source (`[$x^2$](url)` → `[x^2](url)`), while a `<smiles>` label now keeps its full structure (`[<smiles>CCO</smiles>](url)`, was `[CCO](url)`) and so matches the same content written outside a link. Brackets are handled on the way out: a `]` inside a text label is re-escaped (`[a\]b](url)`), since markdown-it hands it over unescaped and it would otherwise close the label early, but a `]` inside an inline-math label is left alone to keep the LaTeX verbatim — so that one label still breaks, as it did before. By design, `tsv`/`csv` still record a link by its href alone — those outputs are for spreadsheet consumers, where the URL is the machine-readable datum and the label is available from the HTML and Markdown outputs. Markdown tables (`| … |`) were never affected — they use a different render path. Pre-existing; unrelated to the list fixes above.
