...
- A list written entirely on one line inside a table cell no longer loses its item bodies in the `table-markdown` / `tsv` / `csv` export: `\item[x] d` exported as `x ` without the `d`, and now matches the multi-line form. The exported markdown also gains a `<br>` before a nested sublist, as the multi-line form already had. HTML output is unchanged. **Migration:** consumers diffing `table-markdown`/`tsv`/`csv` for cells containing single-line LaTeX lists will see these two differences — for `\begin{itemize}\item[1.] a \begin{itemize}\item[x] d\end{itemize}\end{itemize}` in a cell, `table-markdown` goes from `| 1. a&#160;&#160;x <br> |` to `| 1. a<br>&#160;&#160;x d<br> |` (the trailing `<br>` is unchanged).
...
- List-heavy documents parse faster still: the marker width is measured through a glyph-class lookup table instead of per-character regexes, and the speculative-parse env rollback no longer `delete`s keys — `delete` moved the shared parser `env` into a slow property mode for the rest of the parse. Measured on 40 lists × 150 items: 44 → 39 ms; on lists holding a figure: 22 → 17 ms. **Migration:** a consumer that passes its own `env` to `md.parse()`/`md.render()` and inspects it afterwards will now see the internal list/float keys present with value `undefined` instead of absent (`'isBlock' in env` is now `true`); every value-level check behaves as before.
- Documents with many LaTeX lists parse faster: the paragraph and footnote block scans ask repeatedly whether a list starts on a given line, and that answer is now reused within a parse instead of re-parsing the list every time. Measured against 3.0.1: 162 ms → 103 ms on a 4060-line document of lists, 92 ms → 54 ms with a trailing `\footnote`. Output is unchanged.
