...
Five independent groups in one publish. Read **Breaking changes** before upgrading — the six entries under **Fixes that change existing output** need no action. The specs carry the measurements and the reasoning.

- **LaTeX lists.**
  - Markers no longer clip their item text: width is measured per glyph class in `em` (non-ASCII, East-Asian Wide, monospace, math by its rendered width), reserved per nesting level only when it overflows, clamped at `20em`. Items holding a block env are measured too.
  - A block env in a list body renders the same wherever it sits: `table`, `figure`, `center`, `left` and `right` gain their wrapper and caption after a closed sublist or before the first `\item`, and a `tabular` there gains the `table_tabular` wrapper it already had elsewhere.
  - Those five envs are opaque to the list scanner, like `tabular` and `lstlisting` already were, so `\item` in a caption or a stray `\end{itemize}` inside one is text rather than structure — both used to wreck the list. A wrapper stays transparent only when going opaque would leave the list with nothing to close it, or when a list starts inside the wrapper.
  - A single-line `\begin{tabular}{l}q\end{tabular}` in a list body no longer takes the list down with it, nor does one nested on a single line inside another `tabular` (`A & \begin{tabular}{l}x\end{tabular} \\`) — that shape printed the whole list as literal LaTeX.
  - A brace or a stray `\end{itemize}` written in inline code, a fenced block, an `lstlisting` or math no longer costs the list an item: that content is located once per document — math within its paragraph, as its rules pair it — and read the same way by every check. Kinds that overlap are unioned, so a lone backtick inside a listing opens no code span across it. The same input inside `\verb`, an indented block, an html block or a `tabular` renders too, by the rule below.
  - An `\end{itemize}` written inside a wrapper env is taken as its content when the source past the wrapper still closes the list and no list starts inside it — `tabular` always did, and `center`, `left`, `right`, `table` and `figure` now agree: both items survive and the wrapper renders, where the list used to end at that closer with the rest printed as literal LaTeX.
  - A list written inside such a wrapper keeps its nesting depth: `•  –  •` and `decimal | lower-alpha`, where it used to claim top level.
  - Collapsed closers parse, a mid-line closer can open a sibling list, and `\begin`/`\end` are handled in source order — each of these used to leave the outer list unclosed and printed as literal text.
  - Also fixed: empty `<>` item bodies, `Figure N`/`Table N` shifted by a speculative parse, `\itemsep`/`\itemindent` mistaken for `\item`, a list swallowed by a preceding `\footnote` scan, and an orphan `<br>` after a `\renewcommand` line inside a list body (the break stays under `forLatex`, where that line is source to rebuild).
  - Core-markdown `<ul>`/`<ol>` indent `2.5em` instead of the browser default, scoped to `#preview-content`/`#setText`.
  - A host that hands the same `env` object to every render gets the same markers each time: the parsed marker tokens are dropped in the per-render reset, so a render rule that writes into one cannot carry that write into the next document. Within one render those tokens are still shared between its lists — cloning them per list cost 12–29% on list-heavy input, so that boundary stays and is pinned by a test.
  - `data-padding-inline-start` that is not a bare `em` value is still ignored, and now says so once with `console.warn` instead of dropping silently.
  - Diagnostics are deduped per cause family (40 keys each) as well as overall (200), so a document minting hundreds of one kind no longer silences another subsystem's single warning.
  - Not fixed: text sitting after the outermost closer on the same line is dropped, as before.
- **Tabular cells.**
  - Inline content sitting directly in a cell now reaches the `table-markdown`/`tsv`/`csv` export; a single-line nested list used to lose its sublist item bodies there.
  - A link exports its whole label instead of swallowing the rest of the cell, and no longer leaves an open `<a>` in the pptx output. `tsv`/`csv` record a link by its href and an image by its src, as the option docs now say.
- **Performance.**
  - Repeated unclosed `\begin{itemize}` units no longer cost a scan per line: 400 of them go from 865 ms to 12 ms, and the growth is linear. Unclosed `\begin{tabular}` gains as much in absolute terms — 400 units 6950 ms → 138 ms — but its growth stays super-linear (13× over a 4× input, against 58× before). Input interleaving real closers with unclosed envs, and a unit glued to the paragraph above it, are improved and still super-linear too.
  - A footnote at the end of a document costs no ruler walk per block ahead of it, and no list probe per line when the document holds no list: 400 such paragraphs parse as fast as 3.0.1.
  - Well-formed lists parse faster than 3.0.1 — ~46% on 400 list units, ~24% on a mixed 24 KB document, ~16% with a wrapper env in every list — mostly because the itemize marker macros are parsed once per document instead of on every list open. Peak heap over 20 parses of that mixed document is ~20% lower (42–49 MB against 57–59).
- **Command arguments with `\\`.** `findEndMarker` read a preceding `\` as an escape without asking whether that `\` was itself escaped, and `\\` is a line break: `\caption{first line \\}` produced no caption at all, `\section{title \\}` no heading, and `\caption{a \\{b} c}` lost everything after the inner brace. All three render now, which moves the numbering below such a caption — see **Breaking changes**. Looking up a formula's end marker no longer recurses per escaped marker either: a document holding a long run of them (`$$` then `\$$` repeated) came back empty and now renders.
...
- **A caption, heading or footnote whose argument ends in `\\` renders, and shifts the numbering below it.** The argument used to read as unclosed, so the whole command was dropped: `\caption{First line \\}` produced no caption and no `number`, and the next table took `Table 1`. It now renders and that table becomes `Table 2`; the same shift applies to `Figure N` and to section numbers, and a `\footnote{note \\}` that printed as literal text now produces a footnote and its `<section class="footnotes">`. A consumer keying off those numbers, or diffing the HTML, sees the difference. The break itself is not rendered everywhere: a heading emits `<br>`, while a caption and a footnote keep a literal `\` (`Table 1: first line \`) — the same as `\\` written mid-argument, which reads that way on `master` too. Pinned by fixtures, unifying it is a follow-up.
- **`table-markdown` exports a link's address as written**, and the whole label with it, where only its first token was exported before. The escaping guards Markdown syntax, not the URL scheme. A link the validator rejects is not a link on either path — no `<a>`, and the literal `[label](address)` in both the HTML and the export — so a reader that re-parses the export with a permissive validator is the one that resurrects it. Measured identical to 3.0.1 on every scheme tried, custom `validateLink` included; see **Security notes on the Markdown exports** in the README.
...
- **Deep imports:** `ClearTableNumbers`/`ClearFigureNumbers` → `clearTableNumbers`/`clearFigureNumbers`, moved to `lib/markdown/common/caption-counters` with no re-export; `getListLevelState` is gone (the level state is a stack now, read through `getListDepth`), as is the unused `ListItemsBlock`; `ListItemsResult`/`ListInlineContext` dropped `padding` and gained required `openTokens`/`allListTokens`, and `ListInlineContext.li.value` narrowed from `any` to `number` (the inline path stored the `\setcounter` value as a string, which only worked because consumers stringify it); `md-latex-lists-env/latex-list-env-block` additionally exports `pairArgumentSpans`, an internal helper exported for its invariant test rather than for use; `lib/styles/styles-code` alone no longer sizes code text (its `pre` base moved to `MathpixStyle`).
...
- **A chunk before the first `\item` is wrapped in `<li class="li_itemize" data-marker-empty="true">`.** Text, an unsupported command, a fence or a block env there used to sit straight inside the `<ul>`, which admits only `<li>`; it now has one, without a marker or a number. A sublist written in that chunk moves inside the wrapper, where it was a bare `<ul>` child of the list. A consumer diffing the HTML, or selecting `ul > *`, sees both changes.
- **A whitespace-only text node between `<ul>` and `<li>` no longer appears**, `\begin`/`\end` now being handled in source order. Visible to a consumer diffing the HTML string.
- **A stray `\end{itemize}` inside a wrapper env, in code or in math stops ending the list.** Such input rendered one item with the rest as literal LaTeX; it now keeps every item and renders the wrapper, because that text is read as content — provided the source past it still closes the list and no list starts inside the wrapper. The input is malformed (`pdflatex` rejects it), so only degraded output changes, but a consumer diffing the HTML of such documents sees it.
...
- **A bare `\item` inside a `\begin{tabular}` cell in a list body renders as literal text**, where it used to emit an orphan `<li>` in the `<td>` with its marker dropped. Both are wrong for malformed input; only the shape changed — this is the one entry here whose new output is not better than the old, just valid HTML instead of invalid. A full `\begin{itemize}…\end{itemize}` in a cell is unaffected.
- **A nested list carries line numbers under `lineNumbering`.** `data_line_start`, `data_line_end`, `data_line`, `count_line` and the `preview-line` list appear on a nested `<ul>`/`<ol>`, which used to get none of them; the outer list is unchanged, as is `data_parent_line_start` on items. A consumer mapping preview lines back to source can locate a sublist for the first time.
...
1. **Code-block styles** — `styles/styles-code.ts` **and** `codeBlockStyles` in `styles/index.ts`; regenerate the style snapshots. Indivisible: one file alone leaves `pre` and `pre code` disagreeing. Rehearsed: the build passes and 10 tests fail, all this group's own — 7 style snapshots (`MathpixStyle`, `codeStyles`) and the 3 em-relative assertions.
2. **Command arguments with `\\`** — the escape parity in `findEndMarker` (`common.ts`). Reverting it brings back the dropped caption and heading; its optional `inlineCodePositions` parameter must stay, or group 5 loses it. `findEndMarkerPos` lives in `common.ts` now (moved out of `mdPluginRaw`, which re-exports it) — leave it there: group 5's math scanner imports it, and importing from `mdPluginRaw` instead closes a cycle that empties the render. Rehearsed: the build passes and 10 tests fail, all this group's own — 5 parity cases in `_find-end-marker.js`, 2 caption fixtures, 2 section fixtures and the `\footnote{note \\}` quirk; drop the now-unused `isShielded` with it or `tsc` fails on it.
3. **Unclosed-`tabular` performance** — `md-block-rule/begin-tabular/index.ts`.
4. **Tabular-cell link export** — `common/table-markdown.ts`, `common/render-table-cell-content.ts`, the link branch of `render-tabular.ts`.
5. **Lists** — `md-latex-lists-env/`, `common/display-width.ts`, `common/env-transient.ts`, `common/caption-counters.ts`, `common/verbatim-ranges.ts`, `md-latex-footnotes/block-rule.ts`, `styles/styles-lists.ts`, the leaf-run branch of `render-tabular.ts`. Keep `common/src-pos-cache.ts` (group 3 uses it too) and `common/math-spans.ts`, which `begin-tabular/sub-math.ts` now imports its scanner from.

Groups 4 and 5 share two files, so revert 5 first. Commits mix themes — revert by these scopes, not by commit.
