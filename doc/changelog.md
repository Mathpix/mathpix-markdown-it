...
- Marker `padding-inline-start` is now emitted in `ex` (was `px`) so it scales with the container `font-size`, like the marker's math SVG. This also fixes math markers being clipped: a `7.329ex` math marker reserved `51px` but rendered ~68px — it now reserves its exact `widthEx` plus the marker→content gap (also `ex`: `.li_level` `padding-right` `10px` → `1.4ex`).
