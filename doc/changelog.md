...
- **`table-markdown` exports a link's address as written.** The escaping guards Markdown syntax, not the URL scheme, so validating `javascript:`/`data:` is the reader's job — the HTML path has `validateLink` for it. The surface is wider than before, the whole label now being exported.
...
2. **Command arguments with `\\`** — the escape parity in `findEndMarker` (`common.ts`). Reverting it brings back the dropped caption and heading; its optional `inlineCodePositions` parameter must stay, or group 5 loses it. `findEndMarkerPos` lives in `common.ts` now (moved out of `mdPluginRaw`, which re-exports it) — leave it there: group 5's math scanner imports it, and importing from `mdPluginRaw` instead closes a cycle that empties the render.
