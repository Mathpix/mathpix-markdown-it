...
- **An `\item` with no list open stays text.** Written past the closer on one line (`\begin{itemize} \item a \end{itemize} \item[X] b`) it emitted an `<li>` outside any list, which left the markup unusable; the same `<li>` appeared wherever a crossed closer had already closed the list.
