...
- Footnote rule performance:
  ...
  - Boolean `.test()` calls in both rules switched from the imported /g constants (`reOpenTagFootnoteG`, `reOpenTagFootnotetextG`) to module-local stateless aliases (`new RegExp(SOURCE.source)`). Removes a pre-existing footgun: a successful `.test()` on a /g regex advances `lastIndex` past the match, which can yield a one-shot false negative on a subsequent shorter line.
  - Cache helper `getCachedSrcPositions(state, key, patternG)` is module-private inside `md-latex-footnotes/block-rule.ts` (single internal caller; promotion to a shared util is one line away if a second rule needs the pattern). Caller must pass a /g regex owned by the helper (helper mutates `lastIndex`); both module callers pass module-level /g constants. Cache key namespace `__mmd_*` enforced at compile time via the `MmdCacheKey` template literal type for TS callers (JS callers must follow the convention). Cache hit uses `Array.isArray(cached)` rather than `!== undefined`.
...
- New tests:
  - 5 fixtures in `_data/_footnotes_latex/_data-footnote.js`: `\footnote`-prefixed commands that must not trigger the rule (`\footnotemark[1]` mid-line, `\footnotesize` in a multi-line paragraph, repeated `\footnotemark`); mixed `\footnotemark` + real `\footnote{}` in the same paragraph; `\footnote{}` at end of source (boundary check for the `<` comparison in the early-exit).
  - 4 fixtures in `_data/_theorem/_data.js`: three for unregistered environments (`tikzpicture`, `lemma`, `example`) — full HTML pinning shows that the rule no longer emits `class="theorem_block"`; one for `\newtheorem{NAME}` + `\begin{NAME}` on adjacent lines (silent-mode handshake regression).
