...
- **Text written after the outermost `\end{itemize}` on that line is no longer dropped.** `\end{itemize} tail text` lost `tail text`, and the same position lost a `}` left by an unclosed command, a code span, and the opening of a fence — the rule reported success having dropped them. That stretch of the line now goes back to the block phase and renders as it would anywhere else (`tail text` becomes the same `<div>` a standalone paragraph does). Two shapes are not covered. A leftover that opens a list of its own is still dropped, as on 3.0.1: read from that point it would become a top-level list, skipping the check that declined it as a sibling. And a wrapper env after the *outermost* closer — `\begin{itemize}\n\item a\n\end{itemize} \begin{center}c\end{center}` — is kept but not rendered: the list rule declines that line, the paragraph rule takes it, and inside a paragraph `\begin{center}` has no rule to render it, so it prints as literal LaTeX with the list beside it. On 3.0.1 the wrapper was dropped instead, so the output changed. `tabular` and `lstlisting` do have an inline rule and render there, as they did. After a *nested* closer every wrapper renders — that is the next entry.
