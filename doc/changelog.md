...
Five independent groups in one publish. Read **Breaking changes** before upgrading; the specs carry the measurements and the reasoning.
...
- **LaTeX lists.**
  ...
  - Those five envs are opaque to the list scanner, like `tabular` and `lstlisting` already were, so `\item` in a caption or a stray `\end{itemize}` inside one is text rather than structure — both used to wreck the list. A wrapper stays transparent when going opaque would swallow the list's own closer.
...
5. **Lists** — `md-latex-lists-env/`, `common/display-width.ts`, `common/env-transient.ts`, `common/caption-counters.ts`, `md-latex-footnotes/block-rule.ts`, `styles/styles-lists.ts`, the leaf-run branch of `render-tabular.ts`. Keep `common/src-pos-cache.ts`: group 3 uses it too, and reverting this group only leaves `firstPositionAtOrAfter` unused.
