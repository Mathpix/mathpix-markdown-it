...
- **`table-markdown`/`tsv`/`csv` change for cells holding a one-line LaTeX list** (the fix above), and a single-line nested list gains a `<br>` before its sublist.
...
- **Math in a `table-markdown` cell keeps the delimiters of its source, and a link label follows the same rules as the text beside it.** Display math (`$$…$$`) exported as `$…$` before, collapsing to inline on a round-trip; a label was escaped and ignored `outMath.table_markdown` entirely, so `math_inline_delimiters` and `math_as_ascii` now apply inside labels too. The label keeps its LaTeX verbatim, which re-parses only for a reader that has a math rule — a plain CommonMark parser, or `math_as_ascii` where there are no delimiters at all, truncates it at an unbalanced `]` inside the math.
