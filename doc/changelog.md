...
**A minor, deliberately, despite the Breaking changes below.** No documented entry point changed its name, arity or option shape: `markdownToHTML`, the other `MathpixMarkdownModel` methods and `mathpixMarkdownPlugin` are called exactly as before, and most of what changed replaces output that was broken or invalid. What moved is either behind a deep import into `lib/markdown/…`, which is internal and not covered by the version, or the rendered output itself. **A `^3.0.1` range therefore picks this up on a plain `npm install`** — pin the exact version if you need to review the diff first. Two entries reach a consumer with no code change on their side and are worth checking before upgrading: the `tsv`/`csv` cell exports, and the `#preview-content`/`#setText` list and code CSS.
...
- **Performance.** Repeated unclosed `\begin{itemize}` units no longer cost a scan per line: a document of 60 of them renders in 57 ms against 876 ms, and unclosed units now scale like closed ones. Unclosed `\begin{tabular}` is 14× cheaper at 400 units (678 ms against 9.7 s) but stays super-linear, so a document of them is still slow. A footnote at the end of a document costs no ruler walk per block ahead of it. Well-formed lists parse faster than 3.0.1 — 51% on 400 list units, 27% on a mixed 24 KB document, 50% on nested lists — and peak heap over 20 parses of that document is ~20% lower.
...
- **The marker tokens on `*_list_open.itemizeLevel` are shared per macro within a render** and must be treated as read-only: the array is copied per list, the tokens in it are not, so a rule that writes into one changes every later list with the same marker. Cloning them per list measured 12–29% slower on list-heavy input.
...
- **A stray `\end{itemize}` inside a wrapper env, in code or in math stops ending the list**: such input rendered one item with the rest as literal LaTeX, and now keeps every item and renders the wrapper.
...
- **Every list opening straight inside a list gets its own marker-less `<li>`, and that item closes with it.** Both rules now ask the same question — is the container this list opens in a list rather than an item — where the opening looked only at the token before it and the closing guessed from the token after. Two such lists in a row used to share one item, and a list opening after a sibling closed got none at all. A list after a real `\item` is unaffected. Measured over 3226 documents (every list fixture plus 3000 seeded malformed ones): 73 produce valid markup where they did not, none got worse.
