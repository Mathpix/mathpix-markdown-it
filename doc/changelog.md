...
- Marker width is measured more accurately, so markers no longer get too small an indent: East-Asian Wide/Fullwidth characters count as 2 (`\item[11．]`, U+FF0E, no longer undercounted vs `\item[11.42]`); math markers (`\item[$x^4+x^4$]`) use their rendered `widthEx` instead of contributing 0; and wrapped markers (`\item[\textbf{…}]`) are measured through their children. Edge whitespace in a marker (`\item[  wide  ]`) no longer inflates the padding — the marker is measured trimmed, as it renders. ASCII text markers are unchanged. Shared width helpers now live in `common/display-width.ts`.
...
- Marker `padding-inline-start` is now emitted in `em` (was `px`) so it scales with the container `font-size`. This also fixes math markers being clipped: a `7.329ex` math marker reserved `51px` but rendered ~68px — width is now taken from the rendered `widthEx` (or `1.3 ex`/char for text), converted to em via the default font metrics (`ex→em ≈ 0.5186`). The marker gap moves `10px` → `0.625em` and the default (non-custom) list indent `40px` → `2.5em` (equivalent at a 16px base, but scalable). Custom padding is emitted only when it exceeds the default, so it never resolves below `2.5em` (short markers and math without a `widthEx` keep the default). Text markers are ~5–14% tighter than 3.0.1 (reserve is per character cell, not per glyph, so unusually wide all-caps markers can overlap). The emission threshold shifted slightly: a custom marker now gets padding when its computed value exceeds `2.5em` (≈ 3+ characters), so 3-char markers like `\item[abc]` now receive an explicit indent where before they used the default. **Migration:** `data-padding-inline-start` now holds an em value with its unit (e.g. `"3.32em"`, was a bare px number) — downstream consumers reading this attribute must update.
