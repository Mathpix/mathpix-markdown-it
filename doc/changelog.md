...
- Footnote rule performance:
  - `latex_footnote_block` and `latex_footnotetext_block` previously ran an O(N×M) accumulation+regex scan at every block start, dominating parse time on documents with few or no footnotes. Added a per-state cache of token-match positions in `state.src` (single regex sweep on first invocation, pinned to state). Each subsequent rule call short-circuits in O(1) when no relevant token remains at or after the current block's source offset.
  - Inside the Phase 1 forward-scan, the expensive open-tag regex `.test(fullContent)` is gated by two cheap checks: (1) the literal token must have been seen (`/\\footnote(?![a-zA-Z])/` / `/\\(?:bl)?footnotetext(?![a-zA-Z])/` excludes `\footnotemark`, `\footnotesize`, `\footnotetextStyle`); (2) the current line must contain `{` (every regex alternative ends in `{`, so without it the pattern cannot complete). The two gates together prevent the `\footnote\n…(many lines)…\n{` worst case from degrading back to O(N×M).
  - Cache helper `getCachedSrcPositions(state, key, patternG)` is module-private inside `md-latex-footnotes/block-rule.ts`. Cache key is a per-module `Symbol` so collisions with `StateBlock` fields or other plugins' state writes are impossible at runtime. Caller must pass a /g regex owned by the helper (helper-owned constants named `*_SWEEP_G` to flag this to future contributors); helper resets `lastIndex` on entry. Cache hit uses `Array.isArray(cached)` rather than `!== undefined`.
...
- `setPositions` / `setChildrenPositions` skip alignment:
  ...
  - `setChildrenPositions` skips writing `.positions` on frozen tokens (per-child `Object.isExtensible` guard); non-frozen siblings inside `tabular_inline` subtrees still get full position + highlight processing. The same guard is added to the `link_open` special-case branch entry condition (no `link_*` token is currently frozen, but this prevents the same class of `TypeError` if one ever is). The top-level skip list is also extended from `['tabular']` to `['tabular', 'tabular_inline']` for symmetry.

- `BeginTheorem` registration check:
  ...
  - `getTheoremEnvironmentIndex(envName)` lookup is now run early but only in non-silent mode. Unregistered environments bail in O(1) without `endTag()`, forward-scan, or any `state.push`. Silent-mode invocations keep their original close-tag-based answer — required by the existing handshake where `newTheoremBlock`'s terminator pass calls `BeginTheorem(silent)` on the `\begin{NAME}` line that immediately follows `\newtheorem{NAME}{...}`, before the env is registered. Behavior for registered environments is unchanged.
  - **Note for documents using TikZ or other unregistered `\begin{NAME}…\end{NAME}` blocks:** body text inside an unregistered theorem-style environment was already dropped on master by an unrelated math-block fallback rule and is still dropped — this PR only stops emitting the unmatched leading `<div class="theorem_block">` wrapper. Net visual change: previously such blocks rendered as an unclosed wrapper above an empty math-block placeholder; now just the empty placeholder remains. To get the body rendered, register the environment via `\newtheorem{NAME}{…}` (recommended) or wrap the LaTeX block in a fenced code block. See the spec's "Known limitations" section for details.

- New tests:
  - 7 fixtures in `_data/_footnotes_latex/_data-footnote.js`: `\footnote`-prefixed commands that must not trigger the rule (`\footnotemark[1]` mid-line, `\footnotesize` in a multi-line paragraph, repeated `\footnotemark`); mixed `\footnotemark` + real `\footnote{}`; `\footnote{}` at end of source (boundary check for the `<` early-exit comparison); `\footnote{}` after leading whitespace (tShift invariant); nested `\footnote{outer with \footnote{inner}}` (cache must not leak across `state.md.block.parse` boundaries). Plus 9 token-guard soundness cases (one per alternative of `reOpenTagFootnoteG` / `reOpenTagFootnotetextG`) and 2 perf regression smoke-tests — synthetic 4,000-paragraph no-footnote documents with a 1.5 s wall-clock budget after warmup; the optimized path runs in ~110-150 ms.
  - 1 fixture in `_data/_theorem/_data.js` for `\newtheorem{NAME}` + `\begin{NAME}` on adjacent lines (silent-mode handshake regression). Plus 3 quirk fixtures isolated in `_data/_theorem/_data_known_quirks.js` for unregistered environments (`tikzpicture`, `lemma`, `example`) — full HTML pinning under a separate "TO BE FIXED" describe block makes the pin obvious when the upstream quirk is eventually fixed.
  - 3 regression cases in `tests/_html-segments.js` calling `markdownToHTMLSegments({ addPositionsToTokens: true })` on inputs that produce `tabular_inline` as a child of an `inline` block (paragraph + subtable, after inline math, two subtables in one paragraph) — directly exercises the path that previously threw.
  - 3 fixtures in new `_data/_tokenPositions/_data_tabulars.js` for the position-tokens suite; wired via a new `Testing positions for tabulars` describe in `_tokenPositions.js`.
