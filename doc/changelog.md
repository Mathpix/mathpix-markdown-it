...
- Footnote rule performance:
  ...
  - Inside the Phase 1 forward-scan, the expensive `reOpenTagFootnoteG.test(fullContent)` (resp. `reOpenTagFootnotetextG`) is gated by two cheap checks: (1) the literal token must have been seen (`/\\footnote(?![a-zA-Z])/` / `/\\(?:bl)?footnotetext(?![a-zA-Z])/` excludes `\footnotemark`, `\footnotesize`, `\footnotetextStyle`); (2) the current line must contain `{` (every regex alternative ends in `{`, so without it the pattern cannot complete). The two gates together prevent the `\footnote\n…(many lines)…\n{` worst case from degrading back to O(N×M).
  - Cache helper `getCachedSrcPositions(state, key, pattern)` is module-private inside `md-latex-footnotes/block-rule.ts` (single internal caller; promotion to a shared util is one line away if a second rule needs the pattern). Fast path reuses caller's /g regex (helper mutates `lastIndex`); a non-/g regex is silently lifted to a /g copy with `[gy]` stripped from inherited flags so `y` (sticky) does not produce a `SyntaxError`. Cache key namespace `__mmd_*` enforced at compile time via the `MmdCacheKey` template literal type. Cache hit uses `Array.isArray(cached)` rather than `!== undefined`.
