...
Read **Breaking changes** first; everything under **Fixes** needs no action. Reasoning and measurements are in `pr-specs/2026-07-list-rendering-robustness.md` and `pr-specs/2026-07-code-block-font-scaling.md`.

**A minor by decision, not because the changes are small.** Strict semver would call this a major: `tsv`/`csv` cell content and `data-padding-inline-start` are documented output that changed shape, `markdownToHTML` no longer throws out of a failing list rule, and three deep imports are gone. A `^3.0.1` range picks it up on a plain `npm install` — pin the exact version if you need to review the diff first. No documented entry point changed its name, arity or options.

### Breaking changes

- **`data-padding-inline-start` is an em value with its unit** (`"56"` → `"4.23em"`), and is emitted on nested lists too. An unparsable value on a list token is ignored and warns once.
- **`tsv`/`csv` cells.** An image exports its src alone (`![the alt](i.png)` → `i.png`, was `the alti.png`); a link exports its href followed by the rest of the cell (`[a](http://u) tail` → `http://u tail`, was `http://u`). A cell is not an address even when it starts with one.
- **`table-markdown`/`tsv`/`csv` for a cell holding a one-line list.** A nested sublist's item bodies now appear, preceded by a `<br>`; a flat list exports whatever sits before its first `\item` as its own line. A flat list with nothing before that item is unchanged. Four forms pinned in `_data/_tsv` and `_data/_csv`.
- **A `ul`/`ol` with no `.itemize`/`.enumerate` class indents `2.5em` inside `#preview-content`/`#setText`**, where the UA default `40px` applied — this reaches core-markdown lists and lists passed as HTML under `htmlTags: true`. A TOC keeps `padding: 0` under `.table-of-contents` or `#toc_container`; copied elsewhere it shifts `2.5px` per level, so copy it into `#toc_container` or give your container `padding: 0`.
- **The wrapped-list indent rises to specificity `(1,0,1)`**, so a consumer rule at class specificity no longer wins.
- **A failing list rule no longer fails the render.** It warns once per cause and returns degraded HTML, so a `catch` around `markdownToHTML` for that case will not fire. Only a failure past the point where tokens entered the stream still propagates.
- **New `console.warn` diagnostics**, deduplicated per render, capped at 40 per family and 200 overall, informational, with no option to silence them.
- **A consumer's own `env` comes back as the list rule found it.** `isBlock`, `inheritedListType`, `parentType` and `prentLevel` are restored after every list. A key the parse set comes back as an own key holding `undefined` rather than deleted (`delete` drops `env` into dictionary mode). Source caches live on `env` under symbols: invisible to `Object.keys` and JSON, carried by `Object.assign`/`structuredClone`, and emptied rather than left stale across renders.
- **Deep imports.** `ClearTableNumbers`/`ClearFigureNumbers` → `clearTableNumbers`/`clearFigureNumbers` in `lib/markdown/common/caption-counters`, no re-export; `getListLevelState` and `ListItemsBlock` are gone (level state is a stack, read through `getListDepth`); `ListItemsResult`/`ListInlineContext` dropped `padding` and require `openTokens`/`allListTokens`; `skipBackticks` answers past an unmatched run of backticks instead of the end of the string; `LATEX_ITEM_COMMAND_INLINE_RE` needs a real `\item`, so `\itemsep` and prose no longer match; `OpaqueEnvType` gains `table`, `figure`, `center`, `left`, `right` — additive to read, breaking to produce; `lib/styles/styles-code` no longer sizes code text.

### Fixes that change existing output

Nothing to do for these — they replace output that was broken or invalid.

- **Markers no longer clip their item text.** The reserve is measured per glyph class in `em`, per nesting level, capped at `20em`. A wide marker reserves ~21% more than 3.0.1, a narrow or digit one 36–70% less; the default indent and gap are pixel-identical at a 16px base.
- **A block env in a list body renders the same wherever it sits** — `table`, `figure`, `center`, `left`, `right` gain their wrapper and caption after a closed sublist or before the first `\item`, and a `tabular` there gains its `table_tabular` wrapper. All five are opaque to the list scanner, as `tabular` and `lstlisting` already were, so `\item` or a stray closer written inside one is text. A list written in such a wrapper keeps its nesting depth.
- **Content sitting directly in a list is wrapped in an `<li>`**, wherever it comes from: a chunk before the first `\item`, a run in a list opened flush against its parent, or a list opening straight inside a list. A chunk holding only `\setcounter` or `\renewcommand` gets no wrapper — those print nothing, and a token-stream consumer used to read an `\item` the source never had.
- **A command's argument is text, for every reader.** A list closer written in `\caption`, `\item[…]`, a `\renewcommand` body, a code span, math, or in the argument of a rule that re-parses it (`\footnote`, `\footnotetext`, `\text`, `\textbf`) no longer ends the list. On 3.0.1 `\text{f \end{itemize} g}` emitted a `</ul>` with no opener. The malformed converse costs: a list whose only closer sits in a macro body stays literal LaTeX.
- **An `\item` or a closer with no list open stays text**, where a closer emitted a bare `</ul>` and an `\item` an `<li>` with nothing around it.
- **Text is no longer eaten** beside `\renewcommand` on its line, after the outermost `\end{itemize}`, before a `\diagbox`, or by a `\\` at the end of a command argument. The `\\` fix shifts `Table N`, `Figure N` and section numbers below it, since the caption or heading now renders. If you read a `renewcommand` token's `latex`, a line with a tail now yields the inline rule's payload — everything past the command name — where a line without one is unchanged.
- **A leftover past the outermost `\end{itemize}` goes back to the block phase**, so a marker that means something at a line start is taken there: `\end{itemize} # H` gives a heading, `> q` a blockquote, `- a` a Markdown list, ` ``` ` a fence. A heading there shifts the section numbers below it. A line that names no block, `| a | b |` among them, stays a paragraph.
- **An empty `\item` no longer swallows a one-line list.** `\begin{itemize}\item\item x\end{itemize}` rendered as literal LaTeX; the item is now an empty `<li>`, with or without a `[…]` marker.
- **Crossed and collapsed closers parse.** `\end{itemize}` over an open `enumerate` closes what is open rather than what it names; collapsed closers, a mid-line closer opening a sibling, and `\begin`/`\end` in source order each used to leave the outer list unclosed. A list opened mid-paragraph keeps its levels, and a list in a `tabular` cell closes its last `<li>`.
- **Every list reads its own copy of the marker tokens.** A macro is parsed once per render and each list gets a copy, so a write into a marker stays with that list — sharing them leaked into every later list with the same marker. Within one list the accumulation remains: each `<li>` re-renders the same marker tokens, so a marker built from a link carries `target`/`style` once, twice, three times down the list, as on 3.0.1.
- **`table-markdown` exports what the cell holds** — a link's address as written and its whole label, `sub`/`sup`/`ins` markup in a plain cell, `math_inline_delimiters` and `math_as_ascii` inside labels, and a pipe escaped in ascii math. Escaping guards Markdown syntax, not the URL scheme or HTML — see **Security notes on the Markdown exports** in the README.
- **`prentLevel` on an inline-opened `*_list_open` is the real nesting depth**, was always `0`.
- **Nesting depth is reset per parse, not healed mid-render.** A consumer handing `md.renderer.render` its own unbalanced token slice no longer has the drift corrected for it; re-rendering one array is unsupported either way, `attrJoin` piling classes onto the same tokens.
- **Also fixed:** empty `<>` item bodies, `\itemsep`/`\itemindent` read as `\item`, a continuation line containing the word `item` losing its break, a list swallowed by a preceding `\footnote` scan, an orphan `<br>` after a `\renewcommand` line, whitespace-only text nodes between `<ul>` and `<li>`, edge whitespace in a custom marker, a bare `\item` in a `tabular` cell emitting an orphan `<li>`, nested lists missing line numbers under `lineNumbering`, and a marker-less `<li>` exporting a marker looked up by `NaN`.
- **Malformed input costs less.** Repeated unclosed `\begin{itemize}` units scale like closed ones; unclosed `\begin{tabular}` is cheaper but stays super-linear; a footnote at the end of a document costs no ruler walk per block ahead of it; well-formed lists parse faster than 3.0.1 with ~20% lower peak heap over 20 parses. Figures move with input and host — the spec carries the method.
- **A line of `\diagbox` scales.** Both readers of its arguments rebuilt the inline-code index over the whole string, twice per command, so a line of them followed by a code span was `n^1.85`: 342 ms at 3200 commands, 31 ms now. Inherited — 3.0.1 measures the same 334 ms.
- **Code blocks scale with their container.** `pre`/`pre code` sizes are relative, so a code block follows the container `font-size`. Pixel-identical at a 16px base except code padding (16px → 15px) and a raw-HTML `<pre>` with no `<code>` child (13.6px → 15px).
- **Not fixed:** a leftover after the outermost closer that opens a list of its own is still dropped; `\verb` is not modelled as verbatim, so `\item \verb|\end{itemize}|` still ends the list there.
