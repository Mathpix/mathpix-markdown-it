...
- **LaTeX lists.**
  ...
  - A single-line `\begin{tabular}{l}q\end{tabular}` in a list body no longer takes the list down with it.
...
- **A caption, heading or footnote whose argument ends in `\\` renders, and shifts the numbering below it.** The argument used to read as unclosed, so the whole command was dropped: `\caption{First line \\}` produced no caption and no `number`, and the next table took `Table 1`. It now renders and that table becomes `Table 2`; the same shift applies to `Figure N` and to section numbers, and a `\footnote{note \\}` that printed as literal text now produces a footnote and its `<section class="footnotes">`. A consumer keying off those numbers, or diffing the HTML, sees the difference.
