...
- **Performance.** Repeated unclosed `\begin{itemize}` units no longer cost a scan per line — 400 of them go from 865 ms to 12 ms, linear from there; unclosed `\begin{tabular}` gains as much in absolute terms. A footnote at the end of a document costs no ruler walk per block ahead of it. Well-formed lists parse faster than 3.0.1 (~46% on 400 list units, ~24% on a mixed 24 KB document), and peak heap over 20 parses of that document is ~20% lower.
...
- **`data-padding-inline-start` is an em value with its unit** (`"56"` → `"4.23em"`), and is emitted on nested lists too. A value that is not a bare `em` is ignored, and now says so once with `console.warn`.
