...
| File | Purpose |
|------|---------|
| `index.ts` | `SerializedTypstVisitor` class — extends MathJax's `MmlVisitor`, handles root traversal, inferred mrow spacing (decomposed into pattern functions: `handleBigDelimiterPattern`, `handleIdotsintPattern`, `handleBareDelimiterPairPattern`, `handleThousandSepPattern`, `handleEqnArrayMtablePattern`, `handleNotNegationPattern`), big delimiter detection (`\big`, `\Big`, etc.) with `findTypstSymbol` guard, bare delimiter-pair grouping (`\|...\|`, `⌊...⌋`, `⌈...⌉`, `‖...‖`) with function-call-aware scanner to avoid false `‖` pairing inside function args, `\idotsint` grouping (integral-dots-integral pattern via `SUBSCRIPT_KINDS` constant), `\not` negation overlay detection, thousand-separator comma detection, sibling content merging for tagged eqnArray mtables (passes prefix via `data-pre-content` and suffix via `data-post-content` properties), custom command detection in `visitTeXAtomNode` via `getCustomCmdTypstSymbol`, delimiter-pairing guard in `getBigDelimInfo`/`resolveDelimiterMo` (with `console.warn` on errors) to skip `data-custom-cmd` nodes; `serializeRange` uses `needsSpaceBetweenNodes` for correct spacing inside bare-delimiter pairs |
...
| `structural-handlers.ts` | Structural wrappers: `mrow` (delimiter pairing, `\left..\right`, shorthand functions with separator fallback, `\not` negation wrapping via `cancel()`), `mpadded` (mhchem phantom stripping with tripledash sibling detection, `\colorbox`, `mstyle` background handling), `mphantom` (`#hide()`), `menclose` (cancel/xcancel/bcancel, boxed, longdiv with `lr(\) ...)`, circle, selective border strokes via `parseNotation()`/`hasNotation()` Set-based matching, overline/underline), `mstyle` (color wrapping, tripledash dash→hyph replacement) |
...
| `common.ts` | Shared helpers: `initTypstData`, `addToTypstData` (always propagates `typst_inline` with `typst` fallback), `addSpaceToTypstData`, `needsParens` (`s.length > 1`), `isThousandSepComma` (with Indian numbering support), `serializeThousandSepChain`, `needsTokenSeparator`, `needsSpaceBetweenNodes` (extends `needsTokenSeparator` with script+bracket spacing check), `formatScript`, `isFirstChild`/`isLastChild`/`getSiblingIndex`, `getChildText`/`getNodeText`, `getAttrs`/`getProp`/`getContentChildren`, `isNegationOverlay` (`\not` detection), `getCustomCmdTypstSymbol` (custom command → Typst lookup via central map), `escapeTypstString` (backslash/quote escaping for Typst string literals), `serializeCombiningMiChain` (non-Latin script grouping for Devanagari, Arabic, CJK, etc.), `handleAll` (with `needsTokenSeparator` between children) |
| `escape-utils.ts` | Unified expression scanner (`scanExpression`, uses `parts.push()`+`.join('')`) with per-bracket-type depth counters (`parenDepth`, `bracketDepth`, `braceDepth`); shared `escapeAtPositions` helper for position-based backslash escaping; thin wrappers: `escapeContentSeparators`, `escapeCasesSeparators`, `hasTopLevelSeparators`, `escapeLrSemicolons`, `escapeUnbalancedParens`, `escapeLrBrackets` (bracket escaping inside `lr()` to prevent auto-scaling) |
| `bracket-utils.ts` | Delimiter mapping and escaping: `mapDelimiter`, `delimiterToTypst`, `escapeLrDelimiter`, `replaceUnpairedBrackets` (cell-level bracket escaping for mat/cases with scope boundaries for `msqrt`, `mroot`, `mfrac`, `menclose`), `findUnpairedIndices` (single-pass stack pairing), `treeContainsMo`, `serializePrefixBeforeMo`, function-call-aware paren heuristic via `KNOWN_TYPST_FUNCTIONS` |
...
**mhchem `\tripledash` fix:** `\tripledash` now produces `hyph hyph hyph` (was `#hide($-$) "-" "-" "-"`). MathJax emits a complex structure: `mpadded(width=0)` > `mphantom` > `mo(−)` followed by `mpadded` > `mstyle(mathsize=0.5em)` > `mtext("-")` × 3. Two fixes: (1) `mstyle` handler detects tripledash pattern via `isTripledashStyle` and replaces `"-"` with `hyph`; (2) `mpadded` handler suppresses `#hide()` output when `width=0` + `mphantom` and a sibling subtree contains a tripledash mstyle (`hasAdjacentTripledashStyle`).
...
### Custom command dispatch (`data-custom-cmd`)

Some LaTeX commands (e.g. `\Varangle`) expand into visual-hack subtrees in MathJax (overlapping operators, negative kerns) that have no single Unicode equivalent. Rather than pattern-matching these complex trees in every serializer, the system uses a **tag-and-dispatch** approach:
...
- The visitor is read-only over the MathML tree — the only mutations are `markUnpairedBrackets()` setting `node.properties['data-unpaired-bracket']` before traversal, `visitInferredMrowNode` setting `node.properties['data-pre-content']`/`['data-post-content']` on tagged eqnArray mtable nodes with sibling math content, and the `\Varangle` custom handler in `my-BaseMappings.ts` setting `data-custom-cmd` at parse time (before any serializer runs); the `data-custom-cmd` property is also consumed by the patched MathML visitors and the ASCII serializer for clean output
...
- `patchVisitorTeXAtom` in `mathjax.ts` only intercepts nodes with `data-custom-cmd` property (set by custom handlers in `my-BaseMappings.ts`) — all other TeXAtom nodes fall through to original visitor method unchanged
- `\Varangle` custom handler in `my-BaseMappings.ts` produces identical MathML subtree as the previous `['Macro', ...]` definition, with the addition of a `data-custom-cmd` property — HTML rendering is unaffected
