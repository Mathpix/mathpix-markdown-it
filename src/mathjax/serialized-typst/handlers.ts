...
import {
  ...
  isFirstChild, isLastChild, getSiblingIndex, typstPlaceholder, handleAll,
} from "./common";
...
const defaultHandler: HandlerFn = (node, serialize) => {
  return handleAll(node, serialize);
};
...
const mi: HandlerFn = (node, _serialize) => {
...
const mo: HandlerFn = (node, _serialize) => {
  let res: ITypstData = initTypstData();
...
const mtext: HandlerFn = (node, _serialize) => {
  let res: ITypstData = initTypstData();
  if (!node.childNodes || node.childNodes.length === 0) {
    return res;
  }
...
const mfrac: HandlerFn = (node, serialize) => {
  let res: ITypstData = initTypstData();
...
const msub: HandlerFn = (node, serialize) => {
  let res: ITypstData = initTypstData();
  const firstChild = node.childNodes[0] || null;
  const secondChild = node.childNodes[1] || null;
  const dataFirst: ITypstData = firstChild ? serialize.visitNode(firstChild, '') : initTypstData();
...
const msubsup: HandlerFn = (node, serialize) => {
  let res: ITypstData = initTypstData();
  const firstChild = node.childNodes[0] || null;
  const secondChild = node.childNodes[1] || null;
...
const msqrt: HandlerFn = (node, serialize) => {
  let res: ITypstData = initTypstData();
  const firstChild = node.childNodes[0] || null;
  const dataFirst: ITypstData = firstChild ? serialize.visitNode(firstChild, '') : initTypstData();
...
const mroot: HandlerFn = (node, serialize) => {
  let res: ITypstData = initTypstData();
...
const mover: HandlerFn = (node, serialize) => {
  let res: ITypstData = initTypstData();
  const firstChild = node.childNodes[0] || null;
  const secondChild = node.childNodes[1] || null;
  const dataFirst: ITypstData = firstChild ? serialize.visitNode(firstChild, '') : initTypstData();
...
const munder: HandlerFn = (node, serialize) => {
  let res: ITypstData = initTypstData();
  const firstChild = node.childNodes[0] || null;
  const secondChild = node.childNodes[1] || null;
  const dataFirst: ITypstData = firstChild ? serialize.visitNode(firstChild, '') : initTypstData();
...
const munderover: HandlerFn = (node, serialize) => {
  let res: ITypstData = initTypstData();
  const firstChild = node.childNodes[0] || null;
  const secondChild = node.childNodes[1] || null;
  const thirdChild = node.childNodes[2] || null;
...
const mmultiscripts: HandlerFn = (node, serialize) => {
  let res: ITypstData = initTypstData();
...
const mrow: HandlerFn = (node, serialize) => {
  let res: ITypstData = initTypstData();
...
const mphantom: HandlerFn = (node, serialize) => {
  let res: ITypstData = initTypstData();
  const data: ITypstData = handleAll(node, serialize);
  const content = data.typst.trim();
  if (content) {
