...
export const findAmSymbolsToLiner = (input: string, tag = '') => {
  let linerData = null;
  if (tag) {
    linerData = amSymbolsToLiner.find(item => item.tag === tag && item.input === input);
    if (linerData) {
      return linerData;
    }
  }
  return amSymbolsToLiner.find(item => item.input === input);
}
...
export const formatLinerFromAscii = (ascii: string, childLiner?: string, tag = ''): string => {
  const linerData = findAmSymbolsToLiner(ascii, tag);
  if (!linerData) return '';
  const child = (childLiner ?? '').trim();
