...
var MathpixMarkdown_Model = /** @class */ (function () {
    function MathpixMarkdown_Model() {
        ...
        this.checkEquationNumberNode = function (html) {
            try {
                var document_1 = domino.createDocument(html);
                var body = document_1.body;
                var links = body.getElementsByClassName('clickable-link');
                for (var i = 0; i < links.length; i++) {
                    var eq = links[i].getAttribute('value');
                    var equationNumber = document_1.getElementById(eq);
                    if (!equationNumber) {
        ...
        this.checkEquationNumber = function (html) {
            try {
                ...
                return _this.checkEquationNumberNode(html);
            }
        };
