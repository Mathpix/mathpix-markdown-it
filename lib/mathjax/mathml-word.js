...
var MathMLVisitorWord = /** @class */ (function (_super) {
    ...
    MathMLVisitorWord.prototype.restructureMtrForAligned = function (node, space) {
        var nl = (node.isToken || node.childNodes.length === 0 ? ['\n', ''] : ['\n', space])[0];
        var mml = '';
        var attributes = node.Parent && node.Parent.attributes
            ? node.Parent.attributes.getAllAttributes()
            : {};
        var _a = attributes.columnalign, columnalign = _a === void 0 ? '' : _a;
        if (!columnalign) {
            return mml;
        }
        ...
        for (var i = 0; i < node.childNodes.length; i++) {
            ...
            mml += nl + space4 + '<mrow>';
            var child = this.childNodeMml(node.childNodes[i], space4, nl);
            mml += child.trim()
                ? nl + child
                : nl;
