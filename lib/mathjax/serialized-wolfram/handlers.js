...
var mtable = function () {
    return function (node, serialize) {
        ...
        try {
            ...
            var openBranch = ((_a = node.parent) === null || _a === void 0 ? void 0 : _a.kind) === 'mrow'
                ? (_b = node.parent.properties) === null || _b === void 0 ? void 0 : _b.open : '';
            var closeBranch = ((_c = node.parent) === null || _c === void 0 ? void 0 : _c.kind) === 'mrow'
                ? (_d = node.parent.properties) === null || _d === void 0 ? void 0 : _d.close : '';
             ...
             * */
            var isDeterminant = openBranch === '|' && closeBranch === '|';
            if (!openBranch && !closeBranch && ((_e = node.parent) === null || _e === void 0 ? void 0 : _e.kind) === 'inferredMrow' && ((_g = (_f = node.parent) === null || _f === void 0 ? void 0 : _f.childNodes) === null || _g === void 0 ? void 0 : _g.length) > 1) {
                var sFirst = node.parent.childNodes[0].kind === 'mo' ? serialize.visitNode(node.parent.childNodes[0], '') : '';
                openBranch = (sFirst === null || sFirst === void 0 ? void 0 : sFirst.trim()) === '{' ? '{' : '';
            }
            console.log("[MMD]=>mtable=>openBranch=>", openBranch);
            console.log("[MMD]=>mtable=>closeBranch=>", closeBranch);
            console.log("[MMD]=>mtable=>isDeterminant=>", isDeterminant);
            var display = (_j = (_h = node.attributes) === null || _h === void 0 ? void 0 : _h.attributes) === null || _j === void 0 ? void 0 : _j.hasOwnProperty('displaystyle'); // && node.prevClass !== 4;
            console.log("[MMD]=>mtable=>display=>attributes=>", (_l = (_k = node.attributes) === null || _k === void 0 ? void 0 : _k.attributes) === null || _l === void 0 ? void 0 : _l.displaystyle);
            console.log("[MMD]=>mtable=>display=>attributes=>hasOwnProperty=>", (_o = (_m = node.attributes) === null || _m === void 0 ? void 0 : _m.attributes) === null || _o === void 0 ? void 0 : _o.hasOwnProperty('displaystyle'));
            console.log("[MMD]=>mtable=>display=>inherited=>", (_q = (_p = node.attributes) === null || _p === void 0 ? void 0 : _p.inherited) === null || _q === void 0 ? void 0 : _q.displaystyle);
            console.log("[MMD]=>mtable=>display=>", display);
            console.log("[MMD]=>mtable=>node=>", node);
            ...
            // || (node.parent.properties.close === "" || node.parent.properties?.open === "{")) ;
            var isNotMatrix = (openBranch === '{' && !closeBranch)
                || display
                || (!isHasBranchClose && isHasBranchOpen);
            console.log("[MMD]=>mtable=>isNotMatrix=>", isNotMatrix);
            ...
            for (var i = 0; i < node.childNodes.length; i++) {
                ...
                var srow = serialize.visitNode(node.childNodes[i], '');
                console.log("[MMD]=>srow=>", srow);
                console.log("[MMD]=>node.childNodes[i]=>", node.childNodes[i]);
                if (i > 0) {
                    mml += ', ';
                }
...
var mrow = function () {
    return function (node, serialize) {
        ...
        try {
            ...
            console.log("[MMD]=>[mrow]=>node=>", node);
            var openBranch = (_a = node.properties) === null || _a === void 0 ? void 0 : _a.open;
            var closeBranch = (_b = node.properties) === null || _b === void 0 ? void 0 : _b.close;
            /** In terms of notations, a matrix is an array of numbers enclosed by square brackets
             * while a determinant is an array of numbers enclosed by two vertical bars.
             * */
            if (openBranch === '|' && closeBranch === '|'
                && node.childNodes.length === 3 && ((_c = node.childNodes[1]) === null || _c === void 0 ? void 0 : _c.kind) === 'mtable') {
                mml += ' det(';
...
var mtr = function () {
    return function (node, serialize) {
        ...
        try {
            ...
            var mtrContent = '';
            var isNotMatrix = node.isNotMatrix;
            console.log("[MMD]=>mtr=>isNotMatrix=>", isNotMatrix);
            // node.childNodes?.length < 2;
            for (var i = 0; i < node.childNodes.length; i++) {
                var smd = serialize.visitNode(node.childNodes[i], '');
                var smdBefore = i > 0 ? serialize.visitNode(node.childNodes[i - 1], '') : '';
                smdBefore = smdBefore === null || smdBefore === void 0 ? void 0 : smdBefore.trim();
                console.log("[MMD]=>smd=>", smd);
                console.log("[MMD]=>smdBefore=>", smdBefore);
                if (i > 0) {
                    /** Do not put a comma before and after the =, >=, <=, <, > sign */
                    // const operations = ['=', '>=', '<='];
                    var reOperations = /^(?:=|>=|<=|<|>)/;
                    console.log("[MMD]=>match=>smd=>", (_a = smd === null || smd === void 0 ? void 0 : smd.trim()) === null || _a === void 0 ? void 0 : _a.match(reOperations));
                    console.log("[MMD]=>match=>smdBefore=>", smdBefore === null || smdBefore === void 0 ? void 0 : smdBefore.match(reOperations));
                    var hasCommaBefore = (smdBefore === null || smdBefore === void 0 ? void 0 : smdBefore.length) && smdBefore[smdBefore.length - 1] === ',';
                    console.log("[MMD]=>match=>hasCommaBefore=>", hasCommaBefore, smdBefore);
                    if (!isNotMatrix) {
                        isNotMatrix = Boolean(hasCommaBefore || (((_b = smd === null || smd === void 0 ? void 0 : smd.trim()) === null || _b === void 0 ? void 0 : _b.match(reOperations)) || (smdBefore === null || smdBefore === void 0 ? void 0 : smdBefore.match(reOperations))));
                    }
                    mtrContent += display || hasCommaBefore || (((_c = smd === null || smd === void 0 ? void 0 : smd.trim()) === null || _c === void 0 ? void 0 : _c.match(reOperations)) || (smdBefore === null || smdBefore === void 0 ? void 0 : smdBefore.match(reOperations)))
                        ? ''
                        : ', ';
                }
                mtrContent += serialize.visitNode(node.childNodes[i], '');
            }
            console.log("[MMD]=>isNotMatrix=>", isNotMatrix);
...
var msup = function () {
    return function (node, serialize) {
        ...
        try {
            ...
            var secondChild = node.childNodes[1] ? node.childNodes[1] : null;
            var sSecondChild = secondChild ? serialize.visitNode(secondChild, '') : '';
            if (((firstChild === null || firstChild === void 0 ? void 0 : firstChild.kind) === "mi" && !regW.test(sFirstChild)) || (firstChild === null || firstChild === void 0 ? void 0 : firstChild.kind) === "mfrac") {
                mml += "(";
                mml += serialize.visitNode(firstChild, '');
                mml += ")";
            }
