...
var SerializedAsciiVisitor = /** @class */ (function (_super) {
    ...
    SerializedAsciiVisitor.prototype.visitTree = function (node) {
        ...
        this.resAscii.push({ kind: node.kind, res: tslib_1.__assign({}, res) });
        return res;
    };
    ...
    SerializedAsciiVisitor.prototype.visitTextNode = function (node, space) {
        ...
        try {
            res = common_1.AddToAsciiData(res, [node.getText()]);
            return res;
        }
        catch (e) {
    ...
    SerializedAsciiVisitor.prototype.visitXMLNode = function (node, space) {
        ...
        try {
            res = common_1.AddToAsciiData(res, [space + node.getSerializedXML()]);
            return res;
        }
        catch (e) {
    ...
    SerializedAsciiVisitor.prototype.visitInferredMrowNode = function (node, space) {
        ...
        try {
            ...
            if (iclose > -1) {
                ...
                if ((!atr.notation && atrDef.notation === "longdiv") ||
                    ...
                    if (iclose === 0) {
                        ...
                        res = common_1.AddToAsciiData(res, ["(("]);
                        res = common_1.AddToAsciiData(res, [dataDivisor.ascii, dataDivisor.ascii_tsv, dataDivisor.ascii_csv, dataDivisor.ascii_md]);
                        res = common_1.AddToAsciiData(res, [")/("]);
                        mnList.forEach(function (item) {
                            var dataDividend = _this.visitNode(item, '');
                            res = common_1.AddToAsciiData(res, [dataDividend.ascii, dataDividend.ascii_tsv, dataDividend.ascii_csv, dataDividend.ascii_md]);
                        });
                        res = common_1.AddToAsciiData(res, ["))"]);
                    ...
                    if (iclose === 0) {
                        ...
                        if (iclose - mnList.length > 0) {
                            for (var i_3 = 0; i_3 < iclose - mnList.length; i_3++) {
                                ...
                                res = common_1.AddToAsciiData(res, [data_1.ascii, data_1.ascii_tsv, data_1.ascii_csv, data_1.ascii_md]);
                            }
                        }
                        res = common_1.AddToAsciiData(res, ['((']);
                        mnList.forEach(function (item) {
                        ...
                        if (iclose < node.childNodes.length - 1) {
                            for (var i_4 = iclose + 1; i_4 < node.childNodes.length; i_4++) {
                                ...
                                res = common_1.AddToAsciiData(res, [data_2.ascii, data_2.ascii_tsv, data_2.ascii_csv, data_2.ascii_md]);
                            }
                        }
                    }
                    return res;
            ...
            res = common_1.AddToAsciiData(res, [needBranch && hasFunc ? '{:' : '']);
            if (addParens && !group) {
                res = common_1.AddToAsciiData(res, ['(']);
            }
            try {
    ...
    SerializedAsciiVisitor.prototype.visitTeXAtomNode = function (node, space) {
        ...
        try {
            var needFirstSpace = handlers_1.needFirstSpaceBeforeCurrentNode(node, this);
            var children = this.childNodeMml(node, space + '  ', '\n');
            res = common_1.AddToAsciiData(res, [needFirstSpace ? ' ' : '']);
            res = common_1.AddToAsciiData(res, [
    ...
    SerializedAsciiVisitor.prototype.childNodeMml = function (node, space, nl) {
        ...
        try {
            if (node.kind === 'mover' && node.childNodes.length > 1 && node.childNodes[0].kind === 'TeXAtom' && node.childNodes[1].kind === 'TeXAtom') {
                ...
                res = common_1.AddToAsciiData(res, [dataFirstChild.ascii, dataFirstChild.ascii_tsv, dataFirstChild.ascii_csv, dataFirstChild.ascii_md]);
                res = common_1.AddToAsciiData(res, ['^']);
                res = common_1.AddToAsciiData(res, ['(']);
                res = common_1.AddToAsciiData(res, [dataChildNodes.ascii, dataChildNodes.ascii_tsv, dataChildNodes.ascii_csv, dataChildNodes.ascii_md]);
                res = common_1.AddToAsciiData(res, [')']);
            }
            else {
