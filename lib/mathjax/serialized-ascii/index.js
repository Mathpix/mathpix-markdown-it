                    ...
                    if (iclose === 0) {
                        ...
                        if (iclose - mnList.length > 0) {
                            for (var i_1 = 0; i_1 < iclose - mnList.length; i_1++) {
                                ...
                                res = common_1.AddToAsciiData(res, [
                                    data.ascii, data.ascii_tsv, data.ascii_csv
                                ]);
                            }
                        }
                        var dataDivisor = this.visitNode(mclose, '');
                        res = common_1.AddToAsciiData(res, ["(("]);
                        res = common_1.AddToAsciiData(res, [dataDivisor.ascii, dataDivisor.ascii_tsv, dataDivisor.ascii_csv]);
                        res = common_1.AddToAsciiData(res, [")/("]);
                        mnList.forEach(function (item) {
                            var dataDividend = _this.visitNode(item, '');
                            res = common_1.AddToAsciiData(res, [dataDividend.ascii, dataDividend.ascii_tsv, dataDividend.ascii_csv]);
                        });
                        res = common_1.AddToAsciiData(res, ["))"]);
                    ...
                    if (iclose === 0) {
                        ...
                        if (iclose - mnList.length > 0) {
                            for (var i_3 = 0; i_3 < iclose - mnList.length; i_3++) {
                                ...
                                res = common_1.AddToAsciiData(res, [data_1.ascii, data_1.ascii_tsv, data_1.ascii_csv]);
                            }
                        }
                        res = common_1.AddToAsciiData(res, ['((']);
                        mnList.forEach(function (item) {
                        ...
                        if (iclose < node.childNodes.length - 1) {
                            for (var i_4 = iclose + 1; i_4 < node.childNodes.length; i_4++) {
                                ...
                                res = common_1.AddToAsciiData(res, [data_2.ascii, data_2.ascii_tsv, data_2.ascii_csv]);
                            }
                        }
                    }
                    return res;
    ...
    SerializedAsciiVisitor.prototype.childNodeMml = function (node, space, nl) {
        ...
        try {
            if (node.kind === 'mover' && node.childNodes.length > 1 && node.childNodes[0].kind === 'TeXAtom' && node.childNodes[1].kind === 'TeXAtom') {
                ...
                res = common_1.AddToAsciiData(res, [dataFirstChild.ascii, dataFirstChild.ascii_tsv, dataFirstChild.ascii_csv]);
                res = common_1.AddToAsciiData(res, ['^']);
                res = common_1.AddToAsciiData(res, ['(']);
                res = common_1.AddToAsciiData(res, [dataChildNodes.ascii, dataChildNodes.ascii_tsv, dataChildNodes.ascii_csv]);
                res = common_1.AddToAsciiData(res, [')']);
            }
            else {
