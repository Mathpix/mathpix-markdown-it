...
var getDataForVerticalMath = function (serialize, node) {
    ...
    for (var k = 0; k < mtdNode.childNodes.length; k++) {
        ...
        if (child.isKind('inferredMrow') || child.isKind('TeXAtom')) {
            var data = getDataForVerticalMath(serialize, child);
...
var mtable = function () {
    return function (node, serialize) {
        ...
        try {
            ...
            var itShouldBeFlatten = envName === 'array' && !isHasBranchOpen && !isHasBranchClose && !parentIsMenclose;
            /** Vertical math:
            ...
            for (var i = 0; i < countRow; i++) {
                ...
                for (var j = 0; j < countColl; j++) {
                    ...
                    var mmlCollFlat = '';
                    if (isVerticalMath) {
                        var dataColl = getDataForVerticalMath(serialize, mtdNode);
