    ...
    return function (node, serialize) {
        ...
        try {
            ...
            if (abs && abs.length > 1 && regW.test(abs[0])) {
                res = common_1.AddToAsciiData(res, [needFirstSpase(node) ? ' ' : '']);
                res = common_1.AddToAsciiData(res, [abs]);
                res = common_1.AddToAsciiData(res, [needLastSpase(node) ? ' ' : '']);
            }
            else {
                res = common_1.AddToAsciiData(res, [abs]);
            }
...
var mo = function () {
    return function (node, serialize) {
        ...
        try {
            ...
            if (abs && abs.length > 1) {
                res = common_1.AddToAsciiData(res, [regW.test(abs[0]) && needFirstSpase(node) ? ' ' : '']);
                res = common_1.AddToAsciiData(res, [abs]);
                res = common_1.AddToAsciiData(res, [regW.test(abs[abs.length - 1]) && needLastSpase(node) ? ' ' : '']);
            }
            else {
    ...
    return function (node, serialize) {
        ...
        try {
            ...
            if (atr && atr.width === "2em") {
                res = common_1.AddToAsciiData(res, [node.parent.parent && needFirstSpase(node.parent.parent) ? ' ' : '']);
                res = common_1.AddToAsciiData(res, ['qquad']);
                res = common_1.AddToAsciiData(res, [node.parent.parent && needLastSpase(node.parent.parent) ? ' ' : '']);
                return res;
            }
