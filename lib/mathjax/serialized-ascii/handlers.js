...
var menclose = function (handlerApi) {
    return function (node, serialize) {
        ...
        try {
            ...
            res = common_1.AddToAsciiData(res, [isLeft ? '[' : ''], serialize);
            var data = handlerApi.handleAll(node, serialize);
            res = common_1.AddToAsciiData(res, [data.ascii, data.ascii_tsv, data.ascii_csv, data.ascii_md], serialize, node);
            if (atr && atr.lcm) {
                res = common_1.AddToAsciiData(res, [''], serialize);
            }
            res = common_1.AddToAsciiData(res, [isRight ? ']' : ''], serialize);
            return res;
        }
        catch (e) {
        ...
        try {
            ...
            for (var i = 0; i < countRow; i++) {
                ...
                for (var j = 0; j < countColl; j++) {
                    if (j > 0 && !isEqnArrayRow) {
                        ...
                        common_1.pushToSerializedStack(serialize, {
                            ascii: mmlRow,
                    ...
                    if (isVerticalMath) {
                        common_1.clearSerializedChildStack(serialize);
            ...
            for (var i = 0; i < arrRows.length; i++) {
                ...
                var mmlRow = isVerticalMath
                    ? common_1.needSpaceBetweenLetters(mmlTableContent, arrRows[i].mmlRowVerticalMath)
                        ? ' ' + arrRows[i].mmlRowVerticalMath
                        : arrRows[i].mmlRowVerticalMath
                    : arrRows[i].mmlRow;
                var mmlRow_tsv = isVerticalMath
                    ? common_1.needSpaceBetweenLetters(mmlTableContent_tsv, arrRows[i].mmlRowVerticalMath)
                        ? ' ' + arrRows[i].mmlRowVerticalMath
                        : arrRows[i].mmlRowVerticalMath
                    : arrRows[i].mmlRow_tsv;
                var mmlRow_csv = isVerticalMath
                    ? common_1.needSpaceBetweenLetters(mmlTableContent_csv, arrRows[i].mmlRowVerticalMath)
                        ? ' ' + arrRows[i].mmlRowVerticalMath
                        : arrRows[i].mmlRowVerticalMath
                    : arrRows[i].mmlRow_csv;
                var mmlRow_md = isVerticalMath
                    ? common_1.needSpaceBetweenLetters(mmlTableContent_md, arrRows[i].mmlRowVerticalMath)
                        ? ' ' + arrRows[i].mmlRowVerticalMath
                        : arrRows[i].mmlRowVerticalMath
                    : arrRows[i].mmlRow_md;
...
var mrow = function () {
    return function (node, serialize) {
        try {
            ...
            var mmlContent_csv = '';
            var mmlContent_md = '';
            var needFirstSpace = common_1.needFirstSpaceBeforeCurrentNode(node, serialize);
...
var mpadded = function (handlerApi) {
    return function (node, serialize) {
        ...
        try {
            ...
            res = common_1.AddToAsciiData(res, [mmlAdd.ascii, mmlAdd.ascii_tsv, mmlAdd.ascii_csv, mmlAdd.ascii_md], serialize, node);
            res = common_1.AddToAsciiData(res, [
                '"',
                serialize.options.tableToTsv ? '' : '"',
                '',
    ...
    return function (node, serialize) {
        ...
        try {
            ...
            if (secondChild && secondChild.kind === 'mo') {
                ...
                if (asc) {
                    res = common_1.AddToAsciiData(res, [' ' + asc + '('], serialize);
                    ...
                    res = common_1.AddToAsciiData(res, ['^'], serialize);
                    res = common_1.AddToAsciiData(res, [serialize.options.extraBrackets ? '(' : ''], serialize);
    ...
    return function (node, serialize) {
        ...
        try {
            ...
            if (secondChild && secondChild.kind === 'mo') {
                ...
                res = common_1.AddToAsciiData(res, ['_'], serialize);
                res = common_1.AddToAsciiData(res, [serialize.options.extraBrackets ? '(' : ''], serialize);
                var dataSecondChild = secondChild ? serialize.visitNode(secondChild, '') : null;
                res = common_1.AddToAsciiData(res, [
...
var msqrt = function () {
    return function (node, serialize) {
        ...
        try {
            var firstChild = node.childNodes[0] ? node.childNodes[0] : null;
            res = common_1.AddToAsciiData(res, ['sqrt'], serialize);
            var dataFirstChild = firstChild ? serialize.visitNode(firstChild, '') : null;
            res = common_1.AddToAsciiData(res, [
                dataFirstChild ? dataFirstChild.ascii : '',
                dataFirstChild ? dataFirstChild.ascii_tsv : '',
...
var mroot = function () {
    return function (node, serialize) {
        ...
        try {
            var firstChild = node.childNodes[0] ? node.childNodes[0] : null;
            var secondChild = node.childNodes[1] ? node.childNodes[1] : null;
            res = common_1.AddToAsciiData(res, ['root'], serialize);
            res = common_1.AddToAsciiData(res, [secondChild ? '(' : ''], serialize);
            var dataSecondChild = secondChild ? serialize.visitNode(secondChild, '') : null;
            res = common_1.AddToAsciiData(res, [
                dataSecondChild ? dataSecondChild.ascii : '',
                dataSecondChild ? dataSecondChild.ascii_tsv : '',
            ...
            res = common_1.AddToAsciiData(res, [secondChild ? ')' : ''], serialize);
            res = common_1.AddToAsciiData(res, [firstChild ? '(' : ''], serialize);
            var dataFirstChild = firstChild ? serialize.visitNode(firstChild, '') : null;
            res = common_1.AddToAsciiData(res, [
                dataFirstChild ? dataFirstChild.ascii : '',
                dataFirstChild ? dataFirstChild.ascii_tsv : '',
...
var mtext = function () {
    return function (node, serialize) {
        ...
        try {
            ...
            if (asc) {
                res = common_1.AddToAsciiData(res, [asc], serialize, node);
                return res;
            }
            var toTsv = node.attributes.get('toTsv');
            var toCsv = node.attributes.get('toCsv');
    ...
    return function (node, serialize) {
        ...
        try {
            ...
            if (abs && abs.length && common_1.regW.test(abs[0])) {
                var isNeedFirstSpace = common_1.needFirstSpaceBeforeLetter(node, abs, serialize)
                    || common_1.needFirstSpaceBeforeCurrentNode(node, serialize);
                res = common_1.AddToAsciiData(res, [isNeedFirstSpace ? ' ' : ''], serialize);
                res = common_1.AddToAsciiData(res, [abs], serialize, node);
            }
            else {
                res = common_1.AddToAsciiData(res, [abs], serialize, node);
            }
    ...
    return function (node, serialize) {
        ...
        try {
            ...
            if (abs && abs.length > 1) {
                var isNeedFirstSpace = common_1.needFirstSpaceBeforeLetter(node, abs, serialize)
                    || common_1.needFirstSpaceBeforeCurrentNode(node, serialize);
                res = common_1.AddToAsciiData(res, [common_1.regW.test(abs[0]) && isNeedFirstSpace ? ' ' : ''], serialize);
                res = common_1.AddToAsciiData(res, [abs], serialize, node);
            }
            else {
    ...
    return function (node, serialize) {
        ...
        try {
            ...
            if (atr && atr.width === "2em") {
                res = common_1.AddToAsciiData(res, [node.parent.parent && common_1.needFirstSpaceBeforeLetter(node.parent.parent, 'qquad', serialize) ? ' ' : ''], serialize);
                res = common_1.AddToAsciiData(res, ['qquad'], serialize, node);
                return res;
            }
            if (atr && atr.width === "1em") {
                res = common_1.AddToAsciiData(res, [node.parent.parent && common_1.needFirstSpaceBeforeLetter(node.parent.parent, 'quad', serialize) ? ' ' : ''], serialize);
                res = common_1.AddToAsciiData(res, ['quad'], serialize, node);
                return res;
            }
            var data = handlerApi.handleAll(node, serialize);
            res = common_1.AddToAsciiData(res, [data.ascii, data.ascii_tsv, data.ascii_csv, data.ascii_md], serialize, node);
    ...
    try {
        try {
            for (var _b = tslib_1.__values(node.childNodes), _c = _b.next(); !_c.done; _c = _b.next()) {
                var child = _c.value;
                var data = serialize.visitNode(child, '');
                res = common_1.AddToAsciiData(res, [data.ascii, data.ascii_tsv, data.ascii_csv, data.ascii_md], serialize, child);
            }
        }
