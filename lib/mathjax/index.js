...
var AsciiMath = require('mathjax-full/js/input/asciimath.js').AsciiMath;
...
var SerializedAscii_1 = require("./SerializedAscii");
var visitor = new SerializedAscii_1.SerializedMmlVisitor();
...
var OuterHTML = function (node, math, outMath) {
    ...
    if (include_asciimath) {
        ...
        var asciimml = convert(toMathML(math.root)); // => '1 + 2'
        console.log('!!!!!! asciimml =>', asciimml);
        outHTML += '<asciimath style="display: none">' + asciimml + '</asciimath>';
        var asciiSvg = docAsciiMath.convert(asciimml);
        var htmlSvgA = adaptor.outerHTML(asciiSvg);
        if (!outHTML) {
            outHTML += '\n';
        }
        outHTML += htmlSvgA;
