...
var serialized_typst_1 = require("./serialized-typst");
var bracket_utils_1 = require("./serialized-typst/bracket-utils");
var common_1 = require("./serialized-typst/common");
var consts_1 = require("./serialized-typst/consts");
...
var normalizeTypstSpaces = function (s) {
    return s ? s.trim().replace(/(\S) {2,}/g, '$1 ') : '';
};
...
var RE_TRAILING_LINEBREAKS = /(?:\s*\\)+\s*$/;
var stripTrailingLinebreaks = function (s) {
    return s ? s.replace(RE_TRAILING_LINEBREAKS, '') : '';
};
/** Find the first merror node in the tree and return its error message, or null. */
var findMerror = function (node) {
    var e_1, _a;
    var _b, _c, _d;
    if (!node)
        return null;
    if (node.kind === 'merror') {
        var attrMsg = (_b = node.attributes) === null || _b === void 0 ? void 0 : _b.get('data-mjx-error');
        if (attrMsg)
            return attrMsg;
        var textMsg = ((_c = node.childNodes) !== null && _c !== void 0 ? _c : [])
            .map(function (c) { return (0, common_1.getNodeText)(c); }).join('');
        return textMsg || 'Unknown error';
    }
    try {
        for (var _e = tslib_1.__values(((_d = node.childNodes) !== null && _d !== void 0 ? _d : [])), _f = _e.next(); !_f.done; _f = _e.next()) {
            var child = _f.value;
            var err = findMerror(child);
            if (err)
                return err;
        }
    }
...
var toTypstData = function (node, labels) {
    var _a, _b;
    var parseError = findMerror(node);
    if (parseError) {
        return {
            typstmath: '',
            typstmath_inline: '',
    ...
    (0, bracket_utils_1.markUnpairedBrackets)(node);
    var visitorT = new serialized_typst_1.SerializedTypstVisitor(labels !== null && labels !== void 0 ? labels : null);
    var data = visitorT.visitTree(node);
    // Clean up tree mutations so properties don't leak to other visitors
    (0, bracket_utils_1.clearUnpairedBracketMarks)(node);
    clearTypstProperties(node);
    var typstmath = normalizeTypstSpaces(data === null || data === void 0 ? void 0 : data.typst);
    var typstmath_inline = stripTrailingLinebreaks(normalizeTypstSpaces((_a = data === null || data === void 0 ? void 0 : data.typst_inline) !== null && _a !== void 0 ? _a : data === null || data === void 0 ? void 0 : data.typst));
    // Propagate non-fatal conversion errors (handler failures).
    // Output still contains best-effort fallback text.
    var error = ((_b = data === null || data === void 0 ? void 0 : data.errors) === null || _b === void 0 ? void 0 : _b.length)
        ? data.errors.join('; ')
        : undefined;
    return {
        typstmath: typstmath,
        typstmath_inline: typstmath_inline,
        error: error
    };
};
/** Remove data-pre-content and data-post-content properties set during Typst serialization. */
var clearTypstProperties = function (root) {
    var walk = function (node) {
        var e_2, _a;
        if (!node)
            return;
        node.removeProperty(consts_1.DATA_PRE_CONTENT, consts_1.DATA_POST_CONTENT);
        if (node.childNodes) {
            try {
...
var OuterData = function (adaptor, node, math, outMath, forDocx, accessibility) {
    ...
    var _d = outMath.include_mathml, include_mathml = _d === void 0 ? false : _d, _e = outMath.include_mathml_word, include_mathml_word = _e === void 0 ? false : _e, _f = outMath.include_asciimath, include_asciimath = _f === void 0 ? false : _f, _g = outMath.include_latex, include_latex = _g === void 0 ? false : _g, _h = outMath.include_linearmath, include_linearmath = _h === void 0 ? false : _h, _j = outMath.include_typst, include_typst = _j === void 0 ? false : _j, _k = outMath.include_svg, include_svg = _k === void 0 ? true : _k, _l = outMath.include_speech, include_speech = _l === void 0 ? false : _l, _m = outMath.optionAscii, optionAscii = _m === void 0 ? {
        showStyle: false,
        extraBrackets: true,
    } : _m, _o = outMath.output_format, output_format = _o === void 0 ? 'svg' : _o;
    var res = {};
    ...
    var tagLabels = ((_c = (_b = (_a = math.inputJax) === null || _a === void 0 ? void 0 : _a.parseOptions) === null || _b === void 0 ? void 0 : _b.tags) === null || _c === void 0 ? void 0 : _c.labels)
        ? tslib_1.__assign({}, math.inputJax.parseOptions.tags.labels) : null;
    if (include_typst) {
        var typstData = toTypstData(math.root, tagLabels);
...
var OuterDataAscii = function (adaptor, node, math, outMath, forDocx, accessibility) {
    var _a, _b, _c;
    if (forDocx === void 0) { forDocx = false; }
    var _d = outMath.include_mathml, include_mathml = _d === void 0 ? false : _d, _e = outMath.include_mathml_word, include_mathml_word = _e === void 0 ? false : _e, _f = outMath.include_asciimath, include_asciimath = _f === void 0 ? false : _f, _g = outMath.include_typst, include_typst = _g === void 0 ? false : _g, _h = outMath.include_svg, include_svg = _h === void 0 ? true : _h, _j = outMath.include_speech, include_speech = _j === void 0 ? false : _j, _k = outMath.output_format, output_format = _k === void 0 ? 'svg' : _k;
    var res = {};
    var a11y = applyMathJaxA11y(adaptor, node, accessibility, include_speech);
    if (a11y.speech) {
        res.speech = a11y.speech;
...
var OuterDataMathMl = function (adaptor, node, math, outMath, forDocx, accessibility) {
    var _a, _b, _c;
    if (forDocx === void 0) { forDocx = false; }
    var _d = outMath.include_mathml, include_mathml = _d === void 0 ? false : _d, _e = outMath.include_mathml_word, include_mathml_word = _e === void 0 ? false : _e, _f = outMath.include_asciimath, include_asciimath = _f === void 0 ? false : _f, _g = outMath.include_svg, include_svg = _g === void 0 ? true : _g, _h = outMath.include_speech, include_speech = _h === void 0 ? false : _h, _j = outMath.include_linearmath, include_linearmath = _j === void 0 ? false : _j, _k = outMath.include_typst, include_typst = _k === void 0 ? false : _k, _l = outMath.optionAscii, optionAscii = _l === void 0 ? {
        showStyle: false,
        extraBrackets: true
    } : _l, _m = outMath.output_format, output_format = _m === void 0 ? 'svg' : _m;
    var res = {};
    var a11y = applyMathJaxA11y(adaptor, node, accessibility, include_speech);
    if (a11y.speech) {
        res.speech = a11y.speech;
...
var OuterHTML = function (data, outMath, forPptx) {
    ...
    var _a = outMath.include_mathml, include_mathml = _a === void 0 ? false : _a, _b = outMath.include_mathml_word, include_mathml_word = _b === void 0 ? false : _b, _c = outMath.include_asciimath, include_asciimath = _c === void 0 ? false : _c, _d = outMath.include_linearmath, include_linearmath = _d === void 0 ? false : _d, _e = outMath.include_latex, include_latex = _e === void 0 ? false : _e, _f = outMath.include_typst, include_typst = _f === void 0 ? false : _f, _g = outMath.include_svg, include_svg = _g === void 0 ? true : _g, _h = outMath.include_error, include_error = _h === void 0 ? false : _h, _j = outMath.include_speech, include_speech = _j === void 0 ? false : _j;
    var outHTML = '';
    ...
    if (include_typst && data.typstmath) {
        ...
        outHTML += '<typstmath style="display: none;">' + (0, parse_mmd_element_1.formatSource)(data.typstmath) + '</typstmath>';
        outHTML += '<typstmath-inline style="display: none;">' + (0, parse_mmd_element_1.formatSource)(data.typstmath_inline || data.typstmath) + '</typstmath-inline>';
    }
...
exports.MathJax = {
    ...
    TexConvertToTypstData: function (string, options) {
        var _a, _b, _c, _d, _e, _f, _g, _h;
        if (options === void 0) { options = {}; }
        var _j = options.display, display = _j === void 0 ? true : _j, _k = options.metric, metric = _k === void 0 ? {} : _k, _l = options.accessibility, accessibility = _l === void 0 ? null : _l;
        var _m = metric.em, em = _m === void 0 ? 16 : _m, _o = metric.ex, ex = _o === void 0 ? 8 : _o, _p = metric.cwidth, cwidth = _p === void 0 ? 1200 : _p, _q = metric.lwidth, lwidth = _q === void 0 ? 100000 : _q, _r = metric.scale, scale = _r === void 0 ? 1 : _r;
        this.checkAccessibility(accessibility);
        (_c = (_b = (_a = MJ.docTeX.inputJax) === null || _a === void 0 ? void 0 : _a.parseOptions) === null || _b === void 0 ? void 0 : _b.tags) === null || _c === void 0 ? void 0 : _c.reset();
        MJ.docTeX.convert(string, {
            display: display,
            em: em,
            ex: ex,
        ...
        var outputJax = MJ.docTeX.outputJax;
        var tagLabels = (_h = (_g = (_f = (_e = (_d = outputJax === null || outputJax === void 0 ? void 0 : outputJax.math) === null || _d === void 0 ? void 0 : _d.inputJax) === null || _e === void 0 ? void 0 : _e.parseOptions) === null || _f === void 0 ? void 0 : _f.tags) === null || _g === void 0 ? void 0 : _g.labels) !== null && _h !== void 0 ? _h : null;
        return toTypstData(outputJax.math.root, tagLabels);
    },
    MathMLConvertToTypstData: function (string, options) {
        if (options === void 0) { options = {}; }
        var _a = options.display, display = _a === void 0 ? true : _a, _b = options.metric, metric = _b === void 0 ? {} : _b, _c = options.accessibility, accessibility = _c === void 0 ? null : _c;
    ...
    TypesetAsciiMath: function (string, options) {
        if (options === void 0) { options = {}; }
        var _a = options.display, display = _a === void 0 ? true : _a, _b = options.metric, metric = _b === void 0 ? {} : _b, _c = options.outMath, outMath = _c === void 0 ? {} : _c, _d = options.forDocx, forDocx = _d === void 0 ? {} : _d, _e = options.accessibility, accessibility = _e === void 0 ? null : _e;
        var _f = metric.em, em = _f === void 0 ? 16 : _f, _g = metric.ex, ex = _g === void 0 ? 8 : _g, _h = metric.cwidth, cwidth = _h === void 0 ? 1200 : _h, _j = metric.lwidth, lwidth = _j === void 0 ? 100000 : _j, _k = metric.scale, scale = _k === void 0 ? 1 : _k;
        this.checkAccessibility(accessibility);
