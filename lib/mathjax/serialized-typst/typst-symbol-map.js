"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.findTypstSymbol = exports.typstFontMap = exports.typstAccentMap = exports.typstSymbolMap = void 0;
var tslib_1 = require("tslib");
// Greek letters: Unicode → Typst symbol name
var greekSymbols = [
    ["\u03B1", "alpha"],
    ["\u03B2", "beta"],
    ["\u03B3", "gamma"],
    ["\u0393", "Gamma"],
    ["\u03B4", "delta"],
    ["\u0394", "Delta"],
    ["\u03F5", "epsilon.alt"],
    ["\u03B5", "epsilon"],
    ["\u025B", "epsilon"],
    ["\u03B6", "zeta"],
    ["\u03B7", "eta"],
    ["\u03B8", "theta"],
    ["\u03D1", "theta.alt"],
    ["\u0398", "Theta"],
    ["\u03B9", "iota"],
    ["\u03BA", "kappa"],
    ["\u03BB", "lambda"],
    ["\u039B", "Lambda"],
    ["\u03BC", "mu"],
    ["\u03BD", "nu"],
    ["\u03BE", "xi"],
    ["\u039E", "Xi"],
    ["\u03C0", "pi"],
    ["\u03D6", "pi.alt"],
    ["\u03A0", "Pi"],
    ["\u03C1", "rho"],
    ["\u03F1", "rho.alt"],
    ["\u03C3", "sigma"],
    ["\u03C2", "sigma.alt"],
    ["\u03A3", "Sigma"],
    ["\u03C4", "tau"],
    ["\u03C5", "upsilon"],
    ["\u03A5", "Upsilon"],
    ["\u03D5", "phi.alt"],
    ["\u03C6", "phi"],
    ["\u03A6", "Phi"],
    ["\u03C7", "chi"],
    ["\u03C8", "psi"],
    ["\u03A8", "Psi"],
    ["\u03C9", "omega"],
    ["\u03A9", "Omega"], // Ω
];
// Binary operators: Unicode → Typst symbol name
var binaryOperators = [
    ["\u00D7", "times"],
    ["\u00F7", "div"],
    ["\u22C5", "dot.op"],
    ["\u00B1", "plus.minus"],
    ["\u2213", "minus.plus"],
    ["\u2217", "ast.op"],
    ["\u22C6", "star"],
    ["\u2218", "compose"],
    ["\u2219", "bullet"],
    ["\u2022", "bullet"],
    ["\u2295", "plus.o"],
    ["\u2296", "minus.o"],
    ["\u2297", "times.o"],
    ["\u2298", "slash.o"],
    ["\u2299", "dot.o"],
    ["\u222A", "union"],
    ["\u2229", "inter"],
    ["\u2294", "union.sq"],
    ["\u2293", "inter.sq"],
    ["\u2228", "or"],
    ["\u2227", "and"],
    ["\u2216", "without"],
    ["\u25C7", "diamond.stroked"],
    ["\u22C4", "diamond.stroked"],
    ["\u2020", "dagger"],
    ["\u2021", "dagger.double"],
    // Note: U+2210 (∐) is mapped to "product.co" in largeOperators (not duplicated here)
    ["\u2212", "-"],
    ["\u25C3", "triangle.stroked.small.l"],
    ["\u25B9", "triangle.stroked.small.r"],
    ["\u22B2", "lt.tri"],
    ["\u22B3", "gt.tri"],
    ["\u22B4", "lt.tri.eq"],
    ["\u22B5", "gt.tri.eq"],
    ["\u2240", "wreath"],
    ["\u2A3F", "product.co"],
    ["\u22C8", "join"],
    ["\u22C9", "times.l"],
    ["\u22CA", "times.r"],
    ["\u22A0", "times.square"],
    ["\u22CE", "or.curly"],
    ["\u22CF", "and.curly"],
    ["\u228E", "union.plus"],
    ["\u2214", "plus.dot"],
    ["\u22D2", "inter.double"],
    ["\u22D3", "union.double"],
    ["\u22CB", "times.three.l"],
    ["\u22CC", "times.three.r"],
    ["\u229F", "minus.square"],
    ["\u229D", "dash.o"],
    ["\u229B", "ast.op.o"],
    ["\u22A1", "dot.square"],
    ["\u229A", "compose.o"],
    ["\u229E", "plus.square"],
    ["\u22C7", "times.div"], // ⋇ (divideontimes)
];
// Relation operators: Unicode → Typst symbol name
var relationOperators = [
    ["\u2264", "lt.eq"],
    ["\u2265", "gt.eq"],
    ["\u2266", "lt.eq"],
    ["\u2267", "gt.eq"],
    ["\u2A7D", "lt.eq.slant"],
    ["\u2A7E", "gt.eq.slant"],
    ["\u2260", "eq.not"],
    ["\u2261", "equiv"],
    ["\u227A", "prec"],
    ["\u227B", "succ"],
    ["\u2AAF", "prec.eq"],
    ["\u2AB0", "succ.eq"],
    ["\u223C", "tilde.op"],
    ["\u2243", "tilde.eq"],
    ["\u2248", "approx"],
    ["\u2245", "tilde.equiv"],
    ["\u2282", "subset"],
    ["\u2283", "supset"],
    ["\u2286", "subset.eq"],
    ["\u2287", "supset.eq"],
    ["\u2208", "in"],
    ["\u220B", "in.rev"],
    ["\u2209", "in.not"],
    ["\u221D", "prop"],
    ["\u2223", "divides"],
    ["\u2225", "parallel"],
    ["\u22A5", "perp"],
    ["\u226A", "lt.double"],
    ["\u226B", "gt.double"],
    ["\u22A2", "tack.r"],
    ["\u22A3", "tack.l"],
    ["\u22A8", "tack.r.double"],
    ["\u2262", "equiv.not"],
    ["\u226E", "lt.not"],
    ["\u226F", "gt.not"],
    ["\u2270", "lt.eq.not"],
    ["\u2271", "gt.eq.not"],
    ["\u2284", "subset.not"],
    ["\u2285", "supset.not"],
    ["\u2288", "subset.eq.not"],
    ["\u2289", "supset.eq.not"],
    ["\u2241", "tilde.not"],
    ["\u2247", "tilde.equiv.not"],
    ["\u2249", "approx.not"],
    ["\u2272", "lt.tilde"],
    ["\u2273", "gt.tilde"],
    ["\u2A85", "lt.approx"],
    ["\u2A86", "gt.approx"],
    ["\u2250", "≐"],
    ["\u225C", "eq.delta"],
    ["\u227C", "prec.eq"],
    ["\u227D", "succ.eq"],
    ["\u224D", "asymp"],
    ["\u22D8", "lt.triple"],
    ["\u22D6", "lt.dot"],
    ["\u22D9", "gt.triple"],
    ["\u22D7", "gt.dot"],
    ["\u227E", "prec.tilde"],
    ["\u227F", "succ.tilde"],
    ["\u228A", "subset.neq"],
    ["\u228B", "supset.neq"],
    ["\u228F", "subset.sq"],
    ["\u2291", "subset.eq.sq"],
    ["\u2290", "supset.sq"],
    ["\u2292", "supset.eq.sq"],
    ["\u2280", "prec.not"],
    ["\u2281", "succ.not"],
    ["\u224A", "approx.eq"],
    ["\u2276", "lt.gt"],
    ["\u2277", "gt.lt"],
    ["\u22DA", "lt.eq.gt"],
    ["\u22DB", "gt.eq.lt"],
    ["\u2251", "eq.dots"],
    ["\u2253", "eq.dots.up"],
    ["\u2252", "eq.dots.down"],
    ["\u223D", "tilde.eq.rev"],
    ["\u22CD", "tilde.eq.rev"],
    ["\u22D0", "subset.double"],
    ["\u22D1", "supset.double"],
    ["\u22DE", "eq.prec"],
    ["\u22DF", "eq.succ"],
    ["\u2AB7", "prec.approx"],
    ["\u2AB8", "succ.approx"],
    ["\u22A9", "forces"],
    ["\u2242", "minus.tilde"],
    ["\u2A87", "lt.neq"],
    ["\u2A88", "gt.neq"],
    ["\u2268", "lt.nequiv"],
    ["\u2269", "gt.nequiv"],
    ["\u22E6", "lt.ntilde"],
    ["\u22E7", "gt.ntilde"],
    ["\u2A89", "lt.napprox"],
    ["\u2A8A", "gt.napprox"],
    ["\u2AB5", "prec.nequiv"],
    ["\u2AB6", "succ.nequiv"],
    ["\u22E8", "prec.ntilde"],
    ["\u22E9", "succ.ntilde"],
    ["\u2AB9", "prec.napprox"],
    ["\u2ABA", "succ.napprox"],
    ["\u2226", "parallel.not"],
    ["\u22AC", "tack.r.not"],
    ["\u22AD", "tack.r.double.not"],
    ["\u22AE", "forces.not"],
    ["\u22EC", "lt.tri.eq.not"],
    ["\u22ED", "gt.tri.eq.not"], // ⋭ (\ntrianglerighteq)
];
// Arrows: Unicode → Typst symbol name
var arrows = [
    ["\u2190", "arrow.l"],
    ["\u2192", "arrow.r"],
    ["\u2194", "arrow.l.r"],
    ["\u21D0", "arrow.l.double"],
    ["\u21D2", "arrow.r.double"],
    ["\u21D4", "arrow.l.r.double"],
    ["\u2191", "arrow.t"],
    ["\u2193", "arrow.b"],
    ["\u21D1", "arrow.t.double"],
    ["\u21D3", "arrow.b.double"],
    ["\u21A6", "arrow.r.bar"],
    ["\u27FC", "arrow.r.long.bar"],
    ["\u27F6", "arrow.r.long"],
    ["\u27F5", "arrow.l.long"],
    ["\u27F9", "arrow.r.long.double"],
    ["\u27F8", "arrow.l.long.double"],
    ["\u27F7", "arrow.l.r.long"],
    ["\u27FA", "arrow.l.r.long.double"],
    ["\u21AA", "arrow.r.hook"],
    ["\u21A9", "arrow.l.hook"],
    ["\u2197", "arrow.tr"],
    ["\u2198", "arrow.br"],
    ["\u2199", "arrow.bl"],
    ["\u2196", "arrow.tl"],
    ["\u21C0", "harpoon.rt"],
    ["\u21BC", "harpoon.lt"],
    ["\u219E", "arrow.l.twohead"],
    ["\u21A0", "arrow.r.twohead"],
    ["\u21A3", "arrow.r.tail"],
    ["\u2916", "arrow.r.twohead.tail"],
    ["\u21CC", "harpoons.rtlb"],
    ["\u21CB", "harpoons.ltrb"],
    ["\u21C1", "harpoon.rb"],
    ["\u21BD", "harpoon.lb"],
    ["\u21BE", "harpoon.tr"],
    ["\u21BF", "harpoon.tl"],
    ["\u21C2", "harpoon.br"],
    ["\u21C3", "harpoon.bl"],
    ["\u21C6", "arrows.lr"],
    ["\u21C4", "arrows.rl"],
    ["\u21C7", "arrows.ll"],
    ["\u21C9", "arrows.rr"],
    ["\u21DD", "arrow.r.squiggly"],
    ["\u2195", "arrow.t.b"],
    ["\u21D5", "arrow.t.b.double"],
    ["\u219A", "arrow.l.not"],
    ["\u219B", "arrow.r.not"],
    ["\u21CD", "arrow.l.double.not"],
    ["\u21CF", "arrow.r.double.not"],
    ["\u21DA", "arrow.l.triple"],
    ["\u21DB", "arrow.r.triple"],
    ["\u21A2", "arrow.l.tail"],
    ["\u21AB", "arrow.l.loop"],
    ["\u21AC", "arrow.r.loop"],
    ["\u21B6", "arrow.ccw.half"],
    ["\u21B7", "arrow.cw.half"],
    ["\u21C8", "arrows.tt"],
    ["\u21CA", "arrows.bb"],
    ["\u21AD", "arrow.l.r.wave"],
    ["\u21AE", "arrow.l.r.not"],
    ["\u21CE", "arrow.l.r.double.not"], // ⇎ (nLeftrightarrow)
];
// Large operators: Unicode → Typst symbol name
var largeOperators = [
    ["\u2211", "sum"],
    ["\u220F", "product"],
    ["\u2210", "product.co"],
    ["\u222B", "integral"],
    ["\u222C", "integral.double"],
    ["\u222D", "integral.triple"],
    ["\u222E", "integral.cont"],
    ["\u222F", "integral.surf"],
    ["\u2230", "integral.vol"],
    ["\u2232", "integral.cont.cw"],
    ["\u2233", "integral.cont.ccw"],
    ["\u22C3", "union.big"],
    ["\u22C2", "inter.big"],
    ["\u2A01", "plus.o.big"],
    ["\u2A00", "dot.o.big"],
    ["\u2A02", "times.o.big"],
    ["\u2A04", "union.plus.big"],
    ["\u2A06", "union.sq.big"],
    ["\u2A0C", "integral.quad"],
    ["\u22C1", "or.big"],
    ["\u22C0", "and.big"], // ⋀
];
// Delimiters: Unicode → Typst symbol name
var delimiters = [
    ["\u27E8", "chevron.l"],
    ["\u27E9", "chevron.r"],
    ["\u2329", "chevron.l"],
    ["\u232A", "chevron.r"],
    ["\u2308", "ceil.l"],
    ["\u2309", "ceil.r"],
    ["\u230A", "floor.l"],
    ["\u230B", "floor.r"],
    ["\u231C", "corner.l.t"],
    ["\u231D", "corner.r.t"],
    ["\u231E", "corner.l.b"],
    ["\u231F", "corner.r.b"],
    ["\u2016", "bar.v.double"], // ‖
    // Note: \u2225 (∥) is NOT duplicated here — it maps to "parallel" in relationOperators
];
// Dots: Unicode → Typst symbol name
var dots = [
    ["\u2026", "dots"],
    ["\u22EF", "dots.c"],
    ["\u22EE", "dots.v"],
    ["\u22F1", "dots.down"], // ⋱
];
// Miscellaneous symbols: Unicode → Typst symbol name
var miscSymbols = [
    ["\u221A", "\\√"],
    ["\u221E", "infinity"],
    ["\u2207", "nabla"],
    ["\u2202", "partial"],
    ["\u2200", "forall"],
    ["\u2203", "exists"],
    ["\u2204", "exists.not"],
    ["\u2205", "emptyset"],
    ["\u2300", "diameter"],
    ["\u00AC", "not"],
    ["\u22A4", "top"],
    // Note: U+22A5 (⊥) is mapped to "perp" in relationOperators (not duplicated here)
    ["\u2220", "angle"],
    ["\u25B3", "triangle.stroked.t"],
    ["\u2032", "prime"],
    ["\u2033", "prime.double"],
    ["\u2034", "prime.triple"],
    ["\u2035", "prime.rev"],
    ["\u2224", "divides.not"],
    ["\u2135", "aleph"],
    ["\u210F", "planck"],
    ["\u2113", "ell"],
    ["\u211C", "Re"],
    ["\u2111", "Im"],
    ["\u2118", "\u2118"],
    ["\u2201", "complement"],
    ["\u2234", "therefore"],
    ["\u2235", "because"],
    ["\u25A1", "square.stroked"],
    ["\u25FB", "square.stroked"],
    ["\u25FC", "square.filled"],
    ["\u2221", "angle.arc"],
    ["\u2222", "angle.spheric"],
    ["\u25BD", "triangle.stroked.b"],
    ["\u25CA", "diamond.stroked"],
    ["\u2663", "suit.club.filled"],
    ["\u2662", "suit.diamond.stroked"],
    ["\u2661", "suit.heart.stroked"],
    ["\u2660", "suit.spade.filled"],
    ["\u266F", "sharp"],
    ["\u266D", "flat"],
    ["\u266E", "natural"],
    ["\u00F0", "ð"],
    ["\u2127", "Omega.inv"],
    ["\u212B", "angstrom"],
    ["\u2AEB", "tack.t.double"],
    ["\u22B8", "multimap"],
    ["\u2323", "smile"],
    ["\u2322", "frown"],
    ["\u00A0", "space.nobreak"],
    ["\u2061", ""],
    ["\u2062", ""],
    ["\u2063", ""],
    ["\u2064", ""],
    ["$", "\\$"],
    ['"', '\\"'],
    ['#', '\\#'],
    ['&', '\\&'],
    ['_', '\\_'],
    ["\u0131", "dotless.i"],
    ["\u0237", "dotless.j"],
    ["\u25B4", "triangle.filled.small.t"],
    ["\u25BE", "triangle.filled.small.b"],
    ["\u2605", "star.filled"],
    ["\u2713", "checkmark"],
    ["\u2720", "maltese"],
    ["\u25C2", "triangle.filled.small.l"],
    ["\u25B8", "triangle.filled.small.r"], // ▸ (blacktriangleright)
];
// Build the combined symbol map
exports.typstSymbolMap = new Map(tslib_1.__spreadArray(tslib_1.__spreadArray(tslib_1.__spreadArray(tslib_1.__spreadArray(tslib_1.__spreadArray(tslib_1.__spreadArray(tslib_1.__spreadArray(tslib_1.__spreadArray([], tslib_1.__read(greekSymbols), false), tslib_1.__read(binaryOperators), false), tslib_1.__read(relationOperators), false), tslib_1.__read(arrows), false), tslib_1.__read(largeOperators), false), tslib_1.__read(delimiters), false), tslib_1.__read(dots), false), tslib_1.__read(miscSymbols), false));
// Accent Unicode → Typst accent function name
// Maps the accent character (from MathML mover/munder accent attribute) to Typst function
exports.typstAccentMap = new Map([
    ["\u005E", "hat"],
    ["\u00AF", "macron"],
    ["\u203E", "overline"],
    ["\u0332", "underline"],
    ["\u007E", "tilde"],
    ["\u2192", "arrow"],
    ["\u2190", "arrow.l"],
    ["\u2194", "arrow.l.r"],
    ["\u002E", "dot"],
    ["\u02D9", "dot"],
    ["\u00A8", "diaer"],
    ["\u20DB", "dot.triple"],
    ["\u20DC", "dot.quad"],
    ["\u02D8", "breve"],
    ["\u02C7", "caron"],
    ["\u00B4", "acute"],
    ["\u02DD", "acute.double"],
    ["\u0060", "grave"],
    ["\u02DA", "circle"],
    ["\u21C0", "harpoon"],
    ["\u21BC", "harpoon.lt"],
    ["\u23DE", "overbrace"],
    ["\u23DF", "underbrace"],
    ["\u23B4", "overbracket"],
    ["\u23B5", "underbracket"],
    ["―", "overline"], // ― (horizontal bar → overline)
]);
// MathML mathvariant attribute → Typst font function name
exports.typstFontMap = new Map([
    ["bold", "bold"],
    ["bold-italic", "bold"],
    ["double-struck", "bb"],
    ["script", "cal"],
    ["-tex-calligraphic", "cal"],
    ["fraktur", "frak"],
    ["bold-fraktur", "frak"],
    ["normal", "upright"],
    ["italic", "italic"],
    ["-tex-mathit", "italic"],
    ["sans-serif", "sans"],
    ["bold-sans-serif", "sans"],
    ["sans-serif-italic", "sans"],
    ["monospace", "mono"],
]);
// MathJax merges adjacent mo nodes that lack operands into a single text node
// (e.g. \approx \approxeq → one mo with "≈≊"). This regex distinguishes
// merged Unicode symbols from ASCII operator names ("lim", "sin") that must
// stay intact.
var ALL_NON_ASCII = /^[^\x00-\x7F]+$/;
var findTypstSymbol = function (unicode) {
    var e_1, _a;
    var _b;
    var result = exports.typstSymbolMap.get(unicode);
    if (result !== undefined) {
        return result;
    }
    if (unicode.length > 1 && ALL_NON_ASCII.test(unicode)) {
        var parts = [];
        try {
            for (var unicode_1 = tslib_1.__values(unicode), unicode_1_1 = unicode_1.next(); !unicode_1_1.done; unicode_1_1 = unicode_1.next()) {
                var ch = unicode_1_1.value;
                parts.push((_b = exports.typstSymbolMap.get(ch)) !== null && _b !== void 0 ? _b : ch);
            }
        }
        catch (e_1_1) { e_1 = { error: e_1_1 }; }
        finally {
            try {
                if (unicode_1_1 && !unicode_1_1.done && (_a = unicode_1.return)) _a.call(unicode_1);
            }
            finally { if (e_1) throw e_1.error; }
        }
        return parts.join(' ');
    }
    return unicode;
};
exports.findTypstSymbol = findTypstSymbol;
//# sourceMappingURL=typst-symbol-map.js.map