...
var buildNumcasesGrid = function (node, serialize, countRow) {
    ...
    for (var i = 0; i < countRow; i++) {
        var mtrNode = node.childNodes[i];
        var startCol = mtrNode.kind === consts_1.MLABELEDTR ? 2 : 1; // skip label + prefix
        var cells = [];
...
var buildTaggedEqnArray = function (node, serialize, inputRows, countRow, preContent, postContent, inputRowsInline) {
    var res = (0, common_1.initTypstData)();
    ...
    if (explicitTagIndices.length === 1 && autoTagIndices.length === 0 && countRow > 1) {
        ...
        res.typst_inline = inputRowsInline
            ? joinRows(tslib_1.__spreadArray([], tslib_1.__read(inputRowsInline), false), preContent)
...
var buildUntaggedEqnArray = function (inputRows, preContent, postContent, inputRowsInline) {
    var res = (0, common_1.initTypstData)();
    var rows = tslib_1.__spreadArray([], tslib_1.__read(inputRows), false);
    if (postContent && rows.length > 0) {
        rows[rows.length - 1] += " ".concat(postContent);
    ...
    if (inputRowsInline) {
        var inlineRows = tslib_1.__spreadArray([], tslib_1.__read(inputRowsInline), false);
        if (postContent && inlineRows.length > 0) {
            inlineRows[inlineRows.length - 1] += " ".concat(postContent);
        }
 ...
 *  mtable. Returns true when any ancestor mtable will be rendered as mat():
 *  - Regular matrix or cases (not eqnArray)
 *  - eqnArray with row/column lines (rendered as mat to preserve augment)
 *  When the immediate outer mtable is a plain eqnArray (no lines), continues
 *  walking up to check if that eqnArray is itself nested. */
    ...
    while (current) {
        if (current.kind === 'mtd') {
            ...
            if ((outerTable === null || outerTable === void 0 ? void 0 : outerTable.kind) === 'mtable') {
                ...
                if (!isOuterEqnArray)
                    return true;
                // eqnArray with lines → rendered as mat() → we're inside a mat cell
...
var isInsideEqnArrayCellWithSiblings = function (node) {
    ...
    for (var i = 0; i < MAX_DEPTH && current; i++) {
        ...
        if (current.isInferred && ((_a = current.parent) === null || _a === void 0 ? void 0 : _a.kind) === 'mtd') {
            ...
            if (siblingCount <= 1)
                return false; // sole content — no need to mat()-ify
            var mtr_2 = current.parent.parent;
            var outerTable = mtr_2 === null || mtr_2 === void 0 ? void 0 : mtr_2.parent;
            if ((outerTable === null || outerTable === void 0 ? void 0 : outerTable.kind) === 'mtable') {
        ...
        if (current.kind === 'mtd') {
            ...
            if (contentCount <= 1)
                return false; // sole content
            var mtr_3 = current.parent;
            var outerTable = mtr_3 === null || mtr_3 === void 0 ? void 0 : mtr_3.parent;
            if ((outerTable === null || outerTable === void 0 ? void 0 : outerTable.kind) === 'mtable') {
...
var computeAugment = function (node) {
    ...
    for (var i = 0; i < columnlines.length; i++) {
        if (columnlines[i] === 'solid' || columnlines[i] === 'dashed') {
            var pos = i + 1;
            if (pos > maxVline)
                continue; // skip out-of-bounds vlines
    ...
    return "augment: #(".concat(parts.join(', '), ")");
};
/** Build a mat(delim: #none, ...) string for an eqnArray rendered as mat.
 *  align is a pre-formatted string like "#right" or "" (omit for center).
 *  augment is an optional augment string like "augment: #(hline: 2)". */
var buildEqnArrayMatStr = function (inputRows, align, augment) {
    var alignPart = align ? ", align: ".concat(align) : '';
    var augPart = augment ? ", ".concat(augment) : '';
    if (inputRows.length >= 2) {
        return "mat(delim: #none".concat(alignPart).concat(augPart, ", \n  ").concat(inputRows.join(';\n  '), ",\n)");
    }
    return "mat(delim: #none".concat(alignPart).concat(augPart, ", ").concat(inputRows.join('; '), ")");
};
/** eqnArray rendered as mat() — either nested inside a mat cell or has lines.
 *  addDisplay: wrap block output in display() (for nested; not for top-level with lines). */
var buildEqnArrayAsMat = function (inputRows, inputRowsInline, align, augment, addDisplay) {
    var mat = buildEqnArrayMatStr(inputRows, align, augment);
    var matInline = buildEqnArrayMatStr(inputRowsInline, align, augment);
    return {
        typst: addDisplay ? "display(".concat(mat, ")") : mat,
        typst_inline: matInline,
    };
};
...
var buildMatrix = function (node, rows, branchOpen, branchClose, rowsInline) {
    ...
    var frame = node.attributes.isSet('frame')
        ...
        : '';
    var augmentStr = computeAugment(node);
    ...
    if (frame === 'solid') {
        res = (0, common_1.addToTypstData)(res, { typst: "#align(center, box(stroke: ".concat(consts_1.BOX_STROKE, ", inset: ").concat(consts_1.BOX_INSET, ", $ ").concat(matExpr, " $))"), typst_inline: matExprInline !== null && matExprInline !== void 0 ? matExprInline : matExpr });
    }
    else {
...
var mtable = function (node, serialize) {
    ...
    var closeStr = hasParentDelims ? String((_b = (0, common_1.getProp)(parentMrow, 'close')) !== null && _b !== void 0 ? _b : '') : '';
    ...
    var firstRowDisplaystyle = node.childNodes.length > 0
        && ((_c = node.childNodes[0].attributes) === null || _c === void 0 ? void 0 : _c.get('displaystyle')) === true;
    ...
    var insideMatCell = isInsideMatrixCell(node);
    ...
    var insideEqnArrayCell = isGatheredLike && isInsideEqnArrayCellWithSiblings(node);
    ...
    for (var i = 0; i < countRow; i++) {
        ...
        for (var j = startCol; j < countColl; j++) {
            ...
            var inline = ((_g = cellData.typst_inline) !== null && _g !== void 0 ? _g : cellData.typst).trim();
            cells.push(block);
        ...
        if (eqnArrayAsMat) {
            ...
            for (var k = 0; k < cells.length; k += 2) {
                if (k + 1 < cells.length) {
                    ...
                    pairsInline.push("".concat(cellsInline[k], " &").concat(cellsInline[k + 1]));
                }
                else {
                    pairs.push(cells[k]);
    ...
    if (eqnArrayAsMat) {
        ...
        var hasEvenContent = nestedRawRows.some(function (r) {
            return r.some(function (c, k) { return k % 2 === 0 && c !== ''; });
        });
        ...
        if (isGathered || maxCols <= 0) {
            ...
            nestedAlign = '#left'; // all content in odd (left) columns: & content
        }
        else {
            nestedAlign = '#right'; // content in even (right) columns or mixed
        }
        ...
        var augment = hasLines ? computeAugment(node) : '';
        // Nested inside mat cell → wrap in display() to reset scriptlevel.
        // Top-level with lines → no outer display() (cells handle their own).
        ...
        var inlineRows = hasInlineDiff ? rowsInline : undefined;
        if (hasAnyTag) {
        ...
        // Cases environment
        var reverseParam = isReverseCases ? 'reverse: #true, ' : '';
        var casesBody = void 0;
        if (rows.length >= 2) {
            casesBody = "cases(".concat(reverseParam, "\n  ").concat(rows.join(',\n  '), ",\n)");
        }
        else {
            casesBody = "cases(".concat(reverseParam).concat(rows.join(', '), ")");
        }
        res = (0, common_1.addToTypstData)(res, { typst: casesBody });
    }
    else {
        res = buildMatrix(node, rows, branchOpen, branchClose, hasInlineDiff ? rowsInline : undefined);
    }
    // Wrap in display() for block output when this table is inside a mat() cell
    // to prevent Typst scriptlevel reduction. eqnArrays handle this via
    // buildEqnArrayAsMat; this covers cases() and plain matrices.
    if (insideMatCell) {
        var blockTypst = res.typst;
        res = { typst: "display(".concat(blockTypst, ")"), typst_inline: (_h = res.typst_inline) !== null && _h !== void 0 ? _h : blockTypst };
    }
    return res;
};
