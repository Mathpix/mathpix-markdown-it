...
var getLabelKey = function (labelCell) {
    var key = labelCell === null || labelCell === void 0 ? void 0 : labelCell.getProperty(consts_1.DATA_LABEL_KEY);
...
var extractTagFromConditionCell = function (cell) {
    var walk = function (n) {
        ...
        if (n.kind === 'mtext') {
            ...
            var match = text.match(consts_1.RE_TAG_EXTRACT);
...
var buildNumcasesGrid = function (node, serialize, countRow) {
    ...
    var autoTagEntry = '{ counter(math.equation).step(); context counter(math.equation).display(' + consts_1.DEFAULT_EQ_NUMBERING + ') }';
    var rowTagSources = [];
    for (var i = 0; i < countRow; i++) {
        ...
        if (condTag) {
            ...
            var isAutoNumber = !!labelCell.getProperty(consts_1.DATA_TAG_AUTO);
            if (!isAutoNumber) {
    ...
    for (var i = 0; i < countRow; i++) {
        ...
        if (info.source === 'condition') {
            ...
            tagText = '(' + info.content.replace(consts_1.RE_CONTENT_SPECIAL, '\\$&') + ')';
        }
        ...
        if (tagText && info.labelKey) {
            ...
            tagEntries.push('[#figure(kind: "' + consts_1.EQ_TAG_FIGURE_KIND + '", supplement: none, numbering: n => [' + tagText + '], [' + tagText + ']) <' + info.labelKey + '>]');
        }
        else if (tagText) {
            ...
            tagEntries.push('{ counter(math.equation).step(); context { let n = numbering(' + consts_1.DEFAULT_EQ_NUMBERING + ', ..counter(math.equation).get()); [#figure(kind: "' + consts_1.EQ_TAG_FIGURE_KIND + '", supplement: none, numbering: _ => n, [#n]) <' + info.labelKey + '>] } }');
        }
        else {
...
var buildMatrix = function (node, rows, branchOpen, branchClose) {
    ...
    if (hasDelimiters) {
        if (branchOpen) {
            params.push('delim: ' + (0, bracket_utils_1.delimiterToTypst)(branchOpen));
        }
    }
    else {
...
var mtable = function (node, serialize) {
    ...
    var openProp = parentMrow === null || parentMrow === void 0 ? void 0 : parentMrow.getProperty('open');
    var closeProp = parentMrow === null || parentMrow === void 0 ? void 0 : parentMrow.getProperty('close');
    ...
    var isEqnArray = !isNumcases && !isCases && node.childNodes.length > 0
        && ((_b = node.childNodes[0].attributes) === null || _b === void 0 ? void 0 : _b.get('displaystyle'));
    if (isNumcases) {
    ...
    if (isEqnArray) {
        ...
        var preContent = String(node.getProperty(consts_1.DATA_PRE_CONTENT) || '');
        var postContent = String(node.getProperty(consts_1.DATA_POST_CONTENT) || '');
        if (hasAnyTag) {
