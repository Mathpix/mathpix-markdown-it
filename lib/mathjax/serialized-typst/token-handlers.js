...
var wrapCancel = function (expr) {
    return "cancel(".concat((0, escape_utils_1.escapeContentSeparators)((0, escape_utils_1.escapeUnbalancedParens)(expr)), ")");
};
...
var mi = function (node, _serialize) {
    ...
    if (hasNot)
        typstValue = wrapCancel(typstValue);
...
var mo = function (node, _serialize) {
    ...
    if (unpaired)
        return hasNot ? singleTypst(wrapCancel(unpaired.typst)) : unpaired;
    ...
    if (multiword)
        return hasNot ? singleTypst(wrapCancel(multiword.typst.trim())) : multiword;
    ...
    if (namedOp)
        return hasNot ? singleTypst(wrapCancel(namedOp.typst.trim())) : namedOp;
    if (hasNot)
        typstValue = wrapCancel(typstValue);
...
var mtext = function (node, _serialize) {
    ...
    var textContent = "\"".concat(value.replace(/\\(?=")|\\$/g, '\\\\').replace(/"/g, '\\"'), "\"");
    var attrs = (0, common_1.getAttrs)(node);
    var mathvariant = attrs.mathvariant || '';
    if (mathvariant && mathvariant !== 'normal') {
        var fontFn = typst_symbol_map_1.typstFontMap.get(mathvariant);
