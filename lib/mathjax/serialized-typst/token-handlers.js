...
var SCRIPT_PARENT_KINDS = new Set([
...
var isInScriptContext = function (node) {
    return !!node.parent && SCRIPT_PARENT_KINDS.has(node.parent.kind);
};
...
var trySerializeNamedOperator = function (node, value, normalizedName) {
    if (isWordLikeToken(normalizedName)
        ...
        return singleTypst(withContextSpaces(node, "op(\"".concat(escapeTypstString(normalizedName), "\")")));
    }
    return null;
};
...
var mi = function (node, _serialize) {
    ...
    if (node.texClass === MmlNode_1.TEXCLASS.OP && value.length > 1 && !isKnownOperator) {
        typstValue = "op(\"".concat(escapeTypstString(value), "\")");
    }
    ...
    // Font wrapping: skip for known non-bold symbols, operators, and escape-form symbols (\#, \$)
        ...
        if (fontFn) {
            ...
            var inner = value.length > 1 && !isKnownSymbol ? "\"".concat(escapeTypstString(value), "\"") : typstValue;
