...
var needsSpaceAfter = function (node) {
    ...
    try {
        ...
        if (next && (0, common_1.getChildText)(next) === '\u2061' && index + 2 < node.parent.childNodes.length) {
...
var buildLimitBase = function (firstChild, baseTrimmed, base) {
    ...
    if (movablelimits === true) {
        ...
        return { typst: wrapper + '(' + (0, escape_utils_1.escapeContentSeparators)(basePlaceholder) + ')', typst_inline: base };
    }
    else if (movablelimits === false) {
        return { typst: wrapper + '(' + (0, escape_utils_1.escapeContentSeparators)(basePlaceholder) + ')' };
    }
    else {
