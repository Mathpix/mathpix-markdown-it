    ...
    return function (node, _serialize) {
        ...
        try {
            ...
            if (mathvariant && mathvariant !== 'normal') {
                var fontFn = typst_symbol_map_1.typstFontMap.get(mathvariant);
                if (fontFn) {
                    var content = escapeContentSeparators(value) || '""';
...
var mfrac = function () {
    return function (node, serialize) {
        ...
        try {
            ...
            var num = escapeContentSeparators(dataFirst.typst.trim()) || '""';
            var den = escapeContentSeparators(dataSecond.typst.trim()) || '""';
                ...
                if (hasVisibleOpen && hasVisibleClose) {
                    ...
                    if (openDelim === '|' && closeDelim === '|') {
                        ...
                        var escapedClose = (closeDelim in closeMatchOpen && closeMatchOpen[closeDelim] !== openDelim)
                            ? '\\' + closeDelim : close_2;
                        res = (0, common_1.addToTypstData)(res, { typst: 'lr(' + escapedOpen + ' ' + escapeLrSemicolons(trimmedContent) + ' ' + escapedClose + ')' });
                    }
                }
                else {
