...
var PHANTOM_SEARCH_MAX_DEPTH = 5;
    ...
    try {
        ...
        var visitChild_1 = function (child) {
            ...
            if (child.kind === 'mtext') {
                ...
                if (text) {
                    text = text.replace(common_1.RE_NBSP, ' ');
                    text = text.replace(common_1.RE_CONTENT_SPECIAL, '\\$&');
                    parts_1.push(text);
                }
            }
...
var needsSpaceBefore = function (node) {
    ...
    try {
        ...
        if (prev.kind === 'mi' || prev.kind === 'mo') {
            ...
            return common_1.RE_WORD_DOT_END.test(prevTypst);
        }
        if (prev.kind === 'mn') {
            return true;
        }
...
var needsSpaceAfter = function (node) {
    ...
    try {
        ...
        if (next && ((_a = next.childNodes[0]) === null || _a === void 0 ? void 0 : _a.text) === '\u2061' && !(0, common_1.isLastChild)(next)) {
        ...
        if (next && (next.kind === 'mi' || next.kind === 'mo')) {
            ...
            return common_1.RE_WORD_DOT_START.test(nextTypst);
        }
...
var mi = function () {
    return function (node, _serialize) {
        ...
        try {
            ...
            if (typstValue.length > 1 && common_1.RE_WORD_START.test(typstValue)) {
...
var mo = function () {
    return function (node, _serialize) {
        ...
        try {
            ...
            var normalizedValue = value.replace(common_1.RE_UNICODE_SPACES, ' ');
            ...
            // Don't add limits: #true here — parent handler (munderover/munder/mover) decides placement
            if (value.length > 1 && common_1.RE_WORD_START.test(value) && !typst_symbol_map_1.typstSymbolMap.has(value) && !TYPST_MATH_OPERATORS.has(value)) {
    ...
    return function (node, _serialize) {
        ...
        try {
            ...
            if (mathvariant && mathvariant !== 'normal') {
                var fontFn = typst_symbol_map_1.typstFontMap.get(mathvariant);
                if (fontFn) {
                    var content = (0, escape_utils_1.escapeContentSeparators)(value) || '""';
...
var mtext = function () {
    return function (node, _serialize) {
        ...
        try {
            ...
            value = value.replace(common_1.RE_NBSP, ' ');
            ...
            var textContent = '"' + value.replace(/"/g, '\\"') + '"';
...
var mfrac = function () {
    return function (node, serialize) {
        ...
        try {
            ...
            var num = (0, escape_utils_1.escapeContentSeparators)(dataFirst.typst.trim()) || '""';
            var den = (0, escape_utils_1.escapeContentSeparators)(dataSecond.typst.trim()) || '""';
...
var matchBraceAnnotation = function (baseTrimmed, annotation, kinds) {
    ...
    if (!m || !kinds.includes(m[1]))
        return null;
    return { typst: m[1] + '(' + m[2] + ', ' + (0, escape_utils_1.escapeContentSeparators)(annotation) + ')' };
};
    ...
    return function (node, serialize) {
        ...
        try {
            ...
            if (baseTrimmed && needsScriptsWrapper(baseTrimmed)) {
                res = (0, common_1.addToTypstData)(res, { typst: 'scripts(' + (0, escape_utils_1.escapeContentSeparators)(baseTrimmed) + ')' });
            }
            else {
...
var msqrt = function () {
    return function (node, serialize) {
        ...
        try {
            var firstChild = node.childNodes[0] || null;
            var dataFirst = firstChild ? serialize.visitNode(firstChild, '') : (0, common_1.initTypstData)();
            var content = (0, escape_utils_1.escapeContentSeparators)(dataFirst.typst.trim()) || '""';
...
var mroot = function () {
    return function (node, serialize) {
        ...
        try {
            ...
            var radicandContent = (0, escape_utils_1.escapeContentSeparators)(dataRadicand.typst.trim()) || '""';
            // Typst root: root(index, radicand)
            res = (0, common_1.addToTypstData)(res, {
                typst: 'root(' + (0, escape_utils_1.escapeContentSeparators)(dataIndex.typst.trim()) + ', ' + radicandContent + ')'
            });
            return res;
        }
        catch (e) {
...
var buildLimitBase = function (firstChild, baseTrimmed, base) {
    ...
    var baseIsCustomOp = common_1.RE_OP_WRAPPER.test(baseTrimmed);
    ...
    if (movablelimits === true) {
        ...
        return { typst: wrapper + '(' + (0, escape_utils_1.escapeContentSeparators)(baseTrimmed) + ')', typst_inline: base };
    }
    else if (movablelimits === false) {
        // Explicit \limits — force below/above placement in both modes
        return { typst: wrapper + '(' + (0, escape_utils_1.escapeContentSeparators)(baseTrimmed) + ')' };
    }
    else {
        ...
        if (common_1.RE_OP_WRAPPER.test(baseTrimmed) && (firstChild === null || firstChild === void 0 ? void 0 : firstChild.texClass) === MmlNode_1.TEXCLASS.OP) {
    ...
    return function (node, serialize) {
        ...
        try {
            ...
            if ((firstChild === null || firstChild === void 0 ? void 0 : firstChild.kind) === 'mi' && (secondChild === null || secondChild === void 0 ? void 0 : secondChild.kind) === 'mo') {
                var baseText = getNodeText(firstChild);
                var overChar = getNodeText(secondChild);
            ...
            if (secondChild && secondChild.kind === 'mo') {
                var accentChar = getNodeText(secondChild);
            ...
            if (over) {
                ...
                var braceRes = matchBraceAnnotation(baseTrimmed, over, ['overbrace', 'overbracket']);
                if (braceRes) {
                    res = (0, common_1.addToTypstData)(res, braceRes);
                    return res;
                }
...
var munder = function () {
    return function (node, serialize) {
        ...
        try {
            ...
            if ((firstChild === null || firstChild === void 0 ? void 0 : firstChild.kind) === 'mi' && (secondChild === null || secondChild === void 0 ? void 0 : secondChild.kind) === 'mo') {
                var baseText = getNodeText(firstChild);
                var underChar = getNodeText(secondChild);
            ...
            if (under) {
                ...
                var braceRes = matchBraceAnnotation(baseTrimmed, under, ['underbrace', 'underbracket']);
                if (braceRes) {
                    res = (0, common_1.addToTypstData)(res, braceRes);
                    return res;
                }
    ...
    return function (node, serialize) {
        ...
        try {
            ...
            if (!hasPrescripts) {
                ...
                res = (0, common_1.addToTypstData)(res, { typst: baseTrimmed });
                if (postSub) {
                    res = (0, common_1.addToTypstData)(res, { typst: (0, common_1.formatScript)('_', postSub) });
                }
                if (postSup) {
                    res = (0, common_1.addToTypstData)(res, { typst: (0, common_1.formatScript)('^', postSup) });
                }
            }
            else {
                ...
                res = (0, common_1.addToTypstData)(res, {
                    typst: 'attach(' + (0, escape_utils_1.escapeContentSeparators)(baseTrimmed) + ', ' + parts.join(', ') + ')'
                });
            }
            return res;
        }
...
var replaceUnpairedBrackets = function (expr) {
    ...
    if (!common_1.RE_BRACKET_CHARS.test(expr))
        return expr;
    var brackets = [];
    for (var i = 0; i < expr.length; i++) {
        ...
        if ('[](){}'.includes(ch)) {
            ...
            if (ch === '(' && i > 0 && common_1.RE_WORD_DOT_END.test(expr[i - 1])) {
                var depth = 1;
                i++;
    ...
    try {
        for (var _d = tslib_1.__values(Object.entries(OPEN_BRACKETS)), _f = _d.next(); !_f.done; _f = _d.next()) {
    ...
    for (var i = 0; i < expr.length; i++) {
        if (unmatched.has(i)) {
            ...
            if (result.length > 0 && common_1.RE_WORD_DOT_END.test(result[result.length - 1])) {
                result += ' ';
            }
            result += sym;
...
var markUnpairedBrackets = function (root) {
    ...
    var isLeftRightDelimiter = function (moNode) {
        ...
        if (pp.texClass !== MmlNode_1.TEXCLASS.INNER)
            return false;
        if (!pp.hasOwnProperty('open') && !pp.hasOwnProperty('close'))
            return false;
...
var extractTagFromConditionCell = function (cell) {
    var walk = function (n) {
        ...
        if (n.kind === 'mtext') {
            ...
            var match = text.match(common_1.RE_TAG_EXTRACT);
...
var buildNumcasesGrid = function (node, serialize, countRow) {
    ...
    for (var i = 0; i < countRow; i++) {
        ...
        if (condTag) {
            ...
            var isAutoNumber = !!((_b = labelCell.properties) === null || _b === void 0 ? void 0 : _b['data-tag-auto']);
            if (!isAutoNumber) {
    ...
    for (var i = 0; i < countRow; i++) {
        ...
        for (var j = startCol; j < mtrNode.childNodes.length; j++) {
            ...
            if (j === mtrNode.childNodes.length - 1 && rowTagSources[i].source === 'condition') {
                trimmed = trimmed.replace(common_1.RE_TAG_STRIP, '');
        ...
        if (cells.length === 1) {
            ...
            caseRows.push((0, escape_utils_1.escapeCasesSeparators)(replaceUnpairedBrackets(cells[0])));
        }
        else {
            caseRows.push(cells.map(function (c) { return (0, escape_utils_1.escapeCasesSeparators)(replaceUnpairedBrackets(c)); }).join(' & '));
        }
    }
    var casesContent;
    ...
    for (var i = 0; i < countRow; i++) {
        ...
        if (info.source === 'condition') {
            ...
            tagText = '(' + info.content.replace(common_1.RE_CONTENT_SPECIAL, '\\$&') + ')';
        }
...
var buildTaggedEqnArray = function (node, serialize, rows, countRow, preContent, postContent) {
    var _a;
    var res = (0, common_1.initTypstData)();
    ...
    for (var i = 0; i < countRow; i++) {
        ...
        if (mtrNode.kind === 'mlabeledtr' && mtrNode.childNodes.length > 0) {
            ...
            var tagContent = serializeTagContent(labelCell, serialize);
            var isAutoNumber = !!((_a = labelCell.properties) === null || _a === void 0 ? void 0 : _a['data-tag-auto']);
    ...
    if (explicitTagIndices.length === 1 && autoTagIndices.length === 0 && countRow > 1) {
        ...
        var mathContent = preContent
            ? preContent + ' \\\n' + rows.join(' \\\n')
            : rows.join(' \\\n');
        var supplementPart = info.labelKey ? ', supplement: none' : '';
        var numberAlignPart = ', number-align: ' + numberAlign;
        var labelSuffix = info.labelKey ? ' <' + info.labelKey + '>' : '';
        var block = '#math.equation(block: true' + supplementPart
...
var buildUntaggedEqnArray = function (rows, preContent, postContent) {
    var res = (0, common_1.initTypstData)();
    if (postContent && rows.length > 0) {
        rows[rows.length - 1] = rows[rows.length - 1] + ' ' + postContent;
    }
        ...
        try {
            ...
            for (var i = 0; i < countRow; i++) {
                ...
                if (isEqnArray) {
                    ...
                    rows.push(cells.map(function (c) { return (0, escape_utils_1.escapeCasesSeparators)(replaceUnpairedBrackets(c)); }).join(' & '));
                }
                else {
            ...
            if (isEqnArray) {
                ...
                var preContent = ((_g = node.properties) === null || _g === void 0 ? void 0 : _g['data-pre-content']) || '';
                var postContent = ((_h = node.properties) === null || _h === void 0 ? void 0 : _h['data-post-content']) || '';
                if (hasAnyTag) {
                ...
                if (hasVisibleOpen && hasVisibleClose) {
                    ...
                    if (openDelim === '|' && closeDelim === '|') {
                        res = (0, common_1.addToTypstData)(res, { typst: hasSep
                                ? 'lr(| ' + (0, escape_utils_1.escapeLrSemicolons)(trimmedContent) + ' |)'
                                : 'abs(' + trimmedContent + ')' });
                    }
                        ...
                        var escapedOpen = (openDelim in OPEN_BRACKETS && OPEN_BRACKETS[openDelim] !== closeDelim)
                            ? '\\' + openDelim : open_2;
                        var escapedClose = (closeDelim in CLOSE_BRACKETS && CLOSE_BRACKETS[closeDelim] !== openDelim)
                            ? '\\' + closeDelim : close_2;
                        res = (0, common_1.addToTypstData)(res, { typst: 'lr(' + escapedOpen + ' ' + (0, escape_utils_1.escapeLrSemicolons)(trimmedContent) + ' ' + escapedClose + ')' });
                    }
                }
                else {
                    ...
                    var openEsc = openDelim ? escapeLrOpenDelimiter(openDelim) : '';
                    var closeEsc = closeDelim ? escapeLrOpenDelimiter(closeDelim) : '';
                    if (openEsc) {
                        res = (0, common_1.addToTypstData)(res, { typst: 'lr(' + openEsc + ' ' + (0, escape_utils_1.escapeLrSemicolons)(trimmed) + ')' });
                    }
                    else if (closeEsc) {
                        res = (0, common_1.addToTypstData)(res, { typst: 'lr(' + (0, escape_utils_1.escapeLrSemicolons)(trimmed) + ' ' + closeEsc + ')' });
                    }
                    else {
...
var hasPhantomChild = function (node) {
    var check = function (n, depth) {
        var e_13, _a;
        if (!n || depth > PHANTOM_SEARCH_MAX_DEPTH)
            return false;
        if (n.kind === 'mphantom')
            return true;
        if (n.childNodes) {
        ...
        try {
            ...
            if (notation.indexOf('box') > -1) {
                ...
                if (notation.indexOf('downdiagonalstrike') > -1 && notation.indexOf('updiagonalstrike') === -1) {
                    res = (0, common_1.addToTypstData)(res, { typst: 'cancel(inverted: #true, ' + (0, escape_utils_1.escapeContentSeparators)(content) + ')' });
                }
                else {
                    res = (0, common_1.addToTypstData)(res, { typst: 'cancel(' + (0, escape_utils_1.escapeContentSeparators)(content) + ')' });
                }
            }
                ...
                res = (0, common_1.addToTypstData)(res, { typst: 'overline(")"' + (0, escape_utils_1.escapeContentSeparators)((0, escape_utils_1.escapeUnbalancedParens)(content)) + ')' });
            }
                ...
                res = (0, common_1.addToTypstData)(res, { typst: 'sqrt(' + (0, escape_utils_1.escapeContentSeparators)((0, escape_utils_1.escapeUnbalancedParens)(content)) + ')' });
            }
                ...
                if (content.startsWith(')') || content.startsWith('\\)')) {
                    ...
                    var inner = content.slice(skip).trim().replace(common_1.RE_TRAILING_SPACING, '');
                    res = (0, common_1.addToTypstData)(res, { typst: 'underline(")"' + (0, escape_utils_1.escapeContentSeparators)(inner) + ')' });
                }
                else {
                    res = (0, common_1.addToTypstData)(res, { typst: 'underline(' + (0, escape_utils_1.escapeContentSeparators)((0, escape_utils_1.escapeUnbalancedParens)(content)) + ')' });
                }
            }
            else {
...
var handle = function (node, serialize) {
    var handler = handlers[node.kind] || defaultHandler;
