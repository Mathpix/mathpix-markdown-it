...
exports.handle = exports.replaceUnpairedBrackets = void 0;
        ...
        try {
            ...
            if (sup) {
                ...
                if (braceMatch && (braceMatch[1] === 'overbrace' || braceMatch[1] === 'overbracket')) {
                    res = (0, common_1.addToTypstData)(res, { typst: braceMatch[1] + '(' + braceMatch[2] + ', ' + sup + ')' });
                    return res;
                }
            }
        ...
        try {
            ...
            if (sub) {
                ...
                if (braceMatch && (braceMatch[1] === 'underbrace' || braceMatch[1] === 'underbracket')) {
                    res = (0, common_1.addToTypstData)(res, { typst: braceMatch[1] + '(' + braceMatch[2] + ', ' + sub + ')' });
                    return res;
                }
            }
            // \nolimits: wrap known limit-type operators in scripts() to force side placement
    ...
    return function (node, serialize) {
        ...
        try {
            ...
            if (over) {
                ...
                var braceMatch = BRACE_ANNOTATION_RE.exec(baseTrimmed);
                if (braceMatch && (braceMatch[1] === 'overbrace' || braceMatch[1] === 'overbracket')) {
                    res = (0, common_1.addToTypstData)(res, { typst: braceMatch[1] + '(' + braceMatch[2] + ', ' + over + ')' });
                    return res;
                }
    ...
    return function (node, serialize) {
        ...
        try {
            ...
            if (under) {
                ...
                var braceMatch = BRACE_ANNOTATION_RE.exec(baseTrimmed);
                if (braceMatch && (braceMatch[1] === 'underbrace' || braceMatch[1] === 'underbracket')) {
                    res = (0, common_1.addToTypstData)(res, { typst: braceMatch[1] + '(' + braceMatch[2] + ', ' + under + ')' });
                    return res;
                }
                var baseData = buildLimitBase(firstChild, baseTrimmed, dataFirst.typst);
                res = (0, common_1.addToTypstData)(res, baseData);
    ...
    return function (node, serialize) {
        ...
        try {
            ...
            if (notation.indexOf('box') > -1) {
                ...
                res = (0, common_1.addToTypstData)(res, { typst: 'overline(")"' + escapeUnbalancedParens(content) + ')' });
            }
                ...
                res = (0, common_1.addToTypstData)(res, { typst: 'sqrt(' + escapeUnbalancedParens(content) + ')' });
            }
                ...
                if (content.startsWith(')')) {
                    var inner = content.slice(1).trim().replace(/\s+(?:med|thin|thick|quad)$/, '');
                    res = (0, common_1.addToTypstData)(res, { typst: 'underline(")"' + inner + ')' });
                }
                else {
                    res = (0, common_1.addToTypstData)(res, { typst: 'underline(' + escapeUnbalancedParens(content) + ')' });
                }
            }
            else {
