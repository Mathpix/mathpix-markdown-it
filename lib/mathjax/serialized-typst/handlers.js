        ...
        try {
            ...
            if (sup) {
                ...
                if (braceMatch && (braceMatch[1] === 'overbrace' || braceMatch[1] === 'overbracket')) {
                    ...
                    res = (0, common_1.addToTypstData)(res, { typst: braceMatch[1] + '(' + braceMatch[2] + ', ' + (0, escape_utils_1.escapeContentSeparators)(sup) + ')' });
                    return res;
                }
            }
        ...
        try {
            ...
            if (sub) {
                ...
                if (braceMatch && (braceMatch[1] === 'underbrace' || braceMatch[1] === 'underbracket')) {
                    // braceMatch[2] already processed by accent handler — don't re-escape
                    res = (0, common_1.addToTypstData)(res, { typst: braceMatch[1] + '(' + braceMatch[2] + ', ' + (0, escape_utils_1.escapeContentSeparators)(sub) + ')' });
                    return res;
                }
            }
    ...
    return function (node, serialize) {
        ...
        try {
            ...
            if (over) {
                ...
                if (braceMatch && (braceMatch[1] === 'overbrace' || braceMatch[1] === 'overbracket')) {
                    // braceMatch[2] already processed by accent handler — don't re-escape
                    res = (0, common_1.addToTypstData)(res, { typst: braceMatch[1] + '(' + braceMatch[2] + ', ' + (0, escape_utils_1.escapeContentSeparators)(over) + ')' });
                    return res;
                }
    ...
    return function (node, serialize) {
        ...
        try {
            ...
            if (under) {
                ...
                if (braceMatch && (braceMatch[1] === 'underbrace' || braceMatch[1] === 'underbracket')) {
                    // braceMatch[2] already processed by accent handler — don't re-escape
                    res = (0, common_1.addToTypstData)(res, { typst: braceMatch[1] + '(' + braceMatch[2] + ', ' + (0, escape_utils_1.escapeContentSeparators)(under) + ')' });
                    return res;
                }
                var baseData = buildLimitBase(firstChild, baseTrimmed, dataFirst.typst);
                res = (0, common_1.addToTypstData)(res, baseData);
    ...
    return function (node, serialize) {
        ...
        try {
            ...
            if (isEqnArray) {
                ...
                var preContent = ((_j = node.properties) === null || _j === void 0 ? void 0 : _j['data-pre-content']) || '';
                var postContent = ((_k = node.properties) === null || _k === void 0 ? void 0 : _k['data-post-content']) || '';
                if (hasAnyTag) {
                    ...
                    for (var i = 0; i < countRow; i++) {
                        ...
                        if (mtrNode.kind === 'mlabeledtr' && mtrNode.childNodes.length > 0) {
                            ...
                            var tagContent = serializeTagContent(labelCell, serialize);
                            var isAutoNumber = !!((_l = labelCell.properties) === null || _l === void 0 ? void 0 : _l['data-tag-auto']);
                    ...
                    if (explicitTagIndices.length === 1 && autoTagIndices.length === 0 && countRow > 1) {
                        ...
                        var noTagContent = preContent
                            ? preContent + ' \\\n' + rows.join(' \\\n')
                            : rows.join(' \\\n');
                        res = (0, common_1.addToTypstData)(res, { typst: noTagContent });
                    }
                }
                else {
                    ...
                    var noTagContent2 = preContent
                        ? preContent + ' \\\n' + rows.join(' \\\n')
                        : rows.join(' \\\n');
                    res = (0, common_1.addToTypstData)(res, { typst: noTagContent2 });
                }
            }
            else if (isCases) {
                // Cases environment
            ...
            if (isLeftRight && !hasTableChild) {
                ...
                if (hasVisibleOpen && hasVisibleClose) {
                    ...
                    if (openDelim === '|' && closeDelim === '|') {
                        ...
                        var openMatchClose = { '(': ')', '[': ']', '{': '}' };
                        var closeMatchOpen = { ')': '(', '}': '{' };
                        var escapedOpen = (openDelim in openMatchClose && openMatchClose[openDelim] !== closeDelim)
                            ? '\\' + openDelim : open_2;
                        var escapedClose = (closeDelim in closeMatchOpen && closeMatchOpen[closeDelim] !== openDelim)
                            ? '\\' + closeDelim : close_2;
