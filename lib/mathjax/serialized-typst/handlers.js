...
var mroot = function () {
    return function (node, serialize) {
        ...
        try {
            ...
            var radicandContent = dataRadicand.typst.trim() || '""';
...
var escapeCasesCommas = function (expr) {
    var depth = 0;
    var result = '';
...
var isInsideNonEqnArrayTable = function (node) {
    var _a, _b;
    var n = node.parent;
    while (n) {
        if (n.kind === 'mtable') {
            var firstRow = (_a = n.childNodes) === null || _a === void 0 ? void 0 : _a[0];
            return !((_b = firstRow === null || firstRow === void 0 ? void 0 : firstRow.attributes) === null || _b === void 0 ? void 0 : _b.get('displaystyle'));
        }
        n = n.parent;
    }
    return false;
        ...
        try {
            ...
            if (isNumcases) {
                ...
                for (var i = 0; i < countRow; i++) {
                    ...
                    if (cells.length === 1) {
                        ...
                        caseRows.push(cells.map(function (c) { return replaceUnpairedBrackets(c); }).join(' & '));
                    }
                }
            ...
            for (var i = 0; i < countRow; i++) {
                ...
                if (isEqnArray) {
                    ...
                    rows.push(cells.map(function (c) { return escapeCasesCommas(replaceUnpairedBrackets(c)); }).join(' & '));
                }
                else {
                    rows.push(cells.map(function (c) { return replaceUnpairedBrackets(c); }).join(', '));
                }
            }
            if (isEqnArray) {
            ...
            if (isLeftRight && !hasTableChild) {
                ...
                if (hasVisibleOpen && hasVisibleClose) {
                    ...
                    if (openDelim === '|' && closeDelim === '|') {
                        res = (0, common_1.addToTypstData)(res, { typst: 'abs(' + trimmedContent + ')' });
                    }
                        ...
                        res = (0, common_1.addToTypstData)(res, { typst: 'lr(' + open_2 + ' ' + trimmedContent + ' ' + close_2 + ')' });
                    }
                }
                else {
                    ...
                    if (isInsideNonEqnArrayTable(node)) {
                        ...
                        if (openEsc) {
                            res = (0, common_1.addToTypstData)(res, { typst: 'lr(' + openEsc + ' ' + trimmed + ')' });
                        }
                        else if (closeEsc) {
                            res = (0, common_1.addToTypstData)(res, { typst: 'lr(' + trimmed + ' ' + closeEsc + ')' });
                        }
                        else {
                        ...
                        if (hasVisibleOpen) {
                            res = (0, common_1.addToTypstData)(res, { typst: open_2 + ' ' + trimmed });
                        }
                        else if (hasVisibleClose) {
                            res = (0, common_1.addToTypstData)(res, { typst: trimmed + ' ' + close_2 });
                        }
                        else {
                            res = (0, common_1.addToTypstData)(res, { typst: trimmed });
                        }
