...
var isDuplicateLabel = function (key) {
    return !!(0, labels_1.getLabelByKeyFromLabelsList)(key);
};
...
var getLabelKey = function (labelCell, labels) {
    var e_1, _a;
    var _b;
    ...
    if (labels) {
        try {
            var id = (_b = labelCell.attributes) === null || _b === void 0 ? void 0 : _b.get('id');
            if (id) {
                try {
                    for (var _c = tslib_1.__values(Object.entries(labels)), _d = _c.next(); !_d.done; _d = _c.next()) {
                        var _f = tslib_1.__read(_d.value, 2), labelKey = _f[0], info = _f[1];
                        if ((info === null || info === void 0 ? void 0 : info.id) === id) {
...
var isInsideEqnArrayCellWithSiblings = function (node) {
    ...
    var current = node.parent;
    for (var i = 0; i < consts_1.TABLE_ANCESTOR_MAX_DEPTH && current; i++) {
...
var buildFigureTag = function (tagContent, labelKey) {
    ...
    return "[".concat(figure, " <").concat((0, labels_1.sanitizeLabel)(labelKey), ">]");
};
...
var buildAutoTagWithLabel = function (labelKey) {
    ...
    return "{ counter(math.equation).step(); context { let n = ".concat(getNum, "; [").concat(figure, " <").concat((0, labels_1.sanitizeLabel)(labelKey), ">] } }");
};
