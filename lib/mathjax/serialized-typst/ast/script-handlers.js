...
var matchBraceAnnotation = function (baseNode, annotationNode, kinds) {
    var inner = unwrapSeq(baseNode);
    if (inner.type === 'func' && kinds.includes(inner.name)) {
...
var handleScriptAst = function (node, serialize, config) {
    ...
    if (baseEmpty && subEmpty && supEmpty) {
        return builders_1.EMPTY_RESULT;
    }
...
var moverAst = function (node, serialize) {
    ...
    var baseEmpty = (0, builders_1.isEmptyNode)(baseNode);
    var overEmpty = (0, builders_1.isEmptyNode)(overNode);
    ...
    var rawBase = baseEmpty ? '' : (0, serialize_1.serializeTypstMath)(baseNode).trim();
    var over = overEmpty ? '' : (0, serialize_1.serializeTypstMath)(overNode).trim();
    ...
    return {
        node: !baseEmpty ? baseNode : (0, builders_1.placeholder)()
    };
};
...
var munderAst = function (node, serialize) {
    ...
    var baseEmpty = (0, builders_1.isEmptyNode)(baseNode);
    var underEmpty = (0, builders_1.isEmptyNode)(underNode);
    ...
    var rawBase = baseEmpty ? '' : (0, serialize_1.serializeTypstMath)(baseNode).trim();
    var under = underEmpty ? '' : (0, serialize_1.serializeTypstMath)(underNode).trim();
    if (under) {
