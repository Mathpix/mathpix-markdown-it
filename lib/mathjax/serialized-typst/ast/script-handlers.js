...
var msupAst = function (node, serialize) {
    ...
    if (isOpeningBracketBase(node.childNodes[0])) {
        ...
        var supNode_1 = visitChildNode(serialize, node.childNodes[1] || null);
        return {
            node: (0, builders_1.seq)([bracketNode, (0, builders_1.scriptNode)((0, builders_1.placeholder)(), { sup: supNode_1 })])
        };
    }
    var baseNode = visitChildNode(serialize, node.childNodes[0] || null);
    var supNode = visitChildNode(serialize, node.childNodes[1] || null);
    var baseTrimmed = (0, serialize_1.serializeTypstMath)(baseNode).trim();
    var sup = (0, serialize_1.serializeTypstMath)(supNode).trim();
    ...
    if (baseTrimmed && needsScriptsWrapper(baseTrimmed)) {
        return {
            node: (0, builders_1.scriptNode)((0, builders_1.funcCall)('scripts', [(0, builders_1.posArg)((0, builders_1.mathVal)(baseNode))]), { sup: supNode })
        };
    }
    // Prime shorthand: ′ → ', ″ → '', ‴ → '''
    if (sup) {
    ...
    if (needsLimitsMode(baseTrimmed, firstChild)) {
        return {
            node: (0, builders_1.scriptNode)(addLimitsToNode(baseNode), sup ? { sup: supNode } : {})
        };
    }
    return {
        node: (0, builders_1.scriptNode)(baseNode, sup ? { sup: supNode } : {})
    };
};
...
var msubAst = function (node, serialize) {
    if (isOpeningBracketBase(node.childNodes[0])) {
        var bracketNode = visitChildNode(serialize, node.childNodes[0]);
        var subNode_1 = visitChildNode(serialize, node.childNodes[1] || null);
        return {
            node: (0, builders_1.seq)([bracketNode, (0, builders_1.scriptNode)((0, builders_1.placeholder)(), { sub: subNode_1 })])
        };
    }
    var baseNode = visitChildNode(serialize, node.childNodes[0] || null);
    var subNode = visitChildNode(serialize, node.childNodes[1] || null);
    var baseTrimmed = (0, serialize_1.serializeTypstMath)(baseNode).trim();
    var sub = (0, serialize_1.serializeTypstMath)(subNode).trim();
    ...
    if (baseTrimmed && needsScriptsWrapper(baseTrimmed)) {
        return {
            node: (0, builders_1.scriptNode)((0, builders_1.funcCall)('scripts', [(0, builders_1.posArg)((0, builders_1.mathVal)(baseNode))]), { sub: subNode })
        };
    }
    var firstChild = node.childNodes[0] || null;
    if (needsLimitsMode(baseTrimmed, firstChild)) {
        return {
            node: (0, builders_1.scriptNode)(addLimitsToNode(baseNode), sub ? { sub: subNode } : {})
        };
    }
    return {
        node: (0, builders_1.scriptNode)(baseNode, sub ? { sub: subNode } : {})
    };
};
...
var msubsupAst = function (node, serialize) {
    if (isOpeningBracketBase(node.childNodes[0])) {
        var bracketNode = visitChildNode(serialize, node.childNodes[0]);
        var subNode_2 = visitChildNode(serialize, node.childNodes[1] || null);
        var supNode_2 = visitChildNode(serialize, node.childNodes[2] || null);
        var opts_1 = {};
        var sub_1 = (0, serialize_1.serializeTypstMath)(subNode_2).trim();
        var sup_1 = (0, serialize_1.serializeTypstMath)(supNode_2).trim();
        if (sub_1) {
            opts_1.sub = subNode_2;
        }
        if (sup_1) {
            opts_1.sup = supNode_2;
        }
        return {
    ...
    var baseNode = visitChildNode(serialize, node.childNodes[0] || null);
    var subNode = visitChildNode(serialize, node.childNodes[1] || null);
    var supNode = visitChildNode(serialize, node.childNodes[2] || null);
    var baseTrimmed = (0, serialize_1.serializeTypstMath)(baseNode).trim();
    var sub = (0, serialize_1.serializeTypstMath)(subNode).trim();
    var sup = (0, serialize_1.serializeTypstMath)(supNode).trim();
...
var munderoverAst = function (node, serialize) {
    ...
    var rawBase = (0, serialize_1.serializeTypstMath)(baseNode).trim();
    var under = (0, serialize_1.serializeTypstMath)(underNode).trim();
    var over = (0, serialize_1.serializeTypstMath)(overNode).trim();
    var limitBase = buildLimitBase(firstChild, rawBase, baseNode);
    var opts = {};
    if (under) {
        opts.sub = underNode;
