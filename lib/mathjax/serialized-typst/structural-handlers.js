...
var mrow = function (node, serialize) {
    ...
    if (isLeftRight && !hasTableChild) {
        ...
        for (var i = 0; i < node.childNodes.length; i++) {
            ...
            var inlineTypst = (_a = data.typst_inline) !== null && _a !== void 0 ? _a : data.typst;
        ...
        var buildLrExpr = function (cnt) {
            if (hasVisibleOpen_1 && hasVisibleClose_1) {
                ...
                if (openDelim === '|' && closeDelim === '|') {
                    var escaped = (0, escape_utils_1.escapeLrSemicolons)(trimmed);
                    ...
                    var escapedClose = (closeDelim in consts_1.CLOSE_BRACKETS && consts_1.CLOSE_BRACKETS[closeDelim] !== openDelim)
                        ? '\\' + closeDelim : close_1;
                    return "lr(".concat(escapedOpen, " ").concat((0, escape_utils_1.escapeLrSemicolons)(trimmed), " ").concat(escapedClose, ")");
                }
            }
            else {
                var trimmed = cnt.trim();
                ...
                if (openEsc) {
                    return "lr(".concat(openEsc, " ").concat((0, escape_utils_1.escapeLrSemicolons)(trimmed), ")");
                }
                else if (closeEsc) {
                    return "lr(".concat((0, escape_utils_1.escapeLrSemicolons)(trimmed), " ").concat(closeEsc, ")");
                }
                else {
                    return trimmed;
                }
