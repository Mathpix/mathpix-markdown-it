...
var mpadded = function (node, serialize) {
    ...
    var isZeroWidth = atr.width === 0 || atr.width === '0';
    var isZeroHeight = atr.height === 0 || atr.height === '0';
...
var buildStrokeSides = function (words) {
    return BORDER_SIDES.map(function (side) {
        return "".concat(side, ": ").concat(words.has(side) ? consts_1.BOX_STROKE : '0pt');
    }).join(', ');
};
var menclose = function (node, serialize) {
    ...
    if (hasNotation(words, 'box')) {
        ...
        res = (0, common_1.addToTypstData)(res, { typst: "#align(center, box(stroke: ".concat(consts_1.BOX_STROKE, ", inset: ").concat(consts_1.BOX_INSET, ", $").concat(content, "$))"), typst_inline: content });
    }
        ...
        var escaped = (0, escape_utils_1.escapeContentSeparators)((0, escape_utils_1.escapeUnbalancedParens)(content));
        if (hasNotation(words, 'updiagonalstrike') && hasNotation(words, 'downdiagonalstrike')) {
            res = (0, common_1.addToTypstData)(res, { typst: "cancel(cross: #true, ".concat(escaped, ")") });
        }
        else if (hasNotation(words, 'downdiagonalstrike')) {
            res = (0, common_1.addToTypstData)(res, { typst: "cancel(inverted: #true, ".concat(escaped, ")") });
        }
        else {
            res = (0, common_1.addToTypstData)(res, { typst: "cancel(".concat(escaped, ")") });
        }
    }
        ...
        res = (0, common_1.addToTypstData)(res, { typst: "#align(center, circle(inset: ".concat(consts_1.BOX_INSET, ", $").concat(content, "$))"), typst_inline: content });
    }
        ...
        res = (0, common_1.addToTypstData)(res, {
            typst: "#align(center, box(stroke: (".concat(sides, "), inset: ").concat(consts_1.BOX_INSET, ", $ ").concat(content, " $))"),
            typst_inline: content,
        });
    }
...
var hasAdjacentTripledashStyle = function (node) {
    ...
    for (var depth = 0; depth < 3 && cur; depth++) {
        ...
        try {
            for (var _b = (e_6 = void 0, tslib_1.__values(parent_1.childNodes)), _c = _b.next(); !_c.done; _c = _b.next()) {
...
var mstyle = function (node, serialize) {
    ...
    if (isTripledashStyle(node)) {
        var data_1 = (0, common_1.handleAll)(node, serialize);
        var content_1 = data_1.typst.trim();
        var replaced = content_1.replace(QUOTED_DASH_GLOBAL, 'hyph');
        if (replaced !== content_1) {
            res = (0, common_1.addToTypstData)(res, { typst: replaced });
            return res;
        }
    }
    var atr = (0, common_1.getAttrs)(node);
