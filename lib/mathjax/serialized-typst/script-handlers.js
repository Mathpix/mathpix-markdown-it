...
var safeFormatScript = function (prefix, content) {
    return (0, common_1.formatScript)(prefix, (0, escape_utils_1.escapeUnbalancedParens)(content));
};
...
var stripDashes = function (s) {
    return s.replace(HIDE_PATTERN, '').replace(DASH_CHARS, '');
};
...
var TYPST_SEPARATOR_BASES = new Set([',', ';']);
/** Return a safe script base: wraps separator chars in quotes, uses '""' for empty bases. */
var safeScriptBase = function (base, baseTrimmed) {
    if (TYPST_SEPARATOR_BASES.has(baseTrimmed))
        return "\"".concat(baseTrimmed, "\"");
...
var buildLimitBase = function (firstChild, baseTrimmed, base) {
    ...
    if (movablelimits === true) {
        ...
        if (isCustomOp(baseTrimmed) && (firstChild === null || firstChild === void 0 ? void 0 : firstChild.texClass) === MmlNode_1.TEXCLASS.OP) {
            if ((firstChild === null || firstChild === void 0 ? void 0 : firstChild.kind) === consts_1.TEX_ATOM) {
                return { typst: addLimitsParam(baseTrimmed), typst_inline: base };
            }
    ...
    if (atr.linethickness === '0' || atr.linethickness === 0) {
        ...
        if (hasFenceParent) {
            ...
            if (closeDelim === ')') {
                ...
                var delimTypst = openDelim ? "\"".concat(openDelim, "\"") : '#none';
                res = (0, common_1.addToTypstData)(res, { typst: "mat(delim: ".concat(delimTypst, ", ").concat(num, "; ").concat(den, ")") });
            }
        }
        else {
            res = (0, common_1.addToTypstData)(res, { typst: "mat(delim: #none, ".concat(num, "; ").concat(den, ")") });
        }
    }
    else {
...
var msup = function (node, serialize) {
    ...
    if (baseTrimmed && needsScriptsWrapper(baseTrimmed)) {
        ...
        res = (0, common_1.addToTypstData)(res, { typst: safeScriptBase(base, baseTrimmed) });
    }
    ...
    if (sup) {
        ...
        if (primeShorthand) {
            ...
            res = (0, common_1.addToTypstData)(res, { typst: safeFormatScript('^', sup) });
        }
    }
    return res;
};
...
var msub = function (node, serialize) {
    ...
    if (sub) {
        res = (0, common_1.addToTypstData)(res, { typst: safeFormatScript('_', sub) });
    }
    return res;
};
exports.msub = msub;
...
var mover = function (node, serialize) {
    ...
    if (node.attributes.get('accent') && secondChild && secondChild.kind === 'mo') {
        ...
        if (accentFn) {
            var content = (0, common_1.typstPlaceholder)((0, escape_utils_1.escapeContentSeparators)((0, escape_utils_1.escapeUnbalancedParens)(dataFirst.typst.trim())));
    ...
    if (over) {
        var longArrow = CONSTRUCTED_LONG_ARROWS.get("".concat(stripDashes(rawBase), "|").concat(stripDashes(over)));
        if (longArrow) {
            res = (0, common_1.addToTypstData)(res, { typst: longArrow });
            return res;
        }
    }
    if (over) {
        ...
        if (innerBase && (innerBase.kind === 'munder' || innerBase.kind === 'munderover')) {
            res = (0, common_1.addToTypstData)(res, dataFirst);
            res = (0, common_1.addToTypstData)(res, { typst: safeFormatScript('^', over) });
            return res;
        }
...
var munder = function (node, serialize) {
    ...
    if (node.attributes.get('accentunder') && secondChild && secondChild.kind === 'mo') {
        ...
        if (accentFn) {
            ...
            if (underSymbol) {
                res = (0, common_1.addToTypstData)(res, { typst: "limits(".concat(content, ")_").concat(underSymbol) });
                return res;
            }
            if (TYPST_ACCENT_SHORTHANDS.has(accentFn)) {
                res = (0, common_1.addToTypstData)(res, { typst: "".concat(accentFn, "(").concat(content, ")") });
    ...
    if (under) {
        ...
        var baseData = buildLimitBase(firstChild, rawBase, dataFirst.typst);
        res = (0, common_1.addToTypstData)(res, baseData);
        res = (0, common_1.addToTypstData)(res, { typst: safeFormatScript('_', under) });
    }
    else {
        res = (0, common_1.addToTypstData)(res, { typst: (0, common_1.typstPlaceholder)(rawBase) });
    }
...
var mmultiscripts = function (node, serialize) {
    ...
    if (!hasPrescripts) {
        ...
        if (lastPostSub) {
            res = (0, common_1.addToTypstData)(res, { typst: safeFormatScript('_', lastPostSub) });
        }
        if (lastPostSup) {
            res = (0, common_1.addToTypstData)(res, { typst: safeFormatScript('^', lastPostSup) });
        }
    }
    else {
