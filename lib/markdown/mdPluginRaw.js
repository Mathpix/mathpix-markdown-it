...
Object.defineProperty(exports, "findEndMarkerPos", { enumerable: true, get: function () { return common_1.findEndMarkerPos; } });
...
var convert_math_to_html_2 = require("./common/convert-math-to-html");
var common_3 = require("./highlight/common");
...
var list_source_model_1 = require("./md-latex-lists-env/list-source-model");
...
var multiMath = function (state, silent) {
    ...
    var acceptDoubled = ((_a = state.md.options.mathDelimiterMode) !== null && _a !== void 0 ? _a : 'strict') === 'legacy';
    if (match[0] === "\\[") {
        if (!acceptDoubled) {
            return false;
        }
        ...
        if ((_b = data === null || data === void 0 ? void 0 : data.arrClose) === null || _b === void 0 ? void 0 : _b.length) {
            endMarkerPos = state.pos + ((_c = data.arrClose[data.arrClose.length - 1]) === null || _c === void 0 ? void 0 : _c.posStart);
        }
        includeMarkers = true;
    }
    if (!endMarker) {
    ...
    if (!silent) {
        ...
        if ((_d = state.md.options.highlights) === null || _d === void 0 ? void 0 : _d.length) {
        ...
        if (!state.md.options.forLatex) {
            ...
            (0, convert_math_to_html_2.convertMathToHtml)(state, token, state.md.options);
        }
    }
    state.pos = nextPos;
    return true;
...
var simpleMath = function (state, silent) {
    ...
    var endMarkerPos = (0, common_1.findEndMarkerPos)(state.src, endMarker, startMathPos);
    if (endMarkerPos === -1) {
        return false;
    }
...
var renderReference = function (token, options, env, slf) {
    ...
    html += ((_c = token.highlights) === null || _c === void 0 ? void 0 : _c.length)
        ? (0, common_3.highlightText)(token, reference) : reference;
    html += '</a>';
...
function paragraphDiv(state, startLine /*, endLine*/) {
    ...
    var listOpen = false;
    var paragraphStart = state.bMarks[startLine] + state.tShift[startLine];
    ...
    var hasMidLineOpener = function (text) {
        var found = consts_1.BEGIN_LIST_ENV_INLINE_RE.exec(text);
        if (!found || found.index === 0) {
            return false;
        }
        // Masked only after a raw match, so the cost lands on the rare line: an opener in code opens nothing.
        var masked = consts_1.BEGIN_LIST_ENV_INLINE_RE.exec((0, list_source_model_1.maskNonStructure)(text));
        return !!masked && masked.index > 0;
    };
    var midLineOpenerSeen = hasMidLineOpener(lineText);
    var isMath = false;
    var mathEndTag = null;
    ...
    for (; nextLine < endLine; nextLine++) {
        ...
        if (midLineOpenerSeen) {
            ...
            var listDepth = (0, list_source_model_1.listDepthBetween)(state, paragraphStart, state.bMarks[nextLine]);
            listOpen = listDepth > 0
                && (0, list_source_model_1.closersLeftAfter)(state, state.bMarks[nextLine]) >= listDepth;
        }
        ...
        if (!isMath && !openedAuthorBlock && !isMathOpen
            ...
            var shouldBreak = (pickMathStartTag.test(lineText) && (0, common_2.checkTagOutsideInlineCode)(lineText, pickMathStartTag)) ||
                (pickMathEndTag.test(prewLineText) && (0, common_2.checkTagOutsideInlineCode)(prewLineText, pickMathEndTag));
            if (shouldBreak) {
                break;
            }
        }
...
exports.default = (function (options) {
    ...
    return function (md) {
        ...
        var hasInitHook = typeof md.core.ruler.__find__ === 'function'
            && md.core.ruler.__find__('init_math_cache') >= 0;
        if (hasInitHook) {
            md.core.ruler.at('init_math_cache', convert_math_to_html_1.initMathCache);
        }
        else {
            md.core.ruler.before('normalize', 'init_math_cache', convert_math_to_html_1.initMathCache);
        }
        var hasCleanupHook = typeof md.core.ruler.__find__ === 'function'
            && md.core.ruler.__find__('cleanup_math_cache') >= 0;
        if (hasCleanupHook) {
            md.core.ruler.at('cleanup_math_cache', convert_math_to_html_1.cleanupMathCache);
        }
        else {
            md.core.ruler.push('cleanup_math_cache', convert_math_to_html_1.cleanupMathCache);
        }
        ...
        md.block.ruler.before('html_block', 'svg_block', svg_block_1.svg_block, { alt: (0, common_2.getTerminatedRules)("svg_block") });
        if (options === null || options === void 0 ? void 0 : options.forPptx) {
            md.block.ruler.after("fence", 'image_with_size_block', image_block_1.imageWithSizeBlock, { alt: (0, common_2.getTerminatedRules)("image_with_size_block") });
        }
        ...
        if (md.options.previewUuid) {
            md.block.ruler.at("fence", mmd_fence_1.fenceBlock, { alt: (0, common_2.getTerminatedRules)("fence") });
        }
        md.block.ruler.before("ReNewCommand", "newTheoremBlock", block_rule_1.newTheoremBlock, { alt: (0, common_2.getTerminatedRules)("newTheoremBlock") });
        md.inline.ruler.before("escape", "usepackage", usepackage);
        md.block.ruler.before("html_block", "mathMLBlock", mathml_block_1.mathMLBlock, { alt: (0, common_2.getTerminatedRules)("mathMLBlock") });
