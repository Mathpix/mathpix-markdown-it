        ...
        Object.keys(mapping).forEach(function (key) {
            md.renderer.rules[key] = function (tokens, idx) {
                switch (tokens[idx].type) {
                    case "tabular":
                        return render_tabular_1.renderTabular(tokens, tokens[idx], options, md.renderer);
                    case "tabular_inline":
                        return render_tabular_1.renderTabularInline(tokens, tokens[idx], options, md.renderer);
                    case "tsv":
                        return render_tabular_1.renderTSV(tokens, tokens[idx], options);
                    case "reference_note":
