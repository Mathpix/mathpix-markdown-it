...
var LIST_STITCH_TOKEN_TYPES = consts_1.LIST_STRUCTURE_TOKEN_TYPES;
...
var RUN_BOUNDARY_TOKEN_TYPES = new Set(['paragraph_open', 'paragraph_close']);
// A cell token that reaches the leaf branch below, so it may join a leaf run.
var isLeafRunMember = function (token) {
    var _a;
    return !!token
        && !token.hidden
        ...
        && !((_a = token.children) === null || _a === void 0 ? void 0 : _a.length)
...
var ensureOwnAttrs = function (token) {
    var attrs = token.attrs;
    if (attrs && attrs[consts_1.attrsSharedMarker]) {
        token.attrs = attrs.map(function (pair) { return [pair[0], pair[1]]; });
    }
};
...
var computeOutputGates = function (options) {
    var outMath = (options === null || options === void 0 ? void 0 : options.outMath) || {};
    var forMD = !!(options === null || options === void 0 ? void 0 : options.forMD);
    return {
        ...
        needSmoothed: !!(options === null || options === void 0 ? void 0 : options.forPptx),
    };
};
/**
...
var renderNonTableTokenIntoCell = function (token, ctx, acc) {
    ...
    var _c = computeOutputGates(options), needHtml = _c.needHtml, needTsv = _c.needTsv, needCsv = _c.needCsv, needMd = _c.needMd, needSmoothed = _c.needSmoothed;
    ...
    var rendered = slf.renderInline([token], options, env);
    if (needHtml) {
        acc.result += rendered;
    }
    ...
    if ((needTsv || needCsv || needMd || needSmoothed)
        ...
        var run = [];
        for (var i = ctx.idx; i < ctx.tokens.length && isLeafRunMember(ctx.tokens[i]); i++) {
        ...
        var leaf = (0, render_table_cell_content_1.renderTableCellContent)({ children: run }, !!ctx.isSubTable, options, env, slf);
        if (needTsv) {
        ...
        if (needMd) {
            ...
            var prev = ctx.tokens[ctx.idx - 1];
            if (leaf.tableMd && acc.cellMd && !/[\s>]$/.test(acc.cellMd)
                && ((prev === null || prev === void 0 ? void 0 : prev.type) === 'inline' || (prev === null || prev === void 0 ? void 0 : prev.token) === 'inline')) {
                acc.cellMd += ' ';
            }
...
var handleListTokensForCellMarkdown = function (token, ctx, acc) {
    ...
    if ((token === null || token === void 0 ? void 0 : token.type) && consts_1.LIST_OPEN_TOKEN_TYPES.has(token.type)) {
        ...
        var prevIsListClose = !!prevType && consts_1.LIST_CLOSE_TOKEN_TYPES.has(prevType);
    ...
    if ((token === null || token === void 0 ? void 0 : token.type) === "latex_list_item_open") {
        ...
        var level = isEnumerate ? (_c = token.meta) === null || _c === void 0 ? void 0 : _c.enumerateLevel : (_d = token.meta) === null || _d === void 0 ? void 0 : _d.itemizeLevel;
        var listLevel = Math.max(1, level !== null && level !== void 0 ? level : 1);
        ...
        var markerLess = !!((_e = token.meta) === null || _e === void 0 ? void 0 : _e.markerEmpty) && !token.hasOwnProperty('marker');
        var markerMd = '';
        var markerTsv = markerLess ? '' : ' ';
        var markerCsv = markerLess ? '' : ' ';
        // If the token provides a custom marker, use it; otherwise default to bullet markers.
        if (token.hasOwnProperty('marker')) {
            if (((_f = token.markerTokens) === null || _f === void 0 ? void 0 : _f.length) > 0) {
                ...
                markerMd = (_g = markerRender.tableMd) !== null && _g !== void 0 ? _g : '';
                markerTsv += (_h = markerRender.tsv) !== null && _h !== void 0 ? _h : '';
                markerCsv += (_j = markerRender.csv) !== null && _j !== void 0 ? _j : '';
            }
            else {
                markerMd = (_k = token.marker) !== null && _k !== void 0 ? _k : '';
                markerTsv += (_l = token.marker) !== null && _l !== void 0 ? _l : '';
                markerCsv += (_m = token.marker) !== null && _m !== void 0 ? _m : '';
            }
        }
        else if (!((_o = token.meta) === null || _o === void 0 ? void 0 : _o.markerEmpty)) {
            // A marker-less item has no marker to export; inventing one numbered what the document did not.
            var plainMarker = isEnumerate
                ? (0, list_markers_1.getEnumeratePlainMarker)(Math.max(1, (_q = (_p = token.meta) === null || _p === void 0 ? void 0 : _p.enumerateIndex) !== null && _q !== void 0 ? _q : 1), listLevel)
    ...
    if ((token === null || token === void 0 ? void 0 : token.type) === "latex_list_item_close") {
        ...
        var shouldBreak = !prevType || !consts_1.LIST_CLOSE_TOKEN_TYPES.has(prevType);
        if (shouldBreak) {
            addBr();
        }
        return;
...
var renderInlineTokenBlock = function (tokens, options, env, slf, isSubTable, highlight) {
    ...
    var _d = computeOutputGates(options), needHtml = _d.needHtml, needTsv = _d.needTsv, needCsv = _d.needCsv, needMd = _d.needMd, needSmoothed = _d.needSmoothed;
    ...
    var numCol = 0;
    var ctx = { tokens: tokens, idx: 0, options: options, env: env, slf: slf, highlight: highlight, isSubTable: isSubTable };
    for (var idx = 0; idx < tokens.length; idx++) {
        ...
        if (token.token === 'td_close') {
            ...
            var tsvCell = needTsv ? formatTsvCell(cellTsvLines, isSubTable) : '';
            var csvCell = needCsv ? formatCsvCell(cellCsvLines) : '';
            var smoothedVal = needSmoothed ? cellSmoothed : '';
...
var renderTabularInline = function (a, token, options, env, slf) {
    ...
    var needHtml = !(options === null || options === void 0 ? void 0 : options.forMD) && include_table_html;
