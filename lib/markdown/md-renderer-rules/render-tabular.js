...
exports.renderInlineTokenBlock = function (tokens, options, env, slf, isSubTable, highlight) {
    ...
    for (var idx = 0; idx < tokens.length; idx++) {
        ...
        if (token.token === 'td_open') {
            ...
            if (highlight) {
                ...
                var dataAttrsStyle = common_1.getStyleFromHighlight(highlight);
...
exports.renderTabularInline = function (a, token, options, env, slf) {
    ...
    var data = exports.renderInlineTokenBlock(token.children, options, env, slf, token.isSubTable, highlight);
    token.tsv = data.tsv;
    token.csv = data.csv;
    ...
    var tsv = include_tsv && token.tsv
        ? "<tsv style=\"display: none\">" + parse_mmd_element_1.formatSource(tsv_1.TsvJoin(token.tsv, options), true) + "</tsv>"
        : '';
    var tableMd = include_table_markdown && token.tableMd
        ? "<table-markdown style=\"display: none\">" + parse_mmd_element_1.formatSource(table_markdown_1.tableMarkdownJoin(data.tableMd, data.align), true) + "</table-markdown>"
        : '';
    var csv = include_csv && token.csv
        ? "<csv style=\"display: none\">" + parse_mmd_element_1.formatSource(csv_1.CsvJoin(token.csv, options), true) + "</csv>"
        : '';
