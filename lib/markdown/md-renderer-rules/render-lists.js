...
exports.render_itemize_list_open = function (tokens, index, options, env, renderer) {
    ...
    dataPaddingInlineStart = dataPaddingInlineStart
        ...
        : '';
    if (options.forDocx) {
        var itemizeLevelTokens = re_level_1.GetItemizeLevelTokens(tokens[index].itemizeLevel);
        if (level_itemize > 0 && itemizeLevelTokens.length >= level_itemize) {
            var data = isTextMarkerTokens(itemizeLevelTokens[level_itemize - 1], options, renderer);
            var itemizeLevel = re_level_1.GetItemizeLevel(tokens[index].itemizeLevelContents);
...
exports.render_enumerate_list_open = function (tokens, index, options, env, renderer) {
    if (tokens[index].level === 0) {
        level_enumerate = 0;
    }
    level_enumerate++;
...
var generateHtmlForMarkerTokens = function (markerTokens, slf, options) {
    ...
    for (var j = 0; j < markerTokens.length; j++) {
        ...
        htmlMarker += slf.renderInline([markerTokens[j]], options);
    }
    return {
        htmlMarker: htmlMarker,
        markerType: markerType
    ...
    if (token.parentType === "enumerate") {
        ...
        if (token.hasOwnProperty('marker') && token.markerTokens) {
            ...
            if (level_itemize > 0 && itemizeLevelTokens.length >= level_itemize) {
                if (options.forDocx) {
                    // dataAttr += ' data-custom-marker="true"';
                    var data = generateHtmlForMarkerTokens(itemizeLevelTokens[level_itemize - 1], options, slf);
                    htmlMarker = data.htmlMarker;
