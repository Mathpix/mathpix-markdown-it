        ...
        if (!cellsAll[i] || cellsAll[i].length === 0) {
            if (i < cellsAll.length - 1) {
                ...
                for (var k = 0; k < numCol; k++) {
                    ...
                    res = res.concat(tabular_td_1.AddTd('', { h: cAlign[k], v: vAlign[k], w: cWidth[k] }, { left: cLeft, right: cRight, bottom: CellsHLines[i + 1][k], top: i === 0 ? CellsHLines[i][k] : '' }, CellsHLSpaces[i + 1][k]));
                }
        ...
        for (var j = 0; j < numCol; j++) {
            ...
            if (j >= (cells.length) && ic < numCol) {
                for (var k = ic; k < numCol; k++) {
                    ...
                    res = res.concat(tabular_td_1.AddTd('', { h: cAlign[k], v: vAlign[k], w: cWidth[k] }, { left: cLeft_1, right: cRight_1, bottom: CellsHLines[i + 1][k], top: i === 0 ? CellsHLines[i][k] : '' }, CellsHLSpaces[i + 1][k]));
                }
                break;
            }
            ...
            if (cells[j] && cells[j].trim().length > 0) {
                ...
                if (multi) {
                    ...
                    if (mr && mr > 0) {
                        ...
                        if (mr + i >= rows.length - 1) {
                            multi.attrs = tabular_td_1.addHLineIntoStyle(multi.attrs, CellsHLines[mr + i][ic]);
                        }
                    }
                    else {
                        if (mc && mc > 1) {
                    ...
                    if (i === 0) {
                        multi.attrs = tabular_td_1.addHLineIntoStyle(multi.attrs, CellsHLines[i][ic], 'top');
                    }
                    if (mr && mr > 0) {
                        multi.attrs = tabular_td_1.addHLineIntoStyle(multi.attrs, CellsHLines[mr + i][ic]);
                    }
                    else {
                        multi.attrs = tabular_td_1.addHLineIntoStyle(multi.attrs, CellsHLines[i + 1][ic]);
                    }
                ...
                if (parseSub && parseSub.length > 0) {
                    res = res.concat(tabular_td_1.AddTdSubTable(parseSub, { h: cAlign[ic], v: vAlign[ic], w: cWidth[ic] }, { left: cLeft, right: cRight, bottom: CellsHLines[i + 1][ic], top: i === 0 ? CellsHLines[i][ic] : '' }));
                    continue;
                }
                ...
                res = res.concat(tabular_td_1.AddTd(content, { h: cAlign[ic], v: vAlign[ic], w: cWidth[ic] }, { left: cLeft, right: cRight, bottom: CellsHLines[i + 1][ic], top: i === 0 ? CellsHLines[i][ic] : '' }, CellsHLSpaces[i + 1][ic], decimal[ic]));
            }
            else {
                MR[ic] = MR[ic] > 0 ? MR[ic] - 1 : 0;
                if (MR[ic] && MR[ic] > 0) {
                ...
                res = res.concat(tabular_td_1.AddTd('', { h: cAlign[ic], v: vAlign[ic], w: cWidth[ic] }, { left: cLeft, right: cRight, bottom: CellsHLines[i + 1][ic], top: i === 0 ? CellsHLines[i][ic] : '' }, CellsHLSpaces[i + 1][ic]));
            }
        }
        res.push({ token: 'tr_close', tag: 'tr', n: -1 });
    }
...
exports.ParseTabular = function (str, i, align) {
    ...
    if (posEnd > 0) {
        ...
        if (posBegin >= 0) {
            ...
            if (params) {
                align = params.align;
                var subT = str.slice(params.index, posEnd);
                var subRes = setTokensTabular(subT, params.align);
                str = sub_tabular_1.pushSubTabular(str, subRes, posBegin, posEnd, i);
                ...
                var subT = str.slice(posBegin + match.index + match[0].length, posEnd);
                var subRes = setTokensTabular(subT, match[1]);
                str = sub_tabular_1.pushSubTabular(str, subRes, posBegin + match.index, posEnd, i);
                res = exports.ParseTabular(str, 0, align);
            }
        }
        else {
