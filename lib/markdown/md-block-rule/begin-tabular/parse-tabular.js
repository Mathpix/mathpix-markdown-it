    ...
    for (var i = 0; i < rows.length; i++) {
        ...
        for (var j = 0; j < numCol; j++) {
            ...
            if (cells[j] && cells[j].trim().length > 0) {
                var multi = multi_column_row_1.getMultiColumnMultiRow(cells[j], { lLines: cLines[ic], align: cAlign[ic], rLines: cRight }, forLatex);
                if (multi) {
                ...
                var parseSub = sub_tabular_1.getSubTabular(cells[j], 0, true, forLatex);
