...
// `\begin{tabular}` with optional [pos] (group 1) and required {align} (group 2).
var RE_BEGIN_TABULAR_BRACKET = /\\begin{tabular}\s*(?:\[([^\]]*)\])?\s*\{([^}]*)\}/;
// Column spec letters that carry explicit vertical alignment.
var RE_EXPLICIT_V_COL_SPEC = /^[mpb]/;
/**
...
var setTokensTabular = function (str, align, options, isSubTabular, pos) {
    var _a;
    ...
    // Source bracket wins; otherwise fall back to the document-level option.
    var optionPos = (0, common_1.normalizeDefaultCellVerticalAlign)(options === null || options === void 0 ? void 0 : options.defaultCellVerticalAlign);
    var effectivePos = pos !== null && pos !== void 0 ? pos : optionPos;
    var dataAlign = (0, common_1.getVerticallyColumnAlign)(align, numCol, effectivePos);
    ...
    var _b = options.forLatex, forLatex = _b === void 0 ? false : _b, _c = options.outMath, outMath = _c === void 0 ? {} : _c;
    var skipVisual = !!(options === null || options === void 0 ? void 0 : options.forMD) || !!forLatex;
    // Source bracket wins; option only injects 't'/'b' to preserve round-trip.
    var latexPos = pos !== null && pos !== void 0 ? pos : (optionPos === 'c' ? undefined : optionPos);
    // Parent bracket attached to every td_open under forLatex.
    var tdMeta = forLatex && latexPos ? { parentBracket: latexPos } : undefined;
    ...
    var _loop_1 = function (i) {
        ...
        if (!cellsAll[i] || cellsAll[i].length === 0) {
            if (i < cellsAll.length - 1) {
                ...
                for (var k = 0; k < numCol; k++) {
                    ...
                    try {
                        for (var _e = (e_1 = void 0, tslib_1.__values(data_1.res)), _f = _e.next(); !_f.done; _f = _e.next()) {
                            var t = _f.value;
                            res.push(t);
                        }
                    }
        ...
        var _loop_2 = function (j) {
            var e_2, _g, e_3, _h, e_4, _j, e_5, _k, e_6, _l;
            ...
            if (j >= (cells.length) && ic < numCol) {
                for (var k = ic; k < numCol; k++) {
                    ...
                    try {
                        for (var _m = (e_2 = void 0, tslib_1.__values(data_2.res)), _o = _m.next(); !_o.done; _o = _m.next()) {
                            var t = _o.value;
                            res.push(t);
                        }
                    }
            ...
            if (cells[j] && cells[j].trim().length > 0) {
                var multi = (0, multi_column_row_1.getMultiColumnMultiRow)(cells[j], { lLines: cLines[ic], align: cAlign[ic], rLines: cRight, posDefault: effectivePos }, forLatex, options === null || options === void 0 ? void 0 : options.forPptx, skipVisual);
                if (multi) {
                    ...
                    if (multi.subTable) {
                        ...
                        try {
                            for (var _p = (e_3 = void 0, tslib_1.__values(multi.subTable)), _q = _p.next(); !_q.done; _q = _p.next()) {
                                var t = _q.value;
                                res.push(t);
                            }
                        }
                ...
                if (parseSub && parseSub.length > 0) {
                    // Nested \begin{tabular}[…] propagates to outer td; per-column m/p/b wins.
                    var cellV = vAlign[ic];
                    if (!RE_EXPLICIT_V_COL_SPEC.test(colSpec[ic] || '')) {
                        var nestedMatch = (((_a = parseSub[0]) === null || _a === void 0 ? void 0 : _a.content) || '').match(RE_BEGIN_TABULAR_BRACKET);
                        var cellPos = (0, common_1.parseTabularPos)(nestedMatch === null || nestedMatch === void 0 ? void 0 : nestedMatch[1]);
                        if (cellPos) {
                            cellV = cellPos === 't' ? 'top' : cellPos === 'b' ? 'bottom' : 'middle';
                        }
                    }
                    try {
                        for (var _r = (e_4 = void 0, tslib_1.__values(handleSubTable(parseSub, cellV))), _s = _r.next(); !_s.done; _s = _r.next()) {
                            var t = _s.value;
                            res.push(t);
                        }
                    }
                        ...
                        try {
                            if (_s && !_s.done && (_j = _r.return)) _j.call(_r);
                        }
                ...
                try {
                    for (var _t = (e_5 = void 0, tslib_1.__values(data_3.res)), _u = _t.next(); !_u.done; _u = _t.next()) {
                        var t = _u.value;
                        res.push(t);
                    }
                }
                    ...
                    try {
                        if (_u && !_u.done && (_k = _t.return)) _k.call(_t);
                    }
                ...
                try {
                    for (var _v = (e_6 = void 0, tslib_1.__values(data_4.res)), _w = _v.next(); !_w.done; _w = _v.next()) {
                        var t = _w.value;
                        res.push(t);
                    }
                }
                    ...
                    try {
                        if (_w && !_w.done && (_l = _v.return)) _l.call(_v);
                    }
    ...
    if (forLatex) {
        ...
        if (latexPos) {
            tableOpen.meta.bracket = latexPos;
        }
...
var ParseTabular = function (str, i, align, options, isSubTabular, pos) {
    if (align === void 0) { align = ''; }
    if (options === void 0) { options = {}; }
    if (isSubTabular === void 0) { isSubTabular = false; }
    var res = [];
    ...
    if (posEnd > 0) {
        ...
        if (posBegin >= 0) {
            ...
            if (params) {
                ...
                res = (0, exports.ParseTabular)(str, 0, align, options, isSubTabular, pos);
            }
            else {
                var match = str
                    .slice(posBegin)
                    .match(RE_BEGIN_TABULAR_BRACKET);
                var subT = str.slice(posBegin, posEnd + '\\end{tabular}'.length);
            ...
            var subRes = setTokensTabular(subT, align, options, false, pos);
            str = (0, sub_tabular_1.pushSubTabular)(str, subT, subRes, 0, posEnd);
            res = (0, exports.ParseTabular)(str, 0, align, options, isSubTabular, pos);
        }
    }
    else {
        res = setTokensTabular(str, align, options, isSubTabular, pos);
    }
    return res;
};
