...
exports.openTag = consts_1.BEGIN_TABULAR_BRACKET_RE;
exports.openTagG = consts_1.BEGIN_TABULAR_BRACKET_RE_G;
    ...
    if (match) {
        ...
        if (params) {
            ...
            if (mB) {
                ...
                res.push({ type: 'tabular', content: str.slice(mB.index + mB[0].length), align: mB[2], bracket: (0, common_1.parseTabularPos)(mB[1]) });
            }
            else {
...
var parseInlineTabular = function (str) {
    ...
    for (var i = 0; i < str.length; i++) {
        ...
        if (!matchB) {
            ...
            if (!matchE) {
                try {
                    for (var _d = (e_1 = void 0, tslib_1.__values(addContentToList(str.slice(posB, posB + matchB.index + matchB[0].length)))), _e = _d.next(); !_e.done; _e = _d.next()) {
                        var t = _e.value;
                        res.push(t);
                    }
                }
        ...
        if (posB + matchB.index > posE + matchE.index) {
            try {
                for (var _f = (e_2 = void 0, tslib_1.__values(addContentToList(str.slice(pos, pos + matchE.index)))), _g = _f.next(); !_g.done; _g = _f.next()) {
                    var t = _g.value;
                    res.push(t);
                }
            }
            ...
            if (exports.openTag.test(str.slice(posB, posE + matchE.index + matchE[0].length))) {
                ...
                try {
                    for (var _h = (e_3 = void 0, tslib_1.__values(addContentToList(str.slice(pos, posE + matchE.index)))), _j = _h.next(); !_j.done; _j = _h.next()) {
                        var t = _j.value;
                        res.push(t);
                    }
                }
...
var StatePushTabulars = function (state, cTabular, align, startLine) {
    ...
    for (var i = 0; i < cTabular.length; i++) {
        ...
        token.map = [startLine, state.line];
        token.bMarks = 0;
        var res = (0, parse_tabular_1.ParseTabular)(cTabular[i].content, 0, cTabular[i].align, state.md.options, state.env.subTabular, cTabular[i].bracket);
        if (!res || res.length === 0) {
            continue;
        }
        var envSubTabular = !!state.env.subTabular;
        var envIsInline = !!((_a = state.env) === null || _a === void 0 ? void 0 : _a.isInline);
        var sharedEnvToInline = null;
        for (var j = 0; j < res.length; j++) {
            ...
            if (res[j].token === 'inline') {
                ...
                if (res[j].content) {
                    ...
                    if ((0, common_1.detectLocalBlock)(res[j].content)) {
                        state.env.isInline = true;
                        (0, helper_1.parseBlockIntoTokenChildren)(state, res[j].content, token, {
                            disableBlockRules: true,
                        });
                        state.env.isInline = envIsInline;
                        continue;
                    }
                    var isSubTab = res[j].type === 'subTabular' || res[j].isSubTabular;
                    if (isSubTab) {
                        tok.envToInline = (0, env_transient_1.snapshotEnvForInline)(state.env);
                    }
                    else {
                        if (!sharedEnvToInline)
                            sharedEnvToInline = (0, env_transient_1.snapshotEnvForInline)(state.env);
                ...
                var t = res[j].token;
                var isMarker = t === 'td_open' || t === 'td_close'
...
var TABULAR_END_POS_KEY = Symbol('mmd.tabularEndPos');
...
var CLOSE_TAG_SWEEP_G = new RegExp(exports.closeTag.source, 'g');
var lastTabularEndPos = function (state) {
    var slot = state;
    var cached = slot[TABULAR_END_POS_KEY];
    ...
    CLOSE_TAG_SWEEP_G.lastIndex = 0;
    slot[TABULAR_END_POS_KEY] = { src: state.src, lastPos: lastPos };
    return lastPos;
};
var BeginTabular = function (state, startLine, endLine, silent) {
    ...
    if (!consts_1.openTagTabular.test(lineText)) {
        return false;
    }
    ...
    for (; nextLine <= endLine; nextLine++) {
        ...
        var mb = lineText.match(consts_1.BEGIN_LST_RE);
        if (mb) {
            envDepth++;
        }
        else {
            var me = lineText.match(consts_1.END_LST_RE);
            if (me && envDepth > 0)
                envDepth--;
        }
