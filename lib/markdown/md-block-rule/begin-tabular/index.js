...
exports.openTag = /(?:\\begin\s{0,}{tabular}\s{0,}(?:\[[^\]]*\])?\s{0,}\{([^}]*)\})/;
exports.openTagG = /(?:\\begin\s{0,}{tabular}\s{0,}(?:\[[^\]]*\])?\s{0,}\{([^}]*)\})/g;
...
var StatePushTabulars = function (state, cTabular, align, startLine) {
    ...
    for (var i = 0; i < cTabular.length; i++) {
        ...
        token.map = [startLine, state.line];
        token.bMarks = 0;
        var res = (0, parse_tabular_1.ParseTabular)(cTabular[i].content, 0, cTabular[i].align, state.md.options, state.env.subTabular, cTabular[i].pos);
        if (!res || res.length === 0) {
            continue;
        }
