    ...
    if (items && items.length > 0) {
        if (items && items.length > 0) {
            items.forEach(function (item) {
                helper_1.SetTokensBlockParse(state, item.content.trim(), item.startLine, item.endLine + 1);
            });
        }
    }
};
            ...
            items.forEach(function (item) {
                ...
                if (blockStartTag.test(item.content) || (item.content.indexOf('`') > -1) //&& item.content.charCodeAt(0) === 0x5c /* \ */
                    ...
                    if (match) {
                        ...
                        helper_1.SetTokensBlockParse(state, item.content.slice(match.index + match[0].length + 1).trim());
...
exports.ReRenderListsItem = function (state, startLine, endLine, silent) {
    ...
    var dataMarkers = re_level_1.GetItemizeLevelTokensByState(state);
    ...
    if (match) {
        ...
        helper_1.SetTokensBlockParse(state, content.slice(match.index + match[0].length + 1).trim());
