...
var StatePushPatagraphOpenTable = function (state, startLine, nextLine, type, latex, hasAlignTagG, placement) {
    var _a, _b;
...
var BeginTable = function (state, startLine, endLine, silent) {
    ...
    var latex = match[1] === 'figure' || match[1] === 'table'
        ? "\\begin{".concat(match[1], "}[h]")
        : match[0];
    StatePushPatagraphOpenTable(state, startLine, (pE > 0) ? nextLine : nextLine + 1, type, latex, (hasAlignTagG || hasAlignTagIncludeGraphicsG), match[2]);
    if (captionFirst && !state.md.options.forLatex) {
        StatePushCaptionTable(state, type);
    }
    if (pB > 0) {
