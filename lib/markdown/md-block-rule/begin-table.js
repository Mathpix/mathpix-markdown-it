...
var StatePushCaptionTable = function (state, type) {
    ...
    if (!caption)
        return;
    var token;
    var num = type === TBegin.table ? (0, caption_counters_1.currentTableNumber)() : (0, caption_counters_1.currentFigureNumber)();
...
var StatePushPatagraphOpenTable = function (state, startLine, nextLine, type, latex, hasAlignTagG) {
    ...
    if (!caption) {
        ...
        if (type === TBegin.table) {
            currentNumber = (0, caption_counters_1.nextTableNumber)();
        }
        else {
            currentNumber = (0, caption_counters_1.nextFigureNumber)();
        }
        token.attrJoin("class", "table");
        token.attrJoin("number", currentNumber.toString());
    }
    if ((_a = state.md.options) === null || _a === void 0 ? void 0 : _a.forPptx) {
...
var BeginTable = function (state, startLine, endLine, silent) {
    ...
    var envDepth = 0; // >0 — inside a lstlisting; those lines are kept raw so code indentation survives
    var startTabular = 0;
    var match = lineText.match(consts_1.RE_BEGIN_TABLE_OR_FIGURE_WITH_PLACEMENT);
    if (!match) {
        match = lineText.match(consts_1.RE_BEGIN_FIGURE_OR_TABLE_ENV);
    ...
    for (; nextLine < endLine; nextLine++) {
        ...
        if (!matchCaption) {
            ...
            if (matchCaptionB) {
                var _c = (0, common_1.findEndMarker)(lineText, matchCaptionB.index + matchCaptionB[0].length - 1), _d = _c.res, res = _d === void 0 ? false : _d, _e = _c.nextPos, nextPos = _e === void 0 ? 0 : _e;
                if (res) {
        ...
        if (matchNewBegin && ((_b = matchNewBegin[1]) === null || _b === void 0 ? void 0 : _b.trim()) === type) {
            break;
        }
        (_a = (0, helper_1.appendEnvAwareContentLine)(state, nextLine, lineText, resText, envDepth), resText = _a.resText, envDepth = _a.envDepth);
