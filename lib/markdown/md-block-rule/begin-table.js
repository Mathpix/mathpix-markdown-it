...
exports.BeginTable = exports.ClearFigureNumbers = exports.ClearTableNumbers = void 0;
...
var StatePushCaptionTable = function (state, type) {
    ...
    var isLabelFormatEmpty = state.env.captionIsLabelFormatEmpty;
    if (!caption)
        return;
    var token;
    ...
    if (isLabelFormatEmpty) {
        ...
        var capitalizedType = "".concat(type[0].toUpperCase()).concat(type.slice(1));
        token.print = ((_b = (_a = state.md) === null || _a === void 0 ? void 0 : _a.options) === null || _b === void 0 ? void 0 : _b.nonumbers)
            ? "".concat(capitalizedType, ": ")
            : "".concat(capitalizedType, " ").concat(num, ": ");
    }
...
var StatePushParagraphOpenTable = function (state, startLine, nextLine, type, latex, hasAlignTagG, placement) {
    var _a, _b;
    ...
    if (state.md.options.forLatex) {
        token.latex = latex;
        var meta = tslib_1.__assign(tslib_1.__assign({}, ((_a = token.meta) !== null && _a !== void 0 ? _a : {})), { type: type });
    ...
    if ((_b = state.md.options) === null || _b === void 0 ? void 0 : _b.forPptx) {
        if (type === TBegin.table || type === TBegin.figure) {
            token.attrJoin("class", "latex-".concat(type));
        }
    }
...
var StatePushTableContent = function (state, startLine, nextLine, content, align, type) {
    if (consts_1.RE_BEGIN_ALIGN_ENV.test(content)) {
        var matchT = content.match(begin_tabular_1.openTag);
        var matchA = content.match(consts_1.RE_BEGIN_ALIGN_ENV);
...
var InlineBlockBeginTable = function (state, startLine) {
    ...
    var token;
    var match = lineText.match(consts_1.RE_BEGIN_TABLE_OR_FIGURE_WITH_PLACEMENT);
    if (!match) {
        match = lineText.match(consts_1.RE_BEGIN_FIGURE_OR_TABLE_ENV);
    }
    if (!match) {
        return false;
    }
    var trimmed = match[1].trim();
    var type = (trimmed in TBegin ? trimmed : null);
    if (!type) {
        return false;
    }
    ...
    var hasAlignTagG = consts_1.RE_ALIGN_CENTERING_GLOBAL.test(content);
    var hasAlignTagIncludeGraphicsG = type === TBegin.figure
        ? Boolean(content.match(consts_1.RE_INCLUDEGRAPHICS_WITH_ALIGNMENT_GLOBAL)) : false;
    content = content.replace(consts_1.RE_ALIGN_CENTERING_GLOBAL, '');
    var captionPos = {};
    if (!state.md.options.forLatex) {
        matchE = content.match(consts_1.RE_CAPTION_TAG);
        if (matchE) {
            caption = matchE[1];
            ...
            content = content.replace(consts_1.RE_CAPTION_TAG_GLOBAL, '');
        }
        var contentSetupData = (0, common_1.removeCaptionsSetupFromTableAndFigure)(content);
    ...
    state.env.alignEnvBlock = (0, begin_align_1.getAlignByAlignEnvBlock)(content);
    var latex = match[1] === 'figure' || match[1] === 'table'
        ? "\\begin{".concat(match[1], "}[h]")
        : match[0];
    StatePushParagraphOpenTable(state, startLine, startLine + 1, type, latex, hasAlignTagG || hasAlignTagIncludeGraphicsG, (0, consts_1.toFigureTablePlacement)(match[2]));
...
var BeginTable = function (state, startLine, endLine, silent) {
    ...
    var captionPos = {};
    var matchCaption = lineText.match(consts_1.RE_CAPTION_TAG);
    if (matchCaption) {
    ...
    for (; nextLine < endLine; nextLine++) {
        ...
        if (!matchCaption) {
            matchCaption = lineText.match(consts_1.RE_CAPTION_TAG);
            var matchCaptionB = lineText.match(consts_1.RE_CAPTION_TAG_BEGIN);
            if (matchCaptionB) {
                var _b = (0, common_1.findEndMarker)(lineText, matchCaptionB.index + matchCaptionB[0].length - 1), _c = _b.res, res = _c === void 0 ? false : _c, _d = _b.nextPos, nextPos = _d === void 0 ? 0 : _d;
                if (res) {
        ...
        var matchNewBegin = lineText.match(consts_1.RE_BEGIN_TABLE_OR_FIGURE_WITH_PLACEMENT)
            || lineText.match(consts_1.RE_BEGIN_FIGURE_OR_TABLE_ENV);
        if (matchNewBegin && ((_a = matchNewBegin[1]) === null || _a === void 0 ? void 0 : _a.trim()) === type) {
            break;
        }
        if (resText && lineText) {
            resText += '\n' + lineText;
    ...
    var matchAlignTagG = resText.match(consts_1.RE_ALIGN_CENTERING_GLOBAL);
    var hasAlignTagG = Boolean(matchAlignTagG);
    content = state.md.options.forLatex
        ? resText
        : resText.replace(consts_1.RE_ALIGN_CENTERING_GLOBAL, '');
    var hasAlignTagIncludeGraphicsG = type === TBegin.figure
        ? Boolean(content.match(consts_1.RE_INCLUDEGRAPHICS_WITH_ALIGNMENT_GLOBAL)) : false;
    if (!state.md.options.forLatex) {
        matchE = content.match(consts_1.RE_CAPTION_TAG);
        ...
        if (matchCaptionB) {
            ...
            if (data.res) {
                ...
                while (consts_1.RE_CAPTION_TAG_BEGIN.test(content)) {
    ...
    var latex = match[1] === 'figure' || match[1] === 'table'
        ? "\\begin{".concat(match[1], "}[h]")
        : match[0];
    StatePushParagraphOpenTable(state, startLine, (pE > 0) ? nextLine : nextLine + 1, type, latex, (hasAlignTagG || hasAlignTagIncludeGraphicsG), (0, consts_1.toFigureTablePlacement)(match[2]));
    if (captionFirst && !state.md.options.forLatex) {
        StatePushCaptionTable(state, type);
    }
    if (pB > 0) {
