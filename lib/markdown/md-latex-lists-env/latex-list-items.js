...
var ListItems = function (state, items, itemizeLevelTokens, enumerateLevelTypes, li, iOpen, itemizeLevelContents) {
    ...
    try {
        for (var items_2 = tslib_1.__values(items), items_2_1 = items_2.next(); !items_2_1.done; items_2_1 = items_2.next()) {
            ...
            if (consts_2.LATEX_BLOCK_ENV_OPEN_RE.test(listItem.content) || listItem.content.indexOf('`') > -1 || listItem.content.indexOf('~~~') > -1) {
                var match = listItem.content.match(consts_2.LATEX_ITEM_COMMAND_RE);
                if (match) {
...
var ItemsAddToPrev = function (items, lineText, nextLine) {
    ...
    if (!consts_2.END_LIST_ENV_INLINE_RE.test(lineText)) {
        (0, exports.ItemsListPush)(items, lineText, nextLine, nextLine);
    }
    return items;
};
exports.ItemsAddToPrev = ItemsAddToPrev;
var finalizeListItems = function (state, items, itemizeLevelTokens, enumerateLevelTypes, li, iOpen, itemizeLevelContents, tokenStart, ancestorIndentEm) {
    if (ancestorIndentEm === void 0) { ancestorIndentEm = 0; }
    var dataItems = (0, exports.ListItems)(state, items, itemizeLevelTokens, enumerateLevelTypes, li, iOpen, itemizeLevelContents);
    if (tokenStart) {
        var listToken = tokenStart;
        if (!listToken.padding || listToken.padding < dataItems.padding) {
            ...
            var need = dataItems.padding;
            if (need > ancestorIndentEm + consts_1.LIST_DEFAULT_INDENT_EM) {
                var em = Math.round(Math.min(need - ancestorIndentEm, consts_1.LIST_MAX_INDENT_EM) * 100) / 100;
                listToken.indentEm = em;
                listToken.attrSet("data-padding-inline-start", String(em) + "em");
            }
            else {
                listToken.indentEm = consts_1.LIST_DEFAULT_INDENT_EM;
            }
        }
    }
    return {
