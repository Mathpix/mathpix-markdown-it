...
var closersLeftAfter = function (state, at) {
    return structuralCountIn(state, listCloserOffsets(state), CLOSER_SUFFIX_KEY, at, Infinity)
...
var openOpaqueEnv = function (stack, items, openedType, afterBegin, nextLine, state) {
    ...
    var meSameLine = firstUsableCloser(state, nextLine, afterBegin, openedType, false);
    if (!meSameLine) {
...
var handleLstBeginInline = function (lineText, stack, items, nextLine, dStart, itemTag, state) {
    ...
    var mbWrap = mbWrapRaw && hasCloserAhead(state, absoluteOffsetOf(state, nextLine, lineText, mbWrapRaw.index, mbWrapRaw[0]), mbWrapRaw[1])
        ? mbWrapRaw
        : null;
...
var handleLstEndInline = function (lineText, stack, items, nextLine, state) {
    ...
    var me = firstUsableCloser(state, nextLine, lineText, top, true);
    if (!me) {
                ...
                if (iOpen <= 0) {
                    ...
                    if (tailEnv && !tailEnv.isEnd) {
                        ...
                        if (needed <= 0) {
                            ...
                            var ahead = structuralCountIn(state, listCloserOffsets(state), CLOSER_SUFFIX_KEY, state.eMarks[lineIdx], Infinity);
