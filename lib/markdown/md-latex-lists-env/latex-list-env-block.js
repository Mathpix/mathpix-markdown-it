...
var leadingSpaceOf = function (text) { return LEADING_SPACE_RE.exec(text)[0].length; };
    ...
    var processLine = function (lineText, lineIdx) {
        ...
        if (consts_1.RENEWCOMMAND_LINE_RE.test(lineText)) {
            ...
            if (spanEnd > 0 && rest.trim()
                && (!!(0, list_source_model_1.nextListEnvMatch)(rest) || consts_1.LATEX_ITEM_COMMAND_INLINE_RE.test(rest))) {
        ...
        var tailAt = state.eMarks[lineIdx] - tail.length;
            ...
            if (isEnd) {
                ...
                if (iOpen <= 0) {
                    ...
                    if (!siblingClosable) {
                        ...
                        var tailFrom = tailAt + cut + leadingSpaceOf(tail.slice(cut));
                        ...
                        var insideLine = tailFrom > tailAt && tailFrom < state.eMarks[lineIdx];
                        if (insideLine && sE.length > 0 && lineIdx > startLine
                            && !(0, list_source_model_1.nextListEnvMatch)((0, list_source_model_1.maskNonStructure)(sE))) {
...
var Lists = function (state, startLine, endLine, silent) {
    ...
    var inParagraph = state.listParagraphStart;
    if (silent && inParagraph !== undefined
        && (0, list_source_model_1.listDepthBetween)(state, inParagraph, state.bMarks[startLine]) > 0) {
        return false;
    }
    ...
    try {
        ...
        var tailFrom = bufferedState.listTailFrom;
        ...
        if (tailFrom) {
            ...
            if (marks) {
                ...
                var pending = [saved];
                state.listTailMarks = pending;
                try {
                    state.md.block.tokenize(state, tailFrom.line, endLine);
                }
                finally {
                    state.listTailMarks = undefined;
