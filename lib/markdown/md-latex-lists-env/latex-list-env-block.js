...
var pairArgumentSpans = function (text, verbatim) {
    var spans = [];
    var codePositions = (0, common_1.buildInlineCodePositionSet)((0, common_1.getInlineCodeListFromString)(text));
    var range = 0;
    for (var i = 0; i < text.length; i++) {
        ...
        var found = (0, common_1.findEndMarker)(text, i, "{", "}", false, 0, codePositions);
        if (!found.res) {
            continue;
        }
...
var argumentSpansOf = function (state) {
    return (0, src_pos_cache_1.srcValueCached)(state, ARG_SPANS_KEY, function (src) { return pairArgumentSpans(src, verbatimRangesOf(state)); });
};
...
var hasCloserAhead = function (state, from, name) {
    ...
    if (closesOurListWithin(state, from, at)) {
        ...
        if (opensInside > 0 || closersLeftAfter(state, at) < Math.max(1, (0, list_state_1.snapshotListLevels)())) {
            return false;
        }
    }
