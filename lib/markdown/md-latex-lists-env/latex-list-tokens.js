...
exports.processListChildToken = exports.setTokenCloseList = exports.ListOpen = exports.setTokenOpenList = exports.setTokenListItemOpenBlock = exports.wrapListLevelRuns = exports.wrapLooseRun = exports.absorbSublistIntoWrapper = exports.listHostFlags = exports.LIST_CLOSE_TYPES = exports.LIST_OPEN_TYPES = exports.computeMarkerPadding = void 0;
...
var LIST_CONFIG_TOKEN_TYPES = new Set(['setcounter', 'renewcommand']);
...
var absorbSublistIntoWrapper = function (tokens, from) {
    ...
    for (var i_1 = start; i_1 < tokens.length && !hasWrapper; i_1++) {
        hasWrapper = tokens[i_1].type === 'latex_list_item_open' && !!((_a = tokens[i_1].meta) === null || _a === void 0 ? void 0 : _a.markerEmpty);
    }
    if (!hasWrapper) {
        return;
    }
    var _d = pairListTokens(tokens), openerOf = _d.openerOf, closeAt = _d.closeAt;
    ...
    while (i < tokens.length) {
        ...
        if (i >= start && exports.LIST_OPEN_TYPES.has(tokens[i].type) && closeAt[i] >= 0
            && (tail === null || tail === void 0 ? void 0 : tail.type) === 'latex_list_item_close' && ((_c = (_b = openerOf.get(tail)) === null || _b === void 0 ? void 0 : _b.meta) === null || _c === void 0 ? void 0 : _c.markerEmpty)) {
            out.pop();
...
var wrapLooseRun = function (state, from) {
    ...
    if (!run.some(function (t) { return !LIST_CONFIG_TOKEN_TYPES.has(t.type)
    ...
    if (typeof state.Token !== 'function') {
        (0, warn_distinct_1.warnDistinct)('loose-run-no-token', '[list] no Token constructor on the state, run left unwrapped');
        return;
    }
...
var wrapListLevelRuns = function (state, from) {
    var starts = [];
    for (var i = from; i < state.tokens.length; i++) {
        var type = state.tokens[i].type;
        if (exports.LIST_OPEN_TYPES.has(type) || type === 'latex_list_item_close') {
            starts.push(i + 1);
        }
    }
    for (var i = starts.length - 1; i >= 0; i--) {
        (0, exports.wrapLooseRun)(state, starts[i]);
    }
};
exports.wrapListLevelRuns = wrapListLevelRuns;
/**
...
var ListOpen = function (state, startLine, lineText, itemizeLevelTokens, enumerateLevelTypes, itemizeLevelContents, openTokens, allListTokens) {
    ...
    if (strAfter && strAfter.trim().length > 0) {
        ...
        var outerSrc = (0, list_state_1.setListInlineSrc)(state.env, strAfter);
        try {
            state.md.inline.parse(strAfter, state.md, state.env, children);
        }
        finally {
            (0, list_state_1.setListInlineSrc)(state.env, outerSrc);
        }
        ...
        (0, exports.wrapListLevelRuns)(state, looseFrom);
