...
var BEGIN_LIST_ENV_AT = new RegExp(consts_1.BEGIN_LIST_ENV_RE.source.replace(/^\^/, ''), 'y');
var END_LIST_ENV_AT = new RegExp(consts_1.END_LIST_ENV_RE.source.replace(/^\^/, ''), 'y');
var BEGIN_LST_AT = new RegExp(consts_1.BEGIN_LST_INLINE_RE.source, 'y');
var BEGIN_TABULAR_AT = new RegExp(consts_1.BEGIN_TABULAR_INLINE_RE.source, 'y');
var END_LST_SEARCH_G = new RegExp(consts_1.END_LST_INLINE_RE.source, 'g');
var END_TABULAR_SEARCH_G = new RegExp(consts_1.END_TABULAR_INLINE_RE.source, 'g');
...
var RENEWCOMMAND_SPANS = Symbol('renewcommand-spans');
// One index per source too: the memo spares a repeat at the same offset, not the next offset.
var RENEWCOMMAND_CODE_POSITIONS = Symbol('renewcommand-code-positions');
/** Where the command at `cmdAt` ends, absolute, or -1. */
var renewCommandSpanAt = function (state, cmdAt) {
    var cached = state;
    var spans = (0, src_pos_cache_1.srcValueCached)(cached, RENEWCOMMAND_SPANS, function () { return new Map(); });
    var known = spans.get(cmdAt);
    if (known !== undefined) {
        return known;
    }
    var codeIndex = (0, src_pos_cache_1.srcValueCached)(cached, RENEWCOMMAND_CODE_POSITIONS, function (src) { return (0, common_1.buildInlineCodePositionSet)((0, common_1.getInlineCodeListFromString)(src)); });
    var span = (0, common_1.renewCommandSpanEnd)(state.src, cmdAt, codeIndex);
...
var insideRenewCommand = function (state, at) {
    ...
    var spanEnd = renewCommandSpanAt(state, cmdAt);
...
var findFirstCompleteListEnv = function (src, startPos) {
    ...
    var codePositions = function () {
        if (!codeSpans) {
            codeSpans = (0, common_1.buildInlineCodePositionSet)((0, common_1.getInlineCodeListFromString)(src));
        }
        return codeSpans;
    };
    ...
    while (pos < src.length) {
        ...
        if (opaqueStack.length === 0 && src.startsWith('\\renewcommand', pos)) {
            var macroEnd = (0, common_1.renewCommandSpanEnd)(src, pos, codePositions());
        ...
        if (opaqueStack.length > 0) {
            ...
            var endRe = top_1 === "lstlisting" ? END_LST_SEARCH_G : END_TABULAR_SEARCH_G;
            endRe.lastIndex = pos;
            var me = endRe.exec(src);
            if (!me) {
        ...
        var mbList = BEGIN_LIST_ENV_AT.exec(src);
        if (mbList) {
        ...
        var meList = END_LIST_ENV_AT.exec(src);
        if (meList) {
        ...
        var mbLst0 = BEGIN_LST_AT.exec(src);
        var mbTab0 = BEGIN_TABULAR_AT.exec(src);
        if (mbLst0 || mbTab0) {
...
var listCloseInline = function (state, silent) {
    ...
    if (!((_a = state.env) === null || _a === void 0 ? void 0 : _a.isBlock) || (0, list_state_1.isParsingMarker)() || !(0, list_state_1.isListOwnParse)(state)) {
        return false;
    }
    ...
    if (!silent) {
        ...
        var openType = (_c = (_b = state.types) === null || _b === void 0 ? void 0 : _b[state.types.length - 1]) !== null && _c !== void 0 ? _c : (_d = state.env) === null || _d === void 0 ? void 0 : _d.parentType;
        ...
        var _f = (0, latex_list_common_1.getListTokenTypes)(listType), closeType = _f.closeType, htmlTag = _f.htmlTag;
...
var listBeginInline = function (state, silent) {
    ...
    if (!silent) {
        ...
        var _c = (0, latex_list_common_1.getListTokenTypes)(listType), openType = _c.openType, htmlTag = _c.htmlTag;
    ...
    state.pos = startPos + ((_b = match.index) !== null && _b !== void 0 ? _b : 0) + match[0].length;
    return true;
};
...
var listSetCounterInline = function (state, silent) {
    ...
    if (!silent) {
        ...
        var startNumber = (_b = (0, latex_list_common_1.parseSetCounterNumber)(match)) !== null && _b !== void 0 ? _b : 1;
        var content = startNumber.toString();
        var token = state.push("setcounter", "", 0);
        token.content = content;
        if ((_d = (_c = state.md) === null || _c === void 0 ? void 0 : _c.options) === null || _d === void 0 ? void 0 : _d.forLatex) {
            var absoluteEnd = startPos + ((_e = match.index) !== null && _e !== void 0 ? _e : 0) + match[0].length;
            token.latex = state.src.slice(state.pos, absoluteEnd);
        }
    }
    // Advance parser position to just after the \setcounter call
    state.pos = startPos + ((_f = match.index) !== null && _f !== void 0 ? _f : 0) + match[0].length;
    return true;
};
