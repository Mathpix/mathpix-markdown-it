...
var LIST_BEGIN_POS_KEY = Symbol('mmd.footnoteListBeginPos');
var LIST_BEGIN_SWEEP_G = new RegExp(consts_1.BEGIN_LIST_ENV_INLINE_RE.source, 'g');
...
var anyTerminates = function (rules, state, line, endLine) {
...
var RESOLVED_RULES_KEY = Symbol('mmd.resolvedTerminators');
var resolveEnabledRuleFns = function (ruler, names) {
    var token = ruler.__cache__;
    var host = ruler;
    var byNames = host[RESOLVED_RULES_KEY];
    if (!byNames) {
        byNames = new Map();
        host[RESOLVED_RULES_KEY] = byNames;
    }
    var cached = byNames.get(names);
...
var terminatorsWithFence = function (ruler, names, fenceRule) {
    ...
    var withFence = [fenceRule].concat(resolved);
    byNames.set(names, { resolved: resolved, withFence: withFence });
    return withFence;
};
var latex_footnote_block = function (state, startLine, endLine, silent) {
    ...
    try {
        ...
        if (!sawFootnoteToken || !consts_1.reOpenTagFootnoteG.test(lineText)) {
            ...
            var probeRules = (0, src_pos_cache_1.lastMatchPosCached)(state, LIST_BEGIN_POS_KEY, LIST_BEGIN_SWEEP_G) >= state.bMarks[startLine]
                ? terminatorsWithFence(state.md.block.ruler, LIST_TERMINATOR_NAME, fence)
                : FENCE_ONLY;
...
var latex_footnotetext_block = function (state, startLine, endLine, silent) {
    ...
    try {
        ...
        var pending = '';
        var terminate = false;
        var terminatorRules = resolveEnabledRuleFns(state.md.block.ruler, FOOTNOTE_TERMINATOR_NAMES);
        // Literal token can't span `\n` — gate the O(fullContent) regex on per-line presence.
