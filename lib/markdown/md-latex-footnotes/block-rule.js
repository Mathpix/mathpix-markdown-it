...
// Per-state cache of `patternG` match positions in `state.src`. `patternG` MUST be a /g regex — without /g, `exec` ignores `lastIndex` and the loop below would never advance. Caller's contract: pass one of the module-local `*_SWEEP_G` constants. Cache entry stores `state.src` alongside positions so a reused StateBlock with a swapped src cannot return stale offsets.
...
var latex_footnote_block = function (state, startLine, endLine, silent) {
    ...
    try {
        ...
        var positions = getCachedSrcPositions(state, FOOTNOTE_POS_KEY, FOOTNOTE_TOKEN_SWEEP_G);
...
var latex_footnotetext_block = function (state, startLine, endLine, silent) {
    ...
    try {
        ...
        var positions = getCachedSrcPositions(state, FOOTNOTETEXT_POS_KEY, FOOTNOTETEXT_TOKEN_SWEEP_G);
        if (positions.length === 0 || positions[positions.length - 1] < state.bMarks[startLine]) {
            return false;
        }
        var nextLine = startLine + 1;
