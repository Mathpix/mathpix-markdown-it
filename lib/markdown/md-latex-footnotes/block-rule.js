...
var FOOTNOTE_NEEDLES = ['\\footnote'];
var FOOTNOTETEXT_NEEDLES = ['\\footnotetext', '\\blfootnotetext'];
...
var latex_footnote_block = function (state, startLine, endLine, silent) {
    ...
    try {
        ...
        var positions = (0, utils_1.getCachedSrcPositions)(state, FOOTNOTE_POS_KEY, FOOTNOTE_NEEDLES);
        ...
        var sawFootnoteSubstr = lineText.indexOf('\\footnote') !== -1;
        if (!sawFootnoteSubstr || !consts_1.reOpenTagFootnoteG.test(lineText)) {
...
var latex_footnotetext_block = function (state, startLine, endLine, silent) {
    ...
    try {
        ...
        var positions = (0, utils_1.getCachedSrcPositions)(state, FOOTNOTETEXT_POS_KEY, FOOTNOTETEXT_NEEDLES);
        if (positions.length === 0 || positions[positions.length - 1] < state.bMarks[startLine]) {
            return false;
        }
        var nextLine = startLine + 1;
        ...
        var sawFootnotetextSubstr = lineText.indexOf('\\footnotetext') !== -1
            || lineText.indexOf('\\blfootnotetext') !== -1;
        if (!sawFootnotetextSubstr || !consts_1.reOpenTagFootnotetextG.test(lineText)) {
            // jump line-by-line until empty one or EOF
            for (; nextLine < endLine; nextLine++) {
