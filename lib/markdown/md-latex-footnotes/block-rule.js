...
var terminatorsWithFence = function (ruler, names, fence) {
    var resolved = resolveEnabledRuleFns(ruler, names);
    var host = ruler;
    var cached = host[WITH_FENCE_KEY];
    if (cached && cached.resolved === resolved) {
        return cached.withFence;
    }
    var withFence = [fence].concat(resolved);
    host[WITH_FENCE_KEY] = { resolved: resolved, withFence: withFence };
    return withFence;
};
