    ...
    for (i = 0, l = list.length; i < l; i++) {
        ...
        if (list[i].hasOwnProperty('arrContents') && list[i].arrContents.length) {
            for (var j = 0; j < list[i].arrContents.length; j++) {
                stateTokens = createFootnotesTokens(state, stateTokens, refTokens, meta, i, list[i].label, list[i].count, list[i].arrContents[j].tokens, list[i].arrContents[j].content, list[i].isBlock);
            }
        }
        else {
            stateTokens = createFootnotesTokens(state, stateTokens, refTokens, meta, i, list[i].label, list[i].count, list[i].tokens, list[i].content, list[i].isBlock);
        }
    }
