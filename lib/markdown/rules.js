...
function injectLineNumbers(tokens, idx, options, env, slf) {
    ...
    if (tokens[idx].uuid) {
        var label = labels_1.getLabelByUuidFromLabelsList(tokens[idx].uuid);
        if (label) {
            utils_1.attrSetToBegin(tokens[idx].attrs, 'number', label.tag);
            utils_1.attrSetToBegin(tokens[idx].attrs, 'class', label.type + " " + label.id);
            utils_1.attrSetToBegin(tokens[idx].attrs, 'id', label.id);
        }
    }
