...
function html_block_injectLineNumbers(tokens, idx, options, env, slf) {
    var _a = options.htmlSanitize, htmlSanitize = _a === void 0 ? {} : _a;
    var line, endLine, listLine;
    if (htmlSanitize !== false) {
        if (tokens[idx] && tokens[idx].content) {
            console.log('tokens[idx]', tokens[idx]);
            console.log('sanitize(tokens[idx].content, htmlSanitize)=>', sanitize_1.sanitize(tokens[idx].content, htmlSanitize));
            tokens[idx].content = sanitize_1.sanitize(tokens[idx].content, htmlSanitize);
        }
    }
    if (tokens[idx].map && tokens[idx].level === 0) {
        line = tokens[idx].map[0];
...
function html_block_Sanitize(tokens, idx, options, env, slf) {
    ...
    return sanitize_1.sanitize(tokens[idx].content, htmlSanitize);
}
function html_inline_Sanitize(tokens, idx, options) {
    ...
    return htmlSanitize && Object.keys(htmlSanitize).length > 0
        ? sanitize_1.sanitize(tokens[idx].content, Object.assign({}, { skipCloseTag: true }, htmlSanitize))
        : sanitize_1.sanitize(tokens[idx].content, { skipCloseTag: true });
}
...
exports.injectRenderRules = function (renderer) {
    var _a = renderer.options, _b = _a.lineNumbering, lineNumbering = _b === void 0 ? false : _b, _c = _a.htmlSanitize, htmlSanitize = _c === void 0 ? {} : _c, _d = _a.html, html = _d === void 0 ? false : _d;
    if (lineNumbering) {
