...
var SvgDrawer = /** @class */ (function () {
        ...
        for (var i = 0; i < rings.length; i++) {
            ...
            if (ring.elements
                ...
                for (var index = 0; index < ring.edges.length; index++) {
                    ...
                    var isHydrogensA = preprocessor.graph.vertices[edges[item].sourceId].value.bracket && Number(preprocessor.graph.vertices[edges[item].sourceId].value.bracket.hcount) > 0;
                    var isHydrogensB = preprocessor.graph.vertices[edges[item].targetId].value.bracket && Number(preprocessor.graph.vertices[edges[item].targetId].value.bracket.hcount) > 0;
                    if (edges[item].isHaveLine) {
                        continue;
                    }
    ...
    SvgDrawer.prototype.isRing_NNN = function (ring) {
        ...
        if ((arr === null || arr === void 0 ? void 0 : arr.length) > 0) {
            ...
            var indexS = this.findStartNbyEdges(members_2, elements);
            if (indexS !== -1) {
    ...
    SvgDrawer.prototype.drawCommonRing = function (ringId, iEdge) {
        var _this = this;
        ...
        if ((commonRings === null || commonRings === void 0 ? void 0 : commonRings.length) > 1) {
            ...
            if ((commonRings === null || commonRings === void 0 ? void 0 : commonRings.length) === 1) {
                ...
                var edges = utils_1.arrayResortFromElement(ring.edges, iEdge);
                edges.map(function (item, index) {
                    var ed = _this.preprocessor.graph.edges[item];
                    if (index > 0) {
                        if (!(index & 1)) {
                            ed.isHaveLine = true;
                        }
