...
var SvgDrawer = /** @class */ (function () {
                    ...
                    if (!prevHaveLine) {
                        ...
                        if (ring.edgesR.indexOf(item) !== -1) {
                            ...
                            if (ring.elements.indexOf('N') !== -1
                                ...
                                ring.isDrawed = true;
                                if (this_3.isNeighboursVerticesHaveDoubleLine(edges[item], ring, true)
                                    ||
                                        this_3.isNeighboursVerticesHaveCommonRing(edges[item], ring)) {
                ...
                neighbours.map(function (item) {
                    ...
                    vertexN.edges.map(function (ed) {
                        if (ring.edges.indexOf(ed) !== -1) {
                            ...
                            if (ring.edgesR.indexOf(edge.id) !== -1) {
                                if (((ring.neighbours.length === 1 && ring.members.length > 5) && _this.isNeighboursVerticesHaveDoubleLine(edge, ring))
                                    ||
                                        (_this.isNeighboursVerticesHaveDoubleLine(edge, ring, true)
                                            || _this.isNeighboursVerticesHaveCommonRing(edge, ring))) {
                                    edge.isNotHaveLine = false;
                                    edge.isHaveLine = true;
                                    _this.drawCommonRing(ring.id, edge.id, {
    ...
    SvgDrawer.prototype.isNeighboursVerticesHaveDoubleLine = function (edge, ring, notCurrentRing) {
        if (notCurrentRing === void 0) { notCurrentRing = false; }
        var graph = this.preprocessor.graph;
        var res = false;
    ...
    SvgDrawer.prototype.edgeCanNotHaveLine = function (edge) {
        ...
        for (var i = 0; i < vertexA.edges.length; i++) {
            var edgeA = this.preprocessor.graph.edges[vertexA.edges[i]];
        ...
        for (var i = 0; i < vertexB.edges.length; i++) {
            var edgeB = this.preprocessor.graph.edges[vertexB.edges[i]];
