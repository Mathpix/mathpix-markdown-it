...
var render_helpers_1 = require("../canvas/render-helpers");
/**
 ...
 */
var renderSmilesSource = function (smiles, options, include_smiles) {
    if (options.forCanvas) {
        return (0, render_helpers_1.smilesSourceForCanvas)(smiles);
    }
    if (include_smiles) {
...
function injectLineNumbersSmiles(tokens, idx, options, env, slf) {
    ...
    if (token.map && token.level === 0) {
        ...
        token.attrJoin("data_line_start", "".concat(String(line)));
        token.attrJoin("data_line_end", "".concat(String(endLine - 1)));
        token.attrJoin("data_line", "".concat(String([line, endLine])));
        token.attrJoin("count_line", "".concat(String(endLine - line)));
    }
...
var smilesDrawerInline = function (state, silent) {
...
var renderSmilesDrawerBlock = function (tokens, idx, options, env, slf) {
    ...
    var id = !options.isTesting
        ? (0, utils_1.uid)()
        : '';
    var resSvg = !options.forCanvas && (include_svg || options.forDocx)
        ? chemistry_drawer_1.ChemistryDrawer.drawSvgSync(token.content.trim(), id, options, options.forPptx)
        : '';
    if (resSvg && (options.forDocx || options.forLatex)) {
        var imgId = options.forLatex ? id : '';
        resSvg = (0, convert_scv_to_base64_1.default)(resSvg, imgId);
    }
    var attrs = (options === null || options === void 0 ? void 0 : options.lineNumbering)
        ? injectLineNumbersSmiles(tokens, idx, options, env, slf)
        : '';
    var outputSmiles = renderSmilesSource(token.content.trim(), options, include_smiles);
    var maxWidth = options.maxWidth ? " max-width: ".concat(options.maxWidth, "; overflow-x: auto;") : '';
    if (attrs) {
        return maxWidth
            ? "<div ".concat(attrs, "><div class=\"smiles\" style=\"").concat(maxWidth, "\">").concat(outputSmiles).concat(resSvg, "</div></div>")
            : "<div ".concat(attrs, "><div class=\"smiles\">").concat(outputSmiles).concat(resSvg, "</div></div>");
    }
    return maxWidth
        ? "<div><div class=\"smiles\" style=\"".concat(maxWidth, "\">").concat(outputSmiles).concat(resSvg, "</div></div>")
        : "<div><div class=\"smiles\">".concat(outputSmiles).concat(resSvg, "</div></div>");
};
var renderSmilesDrawerInline = function (tokens, idx, options, env, slf) {
    ...
    var maxWidth = options.maxWidth ? " max-width: ".concat(options.maxWidth, "; overflow-x: auto;") : '';
    return maxWidth
        ? "<div class=\"smiles-inline\" style=\"display: inline-block;".concat(maxWidth, "\">").concat(outputSmiles).concat(resSvg, "</div>")
        : "<div class=\"smiles-inline\" style=\"display: inline-block;\">".concat(outputSmiles).concat(resSvg, "</div>");
};
