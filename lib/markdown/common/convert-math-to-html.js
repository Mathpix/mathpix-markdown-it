...
exports.convertMathToHtml = exports.clearTypesetCache = void 0;
 ...
 */
var typesetCaches = new WeakMap();
var getInstanceCache = function (options) {
    var cache = typesetCaches.get(options);
    if (!cache) {
...
var getBucket = function (options, isBlock) {
    return getInstanceCache(options).get(!!isBlock);
};
/** Clear the typeset cache for a specific md instance.
 *  Called at the start of every md.parse() via core.ruler hook. */
var clearTypesetCache = function (options) {
    typesetCaches.delete(options);
};
exports.clearTypesetCache = clearTypesetCache;
/**
...
var typesetMathForToken = function (params) {
    ...
    if (isCacheable) {
        var cached = getBucket(options, isBlock).get(math);
        if (cached) {
            return cached;
        }
    }
    ...
    var result = tslib_1.__assign(tslib_1.__assign({}, fmt), { ascii: typeset.ascii, linear: typeset.linear, ascii_tsv: typeset.ascii_tsv, ascii_csv: typeset.ascii_csv, ascii_md: typeset.ascii_md, labels: typeset.labels });
    if (isCacheable) {
        var bucket = getBucket(options, isBlock);
        var raw = options.typesetCacheSize;
        var maxSize = typeof raw === 'number' && raw >= 0 ? raw : consts_2.DEFAULT_TYPESET_CACHE_SIZE;
