...
var TEXT_LIKE_TYPES = new Set(['text', 'text_special', 'emoji']);
...
var WIDE_BELOW_CJK_LO = 0x231A;
var WIDE_BELOW_CJK_HI = 0x2B55;
// One byte per code point of that span (~2.4KB) so the check is an index, not 30 comparisons.
var wideBelowCjkFlags = (function () {
    var e_1, _a;
    var flags = new Uint8Array(WIDE_BELOW_CJK_HI - WIDE_BELOW_CJK_LO + 1);
    try {
        for (var WIDE_RANGES_BELOW_CJK_1 = tslib_1.__values(WIDE_RANGES_BELOW_CJK), WIDE_RANGES_BELOW_CJK_1_1 = WIDE_RANGES_BELOW_CJK_1.next(); !WIDE_RANGES_BELOW_CJK_1_1.done; WIDE_RANGES_BELOW_CJK_1_1 = WIDE_RANGES_BELOW_CJK_1.next()) {
            var _b = tslib_1.__read(WIDE_RANGES_BELOW_CJK_1_1.value, 2), from = _b[0], to = _b[1];
        ...
        try {
            if (WIDE_RANGES_BELOW_CJK_1_1 && !WIDE_RANGES_BELOW_CJK_1_1.done && (_a = WIDE_RANGES_BELOW_CJK_1.return)) _a.call(WIDE_RANGES_BELOW_CJK_1);
        }
 ...
 */
var isWideChar = function (cp) { return !isZeroWidthChar(cp) && inWideRange(cp); };
 ...
 */
var tokenMarkerWidth = function (token, depth) {
    var _a, _b;
    ...
    if (token.type && MATH_TOKEN_TYPES.has(token.type)) {
        return typeof token.widthEx === 'number' ? token.widthEx * consts_1.EX_TO_EM : 0;
    }
    ...
    if (token.children && token.children.length) {
        ...
        for (var i = 0; i < token.children.length; i++) {
            em += (0, exports.tokenMarkerWidth)(token.children[i], depth + 1);
        }
        return em;
    }
