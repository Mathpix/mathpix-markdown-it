...
var findVerbatimRanges = function (text) {
    ...
    var spans = ranges.map(function (range) { return [range[0], range[1]]; });
    try {
        for (var _c = tslib_1.__values((0, common_1.getInlineCodeListFromString)(text)), _d = _c.next(); !_d.done; _d = _c.next()) {
            var item = _d.value;
    ...
    for (var i = 0; i <= ranges.length; i++) {
        ...
        while (blockFrom < gapTo) {
            ...
            while (seek < blockTo) {
                ...
                var span = (0, math_spans_1.nextMathSpan)(text, seek, true, blockTo, lastEndAt);
                if (!span) {
                    break;
                }
                // Envs only, now that `nextMathSpan` rejects an inline pair reaching past the window: an env
                // body crossing a blank line is covered up to that line.
