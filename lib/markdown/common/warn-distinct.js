...
var warnIfConsoleCan = function (message) {
    var args = [];
    for (var _i = 1; _i < arguments.length; _i++) {
        args[_i - 1] = arguments[_i];
    }
    if (typeof console !== 'undefined' && typeof console.warn === 'function') {
        console.warn.apply(console, tslib_1.__spreadArray([message], tslib_1.__read(args), false));
    }
};
...
var warnDistinct = function (key, message) {
    var _a;
    var args = [];
    ...
    if (usedByFamily >= MAX_KEYS_PER_FAMILY) {
        if (!familyCapReported.has(family)) {
            ...
            warnIfConsoleCan("[mmd] more than ".concat(MAX_KEYS_PER_FAMILY, " distinct '").concat(family, "' diagnostics in one render; the rest of that family are silent"));
        }
        return;
    }
    if (warned.size >= MAX_DISTINCT_KEYS) {
        ...
        if (!capReported) {
            capReported = true;
            warnIfConsoleCan("[mmd] more than ".concat(MAX_DISTINCT_KEYS, " distinct diagnostics in one render; the rest are silent"));
        }
        return;
    }
    warned.add(key);
    perFamily.set(family, usedByFamily + 1);
    warnIfConsoleCan.apply(void 0, tslib_1.__spreadArray([message], tslib_1.__read(args), false));
};
