...
var headingSection = function (state, startLine /*, endLine*/) {
    ...
    var _b = findEndMarker(lineText, startPos), _c = _b.res, res = _c === void 0 ? false : _c, _d = _b.content, content = _d === void 0 ? '' : _d;
    if (!res) {
        return false;
    }
...
var findEndMarker = function (str, startPos, beginMarker, endMarker) {
...
var textAuthor = function (state) {
    ...
    for (var i = 0; i < columns.length; i++) {
        var colArr = columns[i].trim().split('\\\\');
        var column = colArr.join('\n');
        var newToken = {};
...
var renderAuthorToken = function (tokens, index, options, env, slf) {
    ...
    if (token.children && token.children.length) {
        for (var i = 0; i < token.children.length; i++) {
            var tok = token.children[i];
            var content = renderInlineContent(tok, options, env, slf);
            if (attrStyle) {
