...
var headingSection = function (state, startLine, endLine) {
    ...
    if (!res) {
        for (; nextLine <= endLine; nextLine++) {
            ...
            lineText = state.src.slice(pos, max);
            var _f = common_1.findEndMarker(lineText, -1, "{", "}", true), _g = _f.res, res_1 = _g === void 0 ? false : _g, _h = _f.content, content_1 = _h === void 0 ? '' : _h, _j = _f.nextPos, nextPos_1 = _j === void 0 ? 0 : _j;
            if (res_1) {
                resString += resString ? ' ' : '';
...
var pageBreaksBlock = function (state, startLine) {
    var token, lineText, pos = state.bMarks[startLine] + state.tShift[startLine], max = state.eMarks[startLine];
    var nextLine = startLine + 1;
    var startPos = 0;
    lineText = state.src.slice(pos, max).trim();
