...
var textAuthor = function (state, silent) {
    ...
    startPos += match[0].length;
    var _d = (0, common_1.findEndMarker)(state.src, startPos), _e = _d.res, res = _e === void 0 ? false : _e, _f = _d.content, content = _f === void 0 ? '' : _f, _g = _d.nextPos, nextPos = _g === void 0 ? 0 : _g;
    if (!res) {
        return false;
    }
    var type = "author";
    var arrtStyle = structural_1.AUTHOR_STYLE;
    if (!silent) {
        var token = state.push(type, "", 0);
        /** Canvas takes no stylesheet, so the structure of the block has to travel inline. */
        if ((((_a = state.md.options) === null || _a === void 0 ? void 0 : _a.forDocx) || ((_b = state.md.options) === null || _b === void 0 ? void 0 : _b.forCanvas)) && arrtStyle) {
        ...
        for (var i = 0; i < columns.length; i++) {
            ...
            if (colArr && colArr.length) {
                for (var j = 0; j < colArr.length; j++) {
                    ...
                    newToken.offsetLeft = ((_c = colArr[j].trim()) === null || _c === void 0 ? void 0 : _c.length) ? (0, utils_1.getSpacesFromLeft)(colArr[j]) : 0;
...
var getAuthorItemToken = function (tokens, index, options, env, slf) {
    ...
    var attrStyle = options.forDocx
        ? " ".concat(structural_1.AUTHOR_ITEM_STYLE)
        : options.forCanvas ? structural_1.AUTHOR_ITEM_STYLE : '';
    res += attrStyle
...
var getAuthorColumnToken = function (tokens, index, options, env, slf) {
    var res = '';
    var token = tokens[index];
    var attrStyle = options.forDocx || options.forCanvas
        ? structural_1.AUTHOR_COLUMN_STYLE
        : '';
    ...
    if (attrStyle) {
        res += "<p style=\"".concat(attrStyle, "\">").concat(content, "</p>");
    }
    else {
        res += "<p>".concat(content, "</p>");
    }
    return res;
};
var renderAuthorToken = function (tokens, index, options, env, slf) {
    var token = tokens[index];
    var divStyle = options.forDocx || options.forCanvas
        ? token.attrGet('style')
        : '';
    var res = token.children && token.children.length
        ...
        : (0, exports.renderInlineContent)(token, options, env, slf);
    if (divStyle) {
        return "<div class=\"author\" style=\"".concat(divStyle, "\">\n          ").concat(res, "\n        </div>");
    }
    else {
...
var renderTextError = function (tokens, idx, options, env, slf) {
    if (env === void 0) { env = {}; }
    var token = tokens[idx];
    var style = 'color: red; background-color: yellow;';
    if (options.parserErrors === mathpix_markdown_model_1.ParserErrors.hide) {
        style += ' display: none;';
    }
    return "<span style=\"".concat(style, "\">").concat(token.content, "</span>");
};
var renderTextIcon = function (tokens, idx, options, env, slf) {
    if (env === void 0) { env = {}; }
    var token = tokens[idx];
    var attrs = slf.renderAttrs(token);
    return "<span ".concat(attrs, ">").concat(token.content, "</span>");
};
...
exports.default = (function () {
    return function (md) {
        ...
        md.block.ruler.before("lheading", "abstractBlock", abstractBlock, { alt: (0, common_1.getTerminatedRules)('abstractBlock') });
        md.block.ruler.before("lheading", "pageBreaksBlock", pageBreaksBlock, { alt: (0, common_1.getTerminatedRules)('pageBreaksBlock') });
        ...
        md.inline.ruler.before('textTypes', 'textMode', text_mode_1.textMode);
        ...
        md.renderer.rules.link_open = function (tokens, idx, options, env, self) {
            ...
            var pushWordBreak = function (value) {
