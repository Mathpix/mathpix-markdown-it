...
exports.newTheoremBlock = function (state, startLine) {
    ...
    if (!testNewTheorem.test(content) && !consts_1.reNewCommandQedSymbolG.test(content)) {
        return false;
    }
    var children = [];
    ...
    for (var j = 0; j < children.length; j++) {
        ...
        if (((itemBefore === null || itemBefore === void 0 ? void 0 : itemBefore.type) === "newtheorem" || (itemBefore === null || itemBefore === void 0 ? void 0 : itemBefore.type) === "theoremstyle" || (itemBefore === null || itemBefore === void 0 ? void 0 : itemBefore.type) === "renewcommand_qedsymbol")
            && item.type === "softbreak") {
            itemBefore = item;
            if (state.md.options.forLatex) {
                token.children.push(item);
...
exports.BeginTheorem = function (state, startLine, endLine) {
    var _a, _b;
    ...
    token.envStyle = helper_2.theoremEnvironments[envIndex].style;
    if (state.md.options.forLatex) {
        token.latex = latexBegin;
    }
    if (label) {
        var index = helper_2.envNumbers.findIndex(function (item) { return item.label === label; });
        if (index === -1) {
            helper_2.envNumbers.push({
                label: label,
...
exports.BeginProof = function (state, startLine, endLine) {
    var pos = state.bMarks[startLine] + state.tShift[startLine];
    var max = state.eMarks[startLine];
    var token;
    var nextLine = startLine + 1;
