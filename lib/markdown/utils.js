...
exports.skipBackticks = exports.findFirstTagContentWithNesting = exports.isEscapedAt = exports.applyAttrToInlineMath = exports.applyAttrToMathml = exports.isMathInText = exports.addAttributesToParentToken = exports.addAttributesToParentTokenByType = exports.getSpacesFromLeft = exports.canonicalMathPositions = exports.canonicalMath = exports.findOpenCloseTagsMathEnvironment = exports.findOpenCloseTags = exports.findBackTick = exports.attrSetToBegin = exports.uid = exports.arrayResortFromElement = exports.arrayDelElement = exports.arraysCompare = exports.includesMultiMathTag = exports.includesMultiMathBeginTag = exports.hasOddSimpleMathTag = exports.includesSimpleMathTag = exports.isNotBackticked = exports.getLatexTextWidth = exports.getWidthFromDocument = exports.beginTag = exports.endTag = exports.getCachedSrcPositions = void 0;
var tslib_1 = require("tslib");
var consts_1 = require("./common/consts");
// Per-state cache of `needle` offsets in `state.src`; pinned to state so nested `block.parse()` gets its own.
var getCachedSrcPositions = function (state, key, needles) {
    var e_1, _a;
    ...
    var positions = [];
    try {
        for (var needles_1 = tslib_1.__values(needles), needles_1_1 = needles_1.next(); !needles_1_1.done; needles_1_1 = needles_1.next()) {
...
exports.getCachedSrcPositions = getCachedSrcPositions;
        ...
        for (var i = 0; i < str.length; i++) {
            ...
            if ((0, exports.isNotBackticked)(str, match[0])) {
                ...
                if (match[0] === "\\[" || match[0] === "\[") {
                    ...
                    if (consts_1.latexEnvironments.includes(match[1])) {
                        return null;
                    }
    ...
    try {
        ...
        if (token.children.find(function (item) { return item.type === tokenType; })) {
            if ((_a = parentToken.attrs) === null || _a === void 0 ? void 0 : _a.length) {
                if (parentToken.attrs[consts_1.attrsSharedMarker]) {
                    parentToken.attrs = parentToken.attrs.map(function (p) { return [p[0], p[1]]; });
                }
...
var findFirstTagContentWithNesting = function (str, tagOpen, tagClose, pendingBackTick, noBreakBackTick) {
    if (pendingBackTick === void 0) { pendingBackTick = ''; }
    if (noBreakBackTick === void 0) { noBreakBackTick = false; }
    var max = str.length;
    var stack = [];
    ...
    for (var pos = posStart; pos < max; pos++) {
        ...
        if (ch !== 0x5c /* \ */)
            continue;
        // --- try open ---
        var openMatch = str.slice(pos).match(tagOpen);
        if (openMatch && openMatch.index === 0) {
            var content = openMatch[0];
            var open_1 = {
                posStart: pos,
        ...
        // --- try close ---
        var closeMatch = tagClose ? str.slice(pos).match(tagClose) : null;
        if (closeMatch && closeMatch.index === 0) {
            var content = closeMatch[0];
            var close_1 = {
                posStart: pos,
                posEnd: pos + content.length,
                content: content
            ...
            var top_1 = stack.pop();
            if (top_1) {
                var contentPosStart = top_1.open.posEnd;
                var contentPosEnd = close_1.posStart;
                var block = {
                    open: top_1.open,
                    close: close_1,
                    content: str.slice(contentPosStart, contentPosEnd),
                    contentPosStart: contentPosStart,
                    contentPosEnd: contentPosEnd,
                    level: top_1.level
                };
                // If closed the outermost block, return it immediately and stop searching.
                if (top_1.level === 0) {
                    return { block: block, pending: '' };
                }
            }
            pos = close_1.posEnd - 1;
        }
    }
    return { block: null, pending: pending };
};
exports.findFirstTagContentWithNesting = findFirstTagContentWithNesting;
/**
 ...
 */
var skipBackticks = function (src, pos) {
    if (src.charCodeAt(pos) !== 0x60 /* ` */) {
        return pos;
    }
    if ((0, exports.isEscapedAt)(src, pos)) {
        return pos;
    }
    var tickCount = 0;
    ...
    var open = "`".repeat(tickCount);
    var start = pos + tickCount;
    var end = src.indexOf(open, start);
    if (end === -1) {
        return src.length;
    }
    return end + tickCount;
};
exports.skipBackticks = skipBackticks;
