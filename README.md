...
| `mathDelimiterMode`                   | "strict" \| "legacy";*`strict`* | Controls whether double-backslash delimiters `\\( ... \\)` / `\\[ ... \\]` are treated as math. `strict` (default): only single-backslash `\(`/`\[` (and `$`/`$$`) are math; `\\(` renders as literal. `legacy`: also accept `\\(`/`\\[` as math (pre-3.0.0 behavior). Any value other than `legacy` is treated as `strict`. |
